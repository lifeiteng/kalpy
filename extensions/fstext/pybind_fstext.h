...
class PyFst : public Fst<A> {
 public:
  ...
  typename Fst<A>::StateId Start() const override {
    PYBIND11_OVERRIDE_PURE(int,     // Return type (ret_type)
                           Fst<A>,  // Parent class (cname)
                           Start  // Name of function in C++ (must match Python
                                  // name) (fn) Argument(s) (...)
    );
  }
  ...
  size_t NumInputEpsilons(typename Fst<A>::StateId s) const override {
    PYBIND11_OVERRIDE_PURE(size_t,            // Return type (ret_type)
                           Fst<A>,            // Parent class (cname)
  ...
  const std::string &Type() const override {
    PYBIND11_OVERRIDE_PURE(const std::string &,  // Return type (ret_type)
                           Fst<A>,               // Parent class (cname)
  ...
  const SymbolTable *InputSymbols() const override {
    PYBIND11_OVERRIDE_PURE(const SymbolTable *,  // Return type (ret_type)
                           Fst<A>,               // Parent class (cname)
                           InputSymbols  // Name of function in C++ (must match
  ...
  const SymbolTable *OutputSymbols() const override {
    PYBIND11_OVERRIDE_PURE(const SymbolTable *,  // Return type (ret_type)
                           Fst<A>,               // Parent class (cname)
                           OutputSymbols  // Name of function in C++ (must match
                                          // Python name) (fn) Argument(s) (...)
    );
  }
  ...
  void InitArcIterator(typename Fst<A>::StateId s,
...
class PyMutableFst : public MutableFst<A> {
 public:
  ...
  typename MutableFst<A>::Weight Final(
      typename MutableFst<A>::StateId s) const override {
    PYBIND11_OVERRIDE_PURE(
  ...
  void InitArcIterator(typename MutableFst<A>::StateId s,
                       ArcIteratorData<A> *data) const override {
    PYBIND11_OVERRIDE_PURE(void,             // Return type (ret_type)
                           MutableFst<A>,    // Parent class (cname)
                           InitArcIterator,  // Name of function in C++ (must
  ...
  void SetFinal(typename MutableFst<A>::StateId s,
                typename MutableFst<A>::Weight weight =
                    MutableFst<A>::Weight::One()) override {
    PYBIND11_OVERRIDE_PURE(
        void,           // Return type (ret_type)
        MutableFst<A>,  // Parent class (cname)
  ...
  void DeleteStates(
      const std::vector<typename MutableFst<A>::StateId> &s) override {
    PYBIND11_OVERRIDE_PURE(
        void,           // Return type (ret_type)
        MutableFst<A>,  // Parent class (cname)
  ...
  void DeleteStates() override {
    PYBIND11_OVERRIDE_PURE(void,           // Return type (ret_type)
                           MutableFst<A>,  // Parent class (cname)
                           DeleteStates  // Name of function in C++ (must match
                                         // Python name) (fn) Argument(s) (...)
    );
  }
  ...
  SymbolTable *MutableInputSymbols() override {
    PYBIND11_OVERRIDE_PURE(
        ...
        MutableFst<A>,       // Parent class (cname)
        MutableInputSymbols  // Name of function in C++ (must match Python name)
                             // (fn) Argument(s) (...)
    );
  }
  ...
  SymbolTable *MutableOutputSymbols() override {
    PYBIND11_OVERRIDE_PURE(
        SymbolTable *,        // Return type (ret_type)
        MutableFst<A>,        // Parent class (cname)
        MutableOutputSymbols  // Name of function in C++ (must match Python
                              // name) (fn) Argument(s) (...)
    );
  }
...
void RemoveArcsWithSomeInputSymbols(const std::vector<I> &symbols_in,
  ...
  for (StateId s = 0; s < num_states; s++) {
    for (MutableArcIterator<VectorFst<Arc>> iter(fst, s); !iter.Done();
         iter.Next()) {
  ...
  Connect(fst);
  if (fst->NumStates() == 0) KALDI_WARN << "After Connect(), fst was empty.";
}

template <class Arc, class I>
void PenalizeArcsWithSomeInputSymbols(const std::vector<I> &symbols_in,
                                      float penalty, VectorFst<Arc> *fst) {
  typedef typename Arc::StateId StateId;
...
void pybind_expanded_fst_impl(py::module &m, const std::string &class_name,
  ...
  py::class_<PyClass, fst::Fst<A>, PyExpandedFst<A>>(m, class_name.c_str(),
                                                     class_help_doc.c_str())
      .def(py::init<>());
}
...
void pybind_mutable_fst_impl(py::module &m, const std::string &class_name,
  ...
  py::class_<PyClass, fst::ExpandedFst<A>, PyMutableFst<A>>(
      m, class_name.c_str(), class_help_doc.c_str())
      .def(py::init<>());
}
...
void pybind_vector_fst_impl(py::module &m, const std::string &class_name,
  ...
  auto c = py::class_<PyClass, fst::MutableFst<A>>(m, class_name.c_str(),
                                                   class_help_doc.c_str());
  c.def(py::init<>())
      ...
      .def("AddArc", (void(PyClass::*)(StateId, const Arc &))(&PyClass::AddArc),
           py::arg("s"), py::arg("arc"))
      .def("DeleteStates",
      ...
      .def_static("Read", overload_cast_<std::string_view>()(&PyClass::Read),
                  "Read a VectorFst from a file, returning nullptr on error; "
                  "empty "
                  "filename reads from standard input.",
                  py::arg("source"), py::return_value_policy::reference,
                  py::call_guard<py::gil_scoped_release>())
      .def("Write",
           // clang-format off
      ...
      .def("Write",
           ...
           py::arg("filename"), py::call_guard<py::gil_scoped_release>())
      ...
      .def("Connect", [](PyClass *f) { fst::Connect<Arc>(f); })
      /*.def("to_pynini", [](PyClass& f){
       ...
       py_fst.__pyx_base._mfst =
     std::shared_ptr<fst::script::MutableFstClass>(&vf); return py_fst;
     }, py::return_value_policy::take_ownership)*/
      .def_static(
          "from_pynini",
          [](py::object fst) {
...
void pybind_lattice_fst_impl(py::module &m, const std::string &class_name,
  ...
  c.def(py::init<>())
      ...
      .def(
          "ScaleLattice",
          [](PyClass *f, BaseFloat acoustic_scale = 1.0,
          ...
          py::arg("acoustic_scale") = 1.0, py::arg("lm_scale") = 1.0)
      .def("TopSort",
           [](PyClass *f) {
             py::gil_scoped_release release;
             kaldi::uint64 props = f->Properties(fst::kFstProperties, false);
             if (!(props & fst::kTopSorted)) {
               if (!fst::TopSort(f)) KALDI_ERR << "Cycles detected in lattice.";
             }
           })
      .def("write_to_string",
           [](const PyClass &f) {
      ...
      .def_static(
          ...
          py::arg("bytes"), py::return_value_policy::reference);
}
...
void pybind_const_fst_impl(py::module &m, const std::string &class_name,
  ...
  py::class_<PyClass, fst::ExpandedFst<A>>(m, class_name.c_str(),
                                           class_help_doc.c_str())
      .def(py::init<>())
      .def(py::init<const fst::Fst<Arc> &>(), py::arg("fst"))
      .def(py::init<const PyClass &, bool>(), py::arg("fst"),
