...
void pybind_lattice_weight_impl(py::module &m, const std::string &class_name,
  ...
  m.def("ScaleTupleWeight",
        (PyClass(*)(const PyClass &,
                    const std::vector<std::vector<FloatType>> &))(
...

  {
    ...
    py::class_<PyClass>(m, "VectorFstHolder")
        ...
        .def_static("Write", &PyClass::Write, py::arg("os"), py::arg("binary"),
                    py::arg("t"), py::call_guard<py::gil_scoped_release>())
...
void pybind_table_matcher(py::module &m) {
  ...
  auto tm = py::class_<TableMatcher<F>, MatcherBase<typename F::Arc>>(
      m, "TableMatcher", "TableMatcher");
  tm.def(py::init<const F &, MatchType, const TableMatcherOptions &>())
...
void pybind_table_compose(py::module &m) {
  m.def("TableCompose",
        (void (*)(const Fst<Arc> &, const Fst<Arc> &, MutableFst<Arc> *,
                  const TableComposeOptions &))(&TableCompose<Arc>),
        py::arg("ifst1"), py::arg("ifst2"), py::arg("ofst"),
        py::arg("opts") = TableComposeOptions());
  m.def("TableCompose",
        (void (*)(const Fst<Arc> &, const Fst<Arc> &, MutableFst<Arc> *,
                  TableComposeCache<Fst<Arc>> *))(&TableCompose<Arc>),
        py::arg("ifst1"), py::arg("ifst2"), py::arg("ofst"), py::arg("cache"));
  ...
  m.def(
      ...
      py::arg("compose_filter") = "sequence", py::arg("cache") = nullptr);
}

void pybind_fstext_context_fst(py::module &m) {
  using namespace fst;
  m.def("WriteILabelInfo", &WriteILabelInfo,
        "Utility function for writing ilabel-info vectors to disk.",
        py::arg("os"), py::arg("binary"), py::arg("ilabel_info"));
  m.def("ReadILabelInfo", &ReadILabelInfo,
        "Utility function for reading ilabel-info vectors from disk.",
        py::arg("is"), py::arg("binary"), py::arg("ilabel_info"));
  m.def("CreateILabelInfoSymbolTable", &CreateILabelInfoSymbolTable,
        "The following function is mainly of use for printing and debugging.",
        py::arg("ilabel_info"), py::arg("phones_symtab"), py::arg("separator"),
        py::arg("disambig_prefix"));
  m.def(
      "ComposeContext",
      py::overload_cast<const std::vector<int32> &, int32, int32,
                        VectorFst<StdArc> *, VectorFst<StdArc> *,
                        std::vector<std::vector<int32>> *, bool>(
          &ComposeContext),
      "Used in the command-line tool fstcomposecontext.  It creates a context "
      "FST and "
      "composes it on the left with \"ifst\" to make \"ofst\".  It outputs the "
      "label "
      "information to ilabels_out.  \"ifst\" is mutable because we need to add "
      "the "
      "subsequential loop. "
      "\n"
      "@param [in] disambig_syms  List of disambiguation symbols, e.g. the "
      "integer "
      "            ids of #0, #1, #2 ... in the phones.txt. "
      "@param [in] context_width  Size of context window, e.g. 3 for triphone. "
      "@param [in] central_position  Central position in phonetic context "
      "window "
      ...
      "@param [out] ilabels_out  Vector, indexed by ilabel of CLG.fst, "
      ...
      "                  in the program 'fstmakecontextfst'... if true, it "
      "will "
      "                  project on the input after adding the subsequential "
      "loop "
      "                  to 'ifst', which allows us to reconstruct the context "
      "                  fst C.fst.",
      py::arg("disambig_syms"), py::arg("context_width"),
      py::arg("central_position"), py::arg("ifst"), py::arg("ofst"),
      py::arg("ilabels_out"), py::arg("project_ifst") = false);
  m.def(
      "AddSubsequentialLoop", &AddSubsequentialLoop,
      "Modifies an FST so that it transuces the same paths, but the input side "
      "of the "
      "paths can all have the subsequential symbol '$' appended to them any "
      "number of "
      "times (we could easily specify the number of times, but accepting any "
      "number of "
      "repetitions is just more convenient).  The actual way we do this is for "
      "each "
      "final state, we add a transition with weight equal to the final-weight "
      "of that "
      "state, with input-symbol '$' and output-symbols <eps>, and ending in a "
      "new "
      "super-final state that has unit final-probability and a unit-weight "
      "self-loop "
      "with '$' on its input and <eps> on its output.  The reason we don't "
      "just "
      "add a loop to each final-state has to do with preserving stochasticity "
      "(see \\ref fst_algo_stochastic).  We keep the final-probability in all "
      "the "
      "original final-states rather than setting them to zero, so the "
      "resulting FST "
      "can accept zero '$' symbols at the end (in case we had no right "
      "context).",
      py::arg("subseq_symbol"), py::arg("fst"));
  {
    using PyClass = InverseContextFst;

    auto inverse_context_fst =
        py::class_<InverseContextFst, DeterministicOnDemandFst<StdArc>>(
            m, "InverseContextFst");
    inverse_context_fst
        .def(py::init<PyClass::Label, const std::vector<int32> &,
                      const std::vector<int32> &, int32, int32>(),
             py::arg("subsequential_symbol"), py::arg("phones"),
             py::arg("disambig_syms"), py::arg("context_width"),
             py::arg("central_position"))
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("arc"))
...
void pybind_fstext_deterministic_fst(py::module &m) {
  {
    ...
    auto deterministic_on_demand_fst =
        py::class_<DeterministicOnDemandFst<StdArc>,
                   PyDeterministicOnDemandFst>(m, "DeterministicOnDemandFst");
    deterministic_on_demand_fst.def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  {
    ...
    auto backoff_deterministic_on_demand_fst =
        py::class_<BackoffDeterministicOnDemandFst<StdArc>,
                   DeterministicOnDemandFst<StdArc>>(
            m, "BackoffDeterministicOnDemandFst");
    backoff_deterministic_on_demand_fst
        .def(py::init<const Fst<StdArc> &>(), py::arg("fst"))
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  {
    ...
    auto scaled_deterministic_on_demand_fst =
        py::class_<ScaleDeterministicOnDemandFst,
                   DeterministicOnDemandFst<StdArc>>(
            m, "ScaleDeterministicOnDemandFst");
    scaled_deterministic_on_demand_fst
        .def(py::init<float, DeterministicOnDemandFst<StdArc> *>(),
  ...
  }
  {
    ...
    auto unweighted_ngram_fst =
        py::class_<UnweightedNgramFst<StdArc>,
                   DeterministicOnDemandFst<StdArc>>(m, "UnweightedNgramFst");
    unweighted_ngram_fst.def(py::init<int>(), py::arg("n"))
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  {
    ...
    auto compose_deterministic_on_demand_fst =
        py::class_<ComposeDeterministicOnDemandFst<StdArc>,
                   DeterministicOnDemandFst<StdArc>>(
            m, "ComposeDeterministicOnDemandFst");
    compose_deterministic_on_demand_fst
        .def(py::init<DeterministicOnDemandFst<StdArc> *,
                      DeterministicOnDemandFst<StdArc> *>(),
             py::arg("fst1"), py::arg("fst2"))
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  {
    ...
    auto cache_deterministic_on_demand_fst =
        py::class_<CacheDeterministicOnDemandFst<StdArc>,
                   DeterministicOnDemandFst<StdArc>>(
            m, "CacheDeterministicOnDemandFst");
    cache_deterministic_on_demand_fst
        .def(py::init<DeterministicOnDemandFst<StdArc> *, PyClass::StateId>(),
             py::arg("fst"), py::arg("num_cached_arcs") = 100000)
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  {
    ...
    auto lm_example_deterministic_on_demand_fst =
        py::class_<LmExampleDeterministicOnDemandFst<StdArc>,
                   DeterministicOnDemandFst<StdArc>>(
            m, "LmExampleDeterministicOnDemandFst");
    lm_example_deterministic_on_demand_fst
        .def(py::init<void *, PyClass::Label, PyClass::Label>(), py::arg("lm"),
             py::arg("bos_symbol"), py::arg("eos_symbol"))
        .def("Start", &PyClass::Start)
        .def("Final", &PyClass::Final, py::arg("s"))
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("oarc"));
  }
  m.def("ComposeDeterministicOnDemand", &ComposeDeterministicOnDemand<StdArc>,
        "Compose an FST (which may be a lattice) with a "
        "DeterministicOnDemandFst and "
        "store the result in fst_composed.  This is mainly used for expanding "
        "lattice "
        "n-gram histories, where fst1 is a lattice and fst2 is an "
        "UnweightedNgramFst. "
        "This does not call Connect.",
        py::arg("fst1"), py::arg("fst2"), py::arg("fst_composed"));
  m.def(
...
void pybind_fstext_deterministic_lattice(py::module &m) {
  ...
  m.def("DeterminizeLattice",
        py::overload_cast<const Fst<ArcTpl<TropicalWeight>> &,
                          ...
                          DeterminizeLatticeOptions, bool *>(
            &DeterminizeLattice<TropicalWeight, int32>),
        "This function implements the normal version of DeterminizeLattice, in "
        "which "
        "the output strings are represented using sequences of arcs, where all "
        "but "
        "the first one has an epsilon on the input side.  The debug_ptr "
        "argument is "
        "an optional pointer to a bool that, if it becomes true while the "
        "algorithm "
        "is executing, the algorithm will print a traceback and terminate "
        "(used in "
        "fstdeterminizestar.cc debug non-terminating determinization).  More "
        "efficient if ifst is arc-sorted on input label.  If the number of "
        "arcs gets "
        "more than max_states, it will throw std::runtime_error (otherwise "
        "this code "
        "does not use exceptions).  This is mainly useful for debug.",
        py::arg("ifst"), py::arg("ofst"),
        py::arg("opts") = DeterminizeLatticeOptions(),
        py::arg("debug_ptr") = NULL);
  m.def(
      "DeterminizeLattice",
      py::overload_cast<
          const Fst<ArcTpl<TropicalWeight>> &,
          MutableFst<ArcTpl<CompactLatticeWeightTpl<TropicalWeight, int32>>> *,
          DeterminizeLatticeOptions, bool *>(
          &DeterminizeLattice<TropicalWeight, int32>),
      "This is a version of DeterminizeLattice with a slightly more "
      "\"natural\" output format, "
      "where the output sequences are encoded using the CompactLatticeArcTpl "
      "template "
      "(i.e. the sequences of output symbols are represented directly as "
      "strings) "
...
void pybind_fstext_determinize_star(py::module &m) {
  m.def(
      "DeterminizeStar",
      py::overload_cast<VectorFst<StdArc> &, MutableFst<StdArc> *, float,
                        bool *, int, bool>(&DeterminizeStar<VectorFst<StdArc>>),
      "This function implements the normal version of DeterminizeStar, in "
      "which the "
      "output strings are represented using sequences of arcs, where all but "
      "the "
      "first one has an epsilon on the input side.  The debug_ptr argument is "
      "an "
      "optional pointer to a bool that, if it becomes true while the algorithm "
      "is "
      ...
      "exception as soon as the max-states is reached. This can be useful in "
      "test. "
      "If allow_partial is true, the algorithm will output partial results "
      "when the "
      "specified max_states is reached (when larger than zero), instead of "
      "throwing "
      "out an error. "
      "\n"
      ...
      py::arg("ifst"), py::arg("ofst"), py::arg("delta") = kDelta,
      py::arg("debug_ptr") = NULL, py::arg("max_states") = -1,
  ...
  m.def(
      ...
      py::overload_cast<VectorFst<StdArc> &, MutableFst<GallicArc<StdArc>> *,
                        float, bool *, int, bool>(
          &DeterminizeStar<VectorFst<StdArc>>),
      "This is a version of DeterminizeStar with a slightly more \"natural\" "
      "output format, "
      "where the output sequences are encoded using the GallicArc (i.e. the "
      "output symbols "
      "are strings. "
      "If max_states is positive, it will stop determinization and throw an "
      "exception as soon as the max-states is reached.  This can be useful in "
...
void pybind_fstext_epsilon_property(py::module &m) {
  ...
  py::enum_<EpsilonInfo>(m, "EpsilonInfo")
      .value("kStateHasEpsilonArcsEntering",
             EpsilonInfo::kStateHasEpsilonArcsEntering)
      .value("kStateHasNonEpsilonArcsEntering",
             EpsilonInfo::kStateHasNonEpsilonArcsEntering)
      .value("kStateHasEpsilonArcsLeaving",
             EpsilonInfo::kStateHasEpsilonArcsLeaving)
      .value("kStateHasNonEpsilonArcsLeaving",
             EpsilonInfo::kStateHasNonEpsilonArcsLeaving)
      .export_values();

  m.def("ComputeStateInfo",
        py::overload_cast<const VectorFst<StdArc> &, std::vector<char> *>(
            &ComputeStateInfo<StdArc>),
        ...
        py::arg("fst"), py::arg("epsilon_info"));

  m.def("EnsureEpsilonProperty",
        py::overload_cast<VectorFst<StdArc> *>(&EnsureEpsilonProperty<StdArc>),
        "This function modifies the fst (while maintaining equivalence) in "
        "such a way "
        "that, after the modification, all states of the FST which have "
        "epsilon-arcs "
        "entering them, have no non-epsilon arcs entering them, and all states "
        "which "
        "have epsilon-arcs leaving them, have no non-epsilon arcs leaving "
        "them.  It does "
        "this by creating extra states and adding extra epsilon transitions.  "
        "An epsilon "
        "arc is defined as an arc where both the ilabel and the olabel are "
        "epsilons. "
        "This function may fail with KALDI_ASSERT for certain cyclic FSTs, but "
        "is safe "
        "in the acyclic case.",
        py::arg("fst"));
}
void pybind_fstext_factor(py::module &m) {
  m.def(
      ...
      "\n"
      "As a side effect it also sorts the FST in depth-first order.  Factor "
      "will "
      ...
      "\n"
      "This is because it only creates a chain with olabels on the first arc "
      "of the "
      "chain (or a chain with no olabels). [it's possible to construct cases "
      "where "
      "pushing makes things worse, though].  After Factor, the composition of "
      "*ofst "
      "with the result of calling CreateFactorFst(*symbols) should be "
      "equivalent to "
      "fst.  Alternatively, calling ExpandInputSequences with ofst and "
      "*symbols "
      "would produce something equivalent to fst.",
      py::arg("fst"), py::arg("ofst"), py::arg("symbols"));
  ...
  m.def("Factor",
        ...
        py::arg("fst"), py::arg("ofst1"), py::arg("ofst2"));

  m.def("ExpandInputSequences",
        py::overload_cast<const std::vector<std::vector<int32>> &,
                          MutableFst<StdArc> *>(
            &ExpandInputSequences<StdArc, int32>),
        "ExpandInputSequences expands out the input symbols into sequences of "
        "input "
        "symbols.  It creates linear chains of states for each arc that had >1 "
        "augmented symbol on it.  It also sets the input symbol table to NULL, "
        "since "
        "in case you did have a symbol table there it would no longer be "
        "valid.  It "
        "leaves any weight and output symbols on the first arc of the chain.",
        py::arg("sequences"), py::arg("fst"));

  m.def("CreateFactorFst", &CreateFactorFst<StdArc, int32>,
        "The function CreateFactorFst will create an FST that expands out the "
        "\"factors\" that are the indices of the \"sequences\" array, into "
        "linear sequences "
        "of symbols.  There is a single start and end state (state 0), and for "
        "each "
        "nonzero index i into the array \"sequences\", there is an arc from "
        "state 0 that "
        "has output-label i, and enters a chain of states with output epsilons "
        "and input "
        "labels corresponding to the remaining elements of the sequences, "
        "terminating "
        "again in state 0.  This FST is output-deterministic and sorted on "
        "olabel. "
        "Composing an FST on the left with the output of this function, should "
        "be the "
  ...
  m.def(
      "CreateMapFst", &CreateMapFst<StdArc, int32>,
  ...
  py::enum_<StatePropertiesEnum>(m, "StatePropertiesEnum")
      ...
      .value("kStateMultipleArcsOut",
             StatePropertiesEnum::kStateMultipleArcsOut)
      .value("kStateOlabelsOut", StatePropertiesEnum::kStateOlabelsOut)
      .value("kStateIlabelsOut", StatePropertiesEnum::kStateIlabelsOut)
      .export_values();

  m.def("GetStateProperties", &GetStateProperties<StdArc>,
        ...
        py::arg("fst"), py::arg("max_state"), py::arg("props"));
  {
    using PyClass = DfsOrderVisitor<StdArc>;

    auto dfs_order_vistor = py::class_<PyClass>(m, "DfsOrderVisitor");
...
void pybind_fstext_fstext_utils(py::module &m) {
  m.def("HighestNumberedOutputSymbol", &HighestNumberedOutputSymbol<StdArc>,
        "Returns the highest numbered output symbol id of the FST (or zero "
        "for an empty FST.",
        py::arg("fst"));

  m.def("HighestNumberedInputSymbol", &HighestNumberedInputSymbol<StdArc>,
        "Returns the highest numbered input symbol id of the FST (or zero "
        "for an empty FST.",
        py::arg("fst"));

  m.def("NumArcs", &NumArcs<StdArc>,
        "Returns the total number of arcs in an FST.", py::arg("fst"));

  m.def("GetInputSymbols", &GetInputSymbols<StdArc, int32>,
        "GetInputSymbols gets the list of symbols on the input of fst "
        "(including epsilon, if include_eps == true), as a sorted, unique "
        "list.",
        py::arg("fst"), py::arg("include_eps"), py::arg("symbols"));

  m.def("GetOutputSymbols", &GetOutputSymbols<StdArc, int32>,
        "GetOutputSymbols gets the list of symbols on the output of fst "
        "(including epsilon, if include_eps == true)",
        py::arg("fst"), py::arg("include_eps"), py::arg("symbols"));

  m.def("ClearSymbols", &ClearSymbols<StdArc>,
        ...
        py::arg("clear_input"), py::arg("clear_output"), py::arg("fst"));

  m.def("GetSymbols", &GetSymbols<int32>, py::arg("symtab"),
        py::arg("include_eps"), py::arg("syms_out"));

  m.def("DeterminizeStarInLog", &DeterminizeStarInLog, py::arg("fst"),
        py::arg("delta") = kDelta, py::arg("debug_ptr") = NULL,
        py::arg("max_states") = -1, py::call_guard<py::gil_scoped_release>());

  m.def("PushInLogInitial", &PushInLog<REWEIGHT_TO_INITIAL>, py::arg("fst"),
        py::arg("ptype"), py::arg("delta") = kDelta,
        py::call_guard<py::gil_scoped_release>());

  m.def("PushInLogFinal", &PushInLog<REWEIGHT_TO_FINAL>, py::arg("fst"),
        py::arg("ptype"), py::arg("delta") = kDelta,
        py::call_guard<py::gil_scoped_release>());

  m.def(
      "MinimizeEncoded", &MinimizeEncoded<StdArc>,
      "Minimizes after encoding; applicable to all FSTs.  It is like what you "
      "get "
      "from the Minimize() function, except it will not push the weights, or "
      "the "
      "symbols.  This is better for our recipes, as we avoid ever pushing the "
      "weights.  However, it will only minimize optimally if your graphs are "
      "such "
      ...
      py::arg("fst"), py::arg("delta") = kDelta,
      py::call_guard<py::gil_scoped_release>());
  ...
  m.def(
      ...
      "sequences as \"isymbols_out\" and \"osymbols_out\" (removing epsilons), "
      "and "
      "the total weight as \"tot_weight\". The total weight will be "
      "Weight::Zero() "
      "if the FST is empty.  If any of the output pointers are NULL, it does "
      "not "
      "create that output.",
      py::arg("fst"));

  m.def(
      "ConvertNbestToVector", &ConvertNbestToVector<StdArc>,
      ...
      "them into a std::vector of separate FSTs.  This special structure is "
      "that "
      ...
      py::arg("fst"), py::arg("fsts_out"));

  m.def("NbestAsFsts", &NbestAsFsts<StdArc>,
        ...
        py::arg("fst"), py::arg("n"), py::arg("fsts_out"));

  m.def("MakeLinearAcceptor", &MakeLinearAcceptor<StdArc, int32>,
        "Creates unweighted linear acceptor from symbol sequence.",
        py::arg("labels"), py::arg("ofst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("MakeLinearAcceptorWithAlternatives",
        &MakeLinearAcceptorWithAlternatives<StdArc, int32>,
        "Creates an unweighted acceptor with a linear structure, with "
        "alternatives "
        "at each position.  Epsilon is treated like a normal symbol here. "
        "Each position in \"labels\" must have at least one alternative.",
        py::arg("labels"), py::arg("ofst"));

  m.def("SafeDeterminizeWrapper", &SafeDeterminizeWrapper<StdArc>,
        "Does PreDeterminize and DeterminizeStar and then removes the "
        "disambiguation symbols. "
        "This is a form of determinization that will never blow up. "
        "Note that ifst is non-const and can be considered to be destroyed by "
        "this "
        "operation. "
        "Does not do epsilon removal (RemoveEpsLocal)-- this is so it's safe "
        "to cast to "
  ...
  m.def("SafeDeterminizeMinimizeWrapper",
        ...
        "SafeDeterminizeMinimizeWapper is as SafeDeterminizeWrapper except "
        "that it also "
  ...
  m.def("SafeDeterminizeMinimizeWrapperInLog",
        ...
        "SafeDeterminizeMinimizeWapperInLog is as "
        "SafeDeterminizeMinimizeWrapper except "
        "it first casts tothe log semiring.",
        py::arg("ifst"), py::arg("ofst"), py::arg("delta") = kDelta,
        py::call_guard<py::gil_scoped_release>());

  m.def("RemoveSomeInputSymbols", &RemoveSomeInputSymbols<StdArc, int32>,
        "RemoveSomeInputSymbols removes any symbol that appears in "
        "\"to_remove\", from "
        "the input side of the FST, replacing them with epsilon.",
        py::arg("to_remove"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());
  ...
  m.def("RemoveWeights", &RemoveWeights<StdArc>, py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("PrecedingInputSymbolsAreSame", &PrecedingInputSymbolsAreSame<StdArc>,
        ...
        py::arg("start_is_epsilon"), py::arg("fst"));

  m.def("PrecedingInputSymbolsAreSameClass",
        &PrecedingInputSymbolsAreSameClass<
            StdArc, IdentityFunction<typename StdArc::Label>>,
        "This is as PrecedingInputSymbolsAreSame, but with a functor f that "
        "maps labels to classes. "
        "The function tests whether the symbols preceding any given state are "
        "in the same "
        "class. "
        ...
        "Arc::Label. "
        "this must apply to valid labels and also to kNoLabel (so we can have "
        "a marker for "
        "the invalid labels.",
        py::arg("start_is_epsilon"), py::arg("fst"), py::arg("f"),
        py::call_guard<py::gil_scoped_release>());

  m.def("FollowingInputSymbolsAreSame", &FollowingInputSymbolsAreSame<StdArc>,
        "Returns true if and only if the FST is such that the input symbols "
        "on arcs exiting any given state all have the same value. "
        "If end_is_epsilon, treat end-state as an epsilon output arc [i.e. "
        "ensure "
        "end-states cannot have non-epsilon output transitions.]",
        py::arg("end_is_epsilon"), py::arg("fst"));

  m.def("FollowingInputSymbolsAreSameClass",
        &FollowingInputSymbolsAreSameClass<
            StdArc, IdentityFunction<typename StdArc::Label>>,
        py::arg("end_is_epsilon"), py::arg("fst"), py::arg("f"));

  m.def(
      "MakePrecedingInputSymbolsSame", &MakePrecedingInputSymbolsSame<StdArc>,
      "MakePrecedingInputSymbolsSame ensures that all arcs entering any given "
      "fst "
      ...
      "the preceding arcs with non-epsilon input symbol, a new dummy state "
      "that "
      "has an epsilon link to the fst state. "
      "If \"start_is_epsilon\", ensure that start-state can have only "
      "epsilon-links "
      "into it.",
      py::arg("start_is_epsilon"), py::arg("fst"));
  ...
  m.def("MakePrecedingInputSymbolsSameClass",
        &MakePrecedingInputSymbolsSameClass<
            StdArc, IdentityFunction<typename StdArc::Label>>,
        "As MakePrecedingInputSymbolsSame, but takes a functor object that "
        "maps labels to classes.",
        py::arg("start_is_epsilon"), py::arg("fst"), py::arg("f"));

  m.def("MakeFollowingInputSymbolsSame", &MakeFollowingInputSymbolsSame<StdArc>,
        "MakeFollowingInputSymbolsSame ensures that all arcs exiting any given "
        "fst "
        "state have the same input symbol.  It does this by detecting states "
        "that have "
        "differing input symbols on arcs that exit it, and inserting, for each "
        "of the "
        "following arcs with non-epsilon input symbol, a new dummy state that "
        "has an "
        "input-epsilon link from the fst state.  The output symbol and weight "
        "stay on the "
        "link to the dummy state (in order to keep the FST "
        "output-deterministic and "
        "stochastic, if it already was). "
        "If end_is_epsilon, treat \"being a final-state\" like having an "
        "epsilon output "
        "link.",
        py::arg("end_is_epsilon"), py::arg("fst"));
  ...
  m.def("MakeFollowingInputSymbolsSameClass",
        &MakeFollowingInputSymbolsSameClass<
            StdArc, IdentityFunction<typename StdArc::Label>>,
        "As MakeFollowingInputSymbolsSame, but takes a functor object that "
        "maps labels to classes.",
        py::arg("end_is_epsilon"), py::arg("fst"), py::arg("f"));

  m.def(
      "MakeLoopFst", &MakeLoopFst<StdArc>,
      ...
      "final-state becomes a transition to the loop-state.  Each fst in "
      "\"fsts\" "
      "should be an acceptor.  The fst MakeLoopFst returns is "
      "output-deterministic, "
      "but not output-epsilon free necessarily, and arcs are sorted on output "
      "label. "
      "Note: if some of the pointers in the input vector \"fsts\" have the "
      "same "
      ...
      py::arg("fsts"), py::call_guard<py::gil_scoped_release>());

  m.def("ApplyProbabilityScale", &ApplyProbabilityScale<StdArc>,
        "ApplyProbabilityScale is applicable to FSTs in the log or tropical "
        "semiring. "
        "It multiplies the arc and final weights by \"scale\" [this is not the "
        "Mul "
        "operation of the semiring, it's actual multiplication, which is "
        "equivalent "
        "to taking a power in the semiring].",
        py::arg("scale"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("EqualAlign", &EqualAlign<StdArc>,
        "EqualAlign is similar to RandGen, but it generates a sequence with "
        "exactly \"length\" "
        "input symbols.  It returns true on success, false on failure (failure "
        "is partly "
        "random but should never happen in practice for normal speech models.) "
        "It generates a random path through the input FST, finds out which "
        "subset of the "
        "states it visits along the way have self-loops with inupt symbols on "
        "them, and "
        "outputs a path with exactly enough self-loops to have the requested "
        "number "
        "of input symbols. "
        "Note that EqualAlign does not use the probabilities on the FST.  It "
        "just uses "
        "equal probabilities in the first stage of selection (since the output "
        "will anyway "
        "not be a truly random sample from the FST). "
        "The input fst \"ifst\" must be connected or this may enter an "
        "infinite loop.",
        py::arg("ifst"), py::arg("length"), py::arg("rand_seed"),
        py::arg("ofst"), py::arg("fsnum_retriest") = 10);

  m.def("RemoveUselessArcs", &RemoveUselessArcs<StdArc>,
        ...
        "those arcs [for these purposes, epsilon is not treated as a real "
        "symbol]. "
  ...
  m.def("PhiCompose", &PhiCompose<StdArc>,
        ...
        py::arg("fst1"), py::arg("fst2"), py::arg("phi_label"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("PropagateFinal", &PropagateFinal<StdArc>,
        ...
        py::arg("phi_label"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("RhoCompose", &RhoCompose<StdArc>,
        ...
        "the first FST.",
        py::arg("fst1"), py::arg("fst2"), py::arg("rho_label"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def(
      "IsStochasticFst", &IsStochasticFst<StdArc>,
      "This function returns true if, in the semiring of the FST, the sum "
      "(within "
      "the semiring) of all the arcs out of each state in the FST is one, to "
      "within "
      "delta.  After MakeStochasticFst, this should be true (for a connected "
      "FST). "
      "\n"
      "@param fst [in] the FST that we are testing. "
      "@param delta [in] the tolerance to within which we test equality to 1. "
      "@param min_sum [out] if non, NULL, contents will be set to the minimum "
      "sum of weights. "
      "@param max_sum [out] if non, NULL, contents will be set to the maximum "
      "sum of weights. "
      "@return Returns true if the FST is stochastic, and false otherwise.",
      py::arg("fst"), py::arg("delta") = kDelta, py::arg("min_sum") = NULL,
      py::arg("max_sum") = NULL);

  m.def("IsStochasticFstInLog", &IsStochasticFstInLog,
...
void pybind_fstext_grammar_context_fst(py::module &m) {
  ...
  m.def(
      "GetEncodingMultiple", &GetEncodingMultiple,
  ...
  m.def(
      "ComposeContextLeftBiphone", &ComposeContextLeftBiphone,
      ...
      "assumed to be 2 and 1 respectively (meaning, left-biphone phonetic "
      "context). "

      "This function creates a context FST and composes it on the left with "
      "\"ifst\" "
      "to make \"ofst\". "

      "@param [in] nonterm_phones_offset  The integer id of the symbol "
      "                  #nonterm_bos in the phones.txt file.  You can just "
      "set this "
      "                  to a large value (like 1 million) if you are not "
      "actually using "
      ...
      "information "
      "                  about the meaning of that ilabel; see \\ref "
      "tree_ilabel "
      "                  "
      "(http://kaldi-asr.org/doc/tree_externals.html#tree_ilabel) "
      ...
      py::arg("nonterm_phones_offset"), py::arg("disambig_syms"),
      py::arg("ifst"), py::arg("ofst"), py::arg("ilabels"));
  {
    using PyClass = InverseLeftBiphoneContextFst;

    auto inverse_left_biphone_context_fst =
        py::class_<PyClass, DeterministicOnDemandFst<StdArc>>(
            m, "InverseLeftBiphoneContextFst");
    inverse_left_biphone_context_fst
        .def(py::init<PyClass::Label, const std::vector<int32> &,
                      const std::vector<int32> &>(),
             py::arg("nonterm_phones_offset"), py::arg("phones"),
        ...
        .def("GetArc", &PyClass::GetArc, py::arg("s"), py::arg("ilabel"),
             py::arg("arc"))
        .def("IlabelInfo", &PyClass::IlabelInfo,
             "Returns a reference to a vector<vector<int32> > with information "
             "about all "
             "the input symbols of C (i.e. all the output symbols of this "
             "InverseContextFst).  See "
             "\"http://kaldi-asr.org/doc/tree_externals.html#tree_ilabel\".")
        .def("SwapIlabelInfo", &PyClass::SwapIlabelInfo,
             "A way to destructively obtain the ilabel-info.  Only do this if "
             "you "
...
void pybind_fstext_lattice_utils(py::module &m) {
  m.def(
      "ConvertLattice",
      py::overload_cast<
          const ExpandedFst<ArcTpl<TropicalWeight>> &,
          MutableFst<ArcTpl<CompactLatticeWeightTpl<TropicalWeight, int32>>> *,
      ...
      py::arg("ifst"), py::arg("ofst"), py::arg("invert") = true);

  m.def(
      "ConvertLattice",
      py::overload_cast<const ExpandedFst<ArcTpl<LatticeWeightTpl<float>>> &,
                        MutableFst<ArcTpl<CompactLatticeWeightTpl<
                            LatticeWeightTpl<float>, int32>>> *,
  ...
  m.def(
      "ConvertLattice",
      py::overload_cast<const ExpandedFst<ArcTpl<
                            CompactLatticeWeightTpl<TropicalWeight, int32>>> &,
                        MutableFst<ArcTpl<TropicalWeight>> *, bool>(
          &ConvertLattice<TropicalWeight, int32>),
      "Convert lattice CompactLattice  format to Lattice.  This is a bit "
      "like converting from the Gallic semiring.  As for any CompactLattice, "
      "\"ifst\" "
      "must be an acceptor (i.e., ilabels and olabels should be identical).  "
      "If "
      "invert=false, the labels on \"ifst\" become the ilabels on \"ofst\" and "
      "the "
      "strings in the weights of \"ifst\" becomes the olabels.  If invert=true "
      "[default], this is reversed (useful for speech recognition lattices; "
      "our "
      "standard non-compact format has the words on the output side to match "
      "HCLG).",
      py::arg("ifst"), py::arg("ofst"), py::arg("invert") = true);

  m.def(
      "ConvertLattice",
      py::overload_cast<const ExpandedFst<ArcTpl<CompactLatticeWeightTpl<
                            LatticeWeightTpl<float>, int32>>> &,
                        MutableFst<ArcTpl<LatticeWeightTpl<float>>> *, bool>(
          &ConvertLattice<LatticeWeightTpl<float>, int32>),
      "Convert lattice CompactLattice  format to Lattice.  This is a bit "
      "like converting from the Gallic semiring.  As for any CompactLattice, "
      "\"ifst\" "
      "must be an acceptor (i.e., ilabels and olabels should be identical).  "
  ...
  m.def("ConvertLattice",
        py::overload_cast<const ExpandedFst<ArcTpl<LatticeWeightTpl<float>>> &,
                          MutableFst<ArcTpl<TropicalWeight>> *>(
            &ConvertLattice<LatticeWeightTpl<float>, TropicalWeight>),
        "Convert between CompactLattices and Lattices of different floating "
        "point types... "
        "this works between any pair of weight types for which "
...
MutableFst<ArcTpl<LatticeWeightTpl<float>> > *>(&ConvertLattice<TropicalWeight,
LatticeWeightTpl<float>>), "Convert between CompactLattices and Lattices of
different floating point types... " "this works between any pair of weight types
for which ConvertLatticeWeight " "is defined (c.f. lattice-weight.h), and also
includes conversion from " "LatticeWeight to TropicalWeight.", py::arg("ifst"),
    py::arg("ofst"));
m.def("ConvertLattice",
    py::overload_cast<const ExpandedFst<ArcTpl<LatticeWeightTpl<float> > > &,
                MutableFst<ArcTpl<CompactLatticeWeightTpl<LatticeWeightTpl<double>,
int32> > > *>(&ConvertLattice<int32>), "Lattice with float to CompactLattice
with double.", py::arg("ifst"), py::arg("ofst"));
  ...
  */
  m.def("ConvertLattice",
        py::overload_cast<const ExpandedFst<ArcTpl<LatticeWeightTpl<double>>> &,
                          MutableFst<ArcTpl<CompactLatticeWeightTpl<
                              LatticeWeightTpl<float>, int32>>> *>(
            &ConvertLattice<int32>),
        "Lattice with double to CompactLattice with float.", py::arg("ifst"),
        py::arg("ofst"));

  m.def("ConvertLattice",
        py::overload_cast<const ExpandedFst<ArcTpl<CompactLatticeWeightTpl<
                              LatticeWeightTpl<double>, int32>>> &,
  ...
  m.def("ConvertLattice",
        py::overload_cast<const ExpandedFst<ArcTpl<CompactLatticeWeightTpl<
                              LatticeWeightTpl<float>, int32>>> &,
                          MutableFst<ArcTpl<LatticeWeightTpl<double>>> *>(
            &ConvertLattice<int32>),
  ...
  m.def("ConvertFstToLattice",
        py::overload_cast<const ExpandedFst<ArcTpl<TropicalWeight>> &,
                          MutableFst<ArcTpl<LatticeWeightTpl<float>>> *>(
            &ConvertFstToLattice<float>),
        "Converts TropicalWeight to LatticeWeight (puts all the weight on "
        "the first float in the lattice's pair).",
        py::arg("ifst"), py::arg("ofst"));

  m.def("DefaultLatticeScale", &DefaultLatticeScale,
        "Returns a default 2x2 matrix scaling factor for LatticeWeight");

  m.def("AcousticLatticeScale", &AcousticLatticeScale, py::arg("acwt"));

  m.def("GraphLatticeScale", &GraphLatticeScale, py::arg("lmwt"));

  m.def("LatticeScale", &LatticeScale, py::arg("lmwt"), py::arg("acwt"));

  /*
m.def("ScaleLattice",
    py::overload_cast<const std::vector<std::vector<float> > &,
MutableFst<ArcTpl<LatticeWeightTpl<float>> >
*>(&ScaleLattice<LatticeWeightTpl<float>, float>), "Scales the pairs of weights
in LatticeWeight or CompactLatticeWeight by " "viewing the pair (a, b) as a
2-vector and pre-multiplying by the 2x2 matrix " "in \"scale\".  E.g. typically
scale would equal "
        "[ 1   0; "
        "      0  acwt ] "
...
*/
        ...
        py::arg("fst"));
  {
    ...
    auto std_to_lattice_mapper = py::class_<PyClass>(m, "StdToLatticeMapper");
    std_to_lattice_mapper.def(py::init<>())
        ...
        .def("Properties", &PyClass::Properties, py::arg("props"));
  }
  {
    using PyClass = LatticeToStdMapper<float>;

    auto lattice_to_std_mapper = py::class_<PyClass>(m, "LatticeToStdMapper");
...
void pybind_fstext_pre_determinize(py::module &m) {
  m.def(
      "PreDeterminize", &PreDeterminize<StdArc, int32>,
      "PreDeterminize inserts extra symbols on the input side of an FST as "
      "necessary to "
      "ensure that, after epsilon removal, it will be compactly determinizable "
      "by the "
      "determinize* algorithm.  By compactly determinizable we mean that "
      "no original FST state is represented in more than one determinized "
      "state). "

      "Caution: this code is now only used in testing. "

      "The new symbols start from the value \"first_new_symbol\", which should "
      "be "
      "higher than the largest-numbered symbol currently in the FST.  The new "
      "symbols added are put in the array syms_out, which should be empty at "
      "start.",
      py::arg("fst"), py::arg("first_new_symbol"), py::arg("syms_out"));

  m.def("CreateNewSymbols", &CreateNewSymbols<int32>,
        "CreateNewSymbols is a helper function used inside PreDeterminize, and "
        "is also useful "
        "when you need to add a number of extra symbols to a different "
        "vocabulary from the one "
        "modified by PreDeterminize.",
        py::arg("inputSymTable"), py::arg("nSym"), py::arg("prefix"),
        py::arg("syms_out"));

  m.def("AddSelfLoops", &AddSelfLoops<StdArc>,
        "AddSelfLoops is a function you will probably want to use alongside "
        "PreDeterminize, "
        "to add self-loops to any FSTs that you compose on the left hand side "
        "of the one "
        "modified by PreDeterminize. "

        "This function inserts loops with \"special symbols\" [e.g. #0, #1] "
        "into an FST. "
        "This is done at each final state and each state with non-epsilon "
        "output symbols on "
        "at least one arc out of it.  This is to ensure that these symbols, "
        "when inserted into "
        "the input side of an FST we will compose with on the right, can "
        "\"pass through\" this "
        "FST. "

        "At input, isyms and osyms must be vectors of the same size n, "
        "corresponding "
        "to symbols that currently do not exist in 'fst'.  For each state in n "
        "that has "
        "non-epsilon symbols on the output side of arcs leaving it, or which "
        "is a final state, "
        "this function inserts n self-loops with unit weight and one of the n "
        "pairs "
        "of symbols on its input and output.",
        py::arg("fst"), py::arg("isyms"), py::arg("osyms"));

  m.def("DeleteISymbols", &DeleteISymbols<StdArc>,
        ...
        py::arg("fst"), py::arg("symsIn"));

  m.def("CreateSuperFinal", &CreateSuperFinal<StdArc>,
        "CreateSuperFinal takes an FST, and creates an equivalent FST with a "
        "single final "
        "state with no transitions out and unit final weight, by inserting "
        "epsilon transitions "
        "as necessary.",
        py::arg("fst"));
}
void pybind_fstext_prune_special(py::module &m) {
  m.def("PruneSpecial", &PruneSpecial<StdArc>,
        "The function PruneSpecial is like the standard OpenFst function "
        "\"prune\", "
        "except it does not expand the entire \"ifst\"- this is useful for "
        "cases where "
        "ifst is an on-demand FST such as a ComposeFst and we don't want to "
        "visit "
        "it all.  It supports pruning either to a specified beam (if beam is "
        "not One()), or to a specified max_states (if max_states is > 0).  One "
        "of the "
        "two must be specified. "

        "Requirements: "
        "  - Costs must be non-negative (equivalently, weights must not be "
        "greater than One()). "
        "  - There must be a Compare(a, b) function that compares two weights "
        "and returns (-1,0,1) "
        "    if (a<b, a=b, a>b).  We define this in Kaldi, for TropicalWeight, "
        "LogWeight (I think), "
        "    and LatticeWeight... also CompactLatticeWeight, but we doubt that "
        "will be used here; "
        "    better to use PruneCompactLattice().",
        py::arg("ifst"), py::arg("ofst"), py::arg("beam"),
        py::arg("max_states") = 0);

  m.def("PushSpecial", &PushSpecial,
...
void pybind_fstext_remove_eps_local(py::module &m) {
  m.def("RemoveEpsLocal", &RemoveEpsLocal<StdArc>,
        "RemoveEpsLocal remove some (but not necessarily all) epsilons in an "
        "FST, "
        "using an algorithm that is guaranteed to never increase the number of "
        "arcs "
        "in the FST (and will also never increase the number of states).  The "
        "algorithm is not optimal but is reasonably clever.  It does not just "
        "remove "
        "epsilon arcs;it also combines pairs of input-epsilon and "
        "output-epsilon arcs "
        "into one. "
        "The algorithm preserves equivalence and stochasticity in the given "
        "semiring. "
        "If you want to preserve stochasticity in a different semiring (e.g. "
        "log), "
        "then use RemoveEpsLocalSpecial, which only works for StdArc but which "
        "preserves stochasticity, where possible (*) in the LogArc sense.  The "
        "reason that we can't "
        "just cast to a different semiring is that in that case we would no "
        "longer "
        "be able to guarantee equivalence in the original semiring (this "
        "arises from "
        "what happens when we combine identical arcs). "
        "(*) by \"where possible\".. there are situations where we wouldn't be "
        "able to "
        "preserve stochasticity in the LogArc sense while maintaining "
        "equivalence in "
        "the StdArc sense, so in these situations we maintain equivalence.",
        py::arg("fst"));

  m.def("RemoveEpsLocalSpecial", &RemoveEpsLocalSpecial,
...
void pybind_fst_symbol_table(py::module &m) {
  ...
  py::class_<PyClass>(m, "SymbolTable").def(py::init<>());
}
...
void init_fstext(py::module &_m) {
  ...
  py::class_<CacheOptions>(m, "CacheOptions")
      .def(py::init<>())
      .def(py::init<bool, size_t>(), py::arg("gc"), py::arg("gc_limit"))
  ...
  py::class_<ArcMapFstOptions, CacheOptions>(m, "ArcMapFstOptions")
      .def(py::init<>())
      .def(py::init<const CacheOptions &>(), py::arg("opts"));

  py::class_<
      fst::ArcMapFst<StdArc, LatticeArc, fst::StdToLatticeMapper<BaseFloat>>>(
      m, "LmMappedFst");

  auto table_compose_options =
      py::class_<TableComposeOptions, TableMatcherOptions>(
  ...
  table_compose_options.def(py::init<>())
      ...
      .def(py::pickle(
          [](const TableComposeOptions &p) {  // __getstate__
            ...
            return py::make_tuple(p.connect, p.filter_type, p.table_match_type);
          },
  ...
  pybind_sequential_table_reader<fst::VectorFstHolder>(
      m, "SequentialVectorFstReader");
  pybind_random_access_table_reader<fst::VectorFstHolder>(
      m, "RandomAccessVectorFstReader");

  m.def(
      "fst_add_self_loops",
      [](VectorFst<StdArc> *fst, const TransitionModel &trans_model,
         std::vector<int32> disambig_syms_in, BaseFloat self_loop_scale = 1.0,
         bool reorder = true) {
        py::gil_scoped_release gil_release;

        bool check_no_self_loops = true;
        AddSelfLoops(trans_model, disambig_syms_in, self_loop_scale, reorder,
                     check_no_self_loops, fst);
      },
      py::arg("fst"), py::arg("trans_model"), py::arg("disambig_syms_in"),
      py::arg("self_loop_scale") = 1.0, py::arg("reorder") = true);

  m.def(
      "fst_add_self_loops",
      [](py::object fst, const TransitionModel &trans_model,
         std::vector<int32> disambig_syms_in, BaseFloat self_loop_scale = 1.0,
         bool reorder = true) {
        py::gil_scoped_release gil_release;
        ...
        bool check_no_self_loops = true;
        AddSelfLoops(trans_model, disambig_syms_in, self_loop_scale, reorder,
                     check_no_self_loops, &vf);
      },
      py::arg("fst"), py::arg("trans_model"), py::arg("disambig_syms_in"),
      py::arg("self_loop_scale") = 1.0, py::arg("reorder") = true);

  m.def(
      "fst_add_subsequential_loop",
      [](VectorFst<StdArc> *fst, int32 subseq_sym, float delta = kDelta) {
        py::gil_scoped_release gil_release;
        int32 h = HighestNumberedInputSymbol(*fst);
        if (subseq_sym <= h) {
          std::cerr << "fst_add_subsequential_loop: subseq symbol does not "
                       "seem right, "
                    << subseq_sym << " <= " << h << '\n';
        }
        AddSubsequentialLoop(subseq_sym, fst);
      },
      py::arg("fst"), py::arg("subseq_sym"), py::arg("delta") = kDelta);

  m.def(
      "fst_add_subsequential_loop",
      [](py::object fst, int32 subseq_sym, float delta = kDelta) {
        py::gil_scoped_release gil_release;
        auto pywrapfst_mod = py::module_::import("pywrapfst");
        auto ptr = reinterpret_cast<VectorFstObject *>(fst.ptr());
        auto mf = ptr->__pyx_base._mfst->GetMutableFst<StdArc>();
  ...
  m.def(
      "fst_compose_context",
      [](VectorFst<StdArc> *fst, std::vector<int32> disambig_in,
        ...
        if (nonterm_phones_offset < 0) {
          // The normal case.
          ComposeContext(disambig_in, context_width, central_position, fst,
                         &composed_fst, &ilabels);
        } else {
          // The grammar-FST case. See ../doc/grammar.dox for an intro.
          if (context_width != 2 || central_position != 1) {
            KALDI_ERR
                << "Grammar-fst graph creation only supports models with left-"
                   "biphone context.  (--nonterm-phones-offset option was "
                   "supplied).";
          }
          ComposeContextLeftBiphone(nonterm_phones_offset, disambig_in, *fst,
                                    &composed_fst, &ilabels);
        }
      ...
      py::arg("fst"), py::arg("disambig_in"), py::arg("context_width") = 3,
      py::arg("central_position") = 1, py::arg("nonterm_phones_offset") = -1);

  m.def(
      "fst_determinize_log",
      [](VectorFst<StdArc> *fst) { DeterminizeInLog(fst); }, py::arg("fst"));

  m.def(
      "fst_determinize_star",
      [](VectorFst<StdArc> *fst, float delta = kDelta, int max_states = -1,
         bool use_log = false) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("fst"), py::arg("delta") = kDelta, py::arg("max_states") = -1,
      py::arg("use_log") = false);

  m.def(
      "fst_determinize_star",
      [](py::object fst, float delta = kDelta, int max_states = -1,
         bool use_log = false) {
        auto pywrapfst_mod = py::module_::import("pywrapfst");
        auto ptr = reinterpret_cast<VectorFstObject *>(fst.ptr());
        auto mf = ptr->__pyx_base._mfst->GetMutableFst<StdArc>();
  ...
  m.def(
      "fst_is_stochastic",
      [](Fst<StdArc> *fst, float delta = 0.01, bool test_in_log = true) {
        bool ans;
      ...
      py::arg("fst"), py::arg("delta") = 0.01, py::arg("test_in_log") = true);

  m.def(
      "fst_is_stochastic",
      [](py::object fst, float delta = 0.01, bool test_in_log = true) {
        auto pywrapfst_mod = py::module_::import("pywrapfst");
        auto ptr = reinterpret_cast<VectorFstObject *>(fst.ptr());
        auto mf = ptr->__pyx_base._mfst->GetMutableFst<StdArc>();
        VectorFst<StdArc> vf(*mf);
        ...
        StdArc::Weight min, max;
        if (test_in_log)
          ans = IsStochasticFstInLog(vf, delta, &min, &max);
        else
          ans = IsStochasticFst(vf, delta, &min, &max);
        return py::make_tuple(ans, min, max);
      },
      py::arg("fst"), py::arg("delta") = 0.01, py::arg("test_in_log") = true);

  m.def(
      "fst_make_context_syms",
      [](fst::SymbolTable *phones_symtab,
        ...
        fst::SymbolTable *clg_symtab = CreateILabelInfoSymbolTable(
            ilabel_info, *phones_symtab, phone_separator, initial_disambig);
        return clg_symtab;
      },
      py::arg("phones_symtab"), py::arg("ilabel_info"),
      py::arg("phone_separator") = "/", py::arg("initial_disambig") = "#-1");

  m.def(
      "fst_minimize_encoded",
      [](VectorFst<StdArc> *fst, float delta = kDelta) {
        py::gil_scoped_release gil_release;
  ...
  m.def(
      "fst_minimize_encoded",
      [](py::object fst, float delta = kDelta) {
        auto pywrapfst_mod = py::module_::import("pywrapfst");
        auto ptr = reinterpret_cast<VectorFstObject *>(fst.ptr());
        auto mf = ptr->__pyx_base._mfst->GetMutableFst<StdArc>();
        VectorFst<StdArc> vf(*mf);
  ...
  m.def(
      "fst_phi_compose",
      [](VectorFst<StdArc> *fst1, VectorFst<StdArc> *fst2, int32 phi_label) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("fst1"), py::arg("fst2"), py::arg("phi_label"));

  m.def(
      "fst_push_special",
      [](VectorFst<StdArc> *fst, BaseFloat delta = kDelta) {
        py::gil_scoped_release gil_release;
  ...
  m.def(
      "fst_push_special",
      [](py::object fst, BaseFloat delta = kDelta) {
        auto pywrapfst_mod = py::module_::import("pywrapfst");
        auto ptr = reinterpret_cast<VectorFstObject *>(fst.ptr());
        auto mf = ptr->__pyx_base._mfst->GetMutableFst<StdArc>();
        VectorFst<StdArc> vf(*mf);
  ...
  m.def(
      "fst_arc_sort",
      [](VectorFst<StdArc> *fst, std::string sort_type = "ilabel") {
        py::gil_scoped_release gil_release;
      ...
      py::arg("fst"), py::arg("sort_type") = "ilabel");
  ...
  m.def(
      "fst_rm_eps_local",
      [](VectorFst<StdArc> *fst, bool use_log = false,
      ...
      py::arg("fst"), py::arg("use_log") = false,
      py::arg("stochastic_in_log") = true);

  m.def(
      "fst_rm_symbols",
      [](VectorFst<StdArc> *fst, std::vector<int32> disambig_in,
         bool apply_to_output = false, bool remove_arcs = false,
      ...
      py::arg("fst"), py::arg("disambig_in"),
      py::arg("apply_to_output") = false, py::arg("remove_arcs") = false,
