...
void pybind_basis_fmllr_diag_gmm(py::module &m) {
  {
    ...
    auto basis_fmllr_options = py::class_<PyClass>(m, "BasisFmllrOptions");
  ...
  }
  {
    ...
    auto basis_fmllr_accus = py::class_<PyClass>(m, "BasisFmllrAccus");
    ...
    basis_fmllr_accus.def(py::init<>())
        ...
        .def("AccuGradientScatter", &PyClass::AccuGradientScatter,
  ...
  }
  {
    ...
    auto basis_fmllr_estimate = py::class_<PyClass>(m, "BasisFmllrEstimate");
    ...
    basis_fmllr_estimate.def(py::init<>())
        ...
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("EstimateFmllrBasis", &PyClass::EstimateFmllrBasis,
             "Estimate the base matrices efficiently in a Maximum Likelihood "
             "manner. "
             "It takes diagonal GMM as argument, which will be used for "
             "preconditioner "
             "computation. The total number of bases is fixed to "
             "N = (dim + 1) * dim "
             "Note that SVD is performed in the normalized space. The base "
             "matrices "
             "are finally converted back to the unnormalized space.",
             py::arg("am_gmm"), py::arg("basis_accus"))
        .def("ComputeAmDiagPrecond", &PyClass::ComputeAmDiagPrecond,
             "This function computes the preconditioner matrix, prior to base "
             "matrices "
             "estimation. Since the expected values of G statistics are used, "
             "it "
             ...
             py::arg("am_gmm"), py::arg("pre_cond"))
        .def("Dim", &PyClass::Dim)
        .def("BasisSize", &PyClass::BasisSize)
        .def("ComputeTransform", &PyClass::ComputeTransform,
             "This function performs speaker adaptation, computing the fMLLR "
             "matrix "
             "based on speaker statistics. It takes fMLLR stats as argument. "
             "The basis weights (d_{1}, d_{2}, ..., d_{N}) are also optimized "
             "explicitly. Finally, it returns objective function improvement "
             "over "
             "all the iterations, compared with the value at the initial value "
             "of "
             "\"out_xform\" (or the unit transform if not provided). "
             "The coefficients are output to \"coefficients\" only if the "
             "vector is "
             "provided. "
             "See section 5.3 of the paper for more details.",
             py::arg("spk_stats"), py::arg("out_xform"),
             py::arg("coefficients"), py::arg("options"));
    ;
  }
}

void pybind_cmvn(py::module &m) {
  m.def("InitCmvnStats", &InitCmvnStats,
  ...
  m.def("AccCmvnStats",
        py::overload_cast<const VectorBase<BaseFloat> &, BaseFloat,
                          MatrixBase<double> *>(&AccCmvnStats),
        "Accumulation from a single frame (weighted).", py::arg("feat"),
        py::arg("weight"), py::arg("stats"));
  m.def("AccCmvnStats",
        py::overload_cast<const MatrixBase<BaseFloat> &,
                          const VectorBase<BaseFloat> *, MatrixBase<double> *>(
            &AccCmvnStats),
        "Accumulation from a feature file (possibly weighted-- useful in "
        "excluding silence).",
  ...
  m.def(
      ...
      "Calculate CMVN from a speaker's utterances", py::arg("uttlist"),
      py::arg("feat_reader"));

  m.def(
      "apply_cmvn",
      [](const Matrix<BaseFloat> &feats, const Matrix<double> &cmvn_stats,
         bool reverse = false, bool norm_vars = false) {
      ...
      py::arg("feats"), py::arg("cmvn_stats"), py::arg("reverse") = false,
      py::arg("norm_vars") = false);

  m.def(
      "ApplyCmvn", &ApplyCmvn,
      "Apply cepstral mean and variance normalization to a matrix of features. "
      "If norm_vars == true, expects stats to be of dimension 2 by (dim+1), "
      "but "
      "if norm_vars == false, will accept stats of dimension 1 by (dim+1); "
      "these "
  ...
  m.def(
      "apply_transform",
      [](const Matrix<BaseFloat> &feat, const Matrix<BaseFloat> &trans) {
        py::gil_scoped_release release;
        int32 transform_rows = trans.NumRows(),
              transform_cols = trans.NumCols(), feat_dim = feat.NumCols();
        ...
        if (transform_cols == feat_dim) {
          ...
          SubMatrix<BaseFloat> linear_part(trans, 0, transform_rows, 0,
                                           feat_dim);
  ...
  m.def("ApplyCmvnReverse", &ApplyCmvnReverse,
        "This is as ApplyCmvn, but does so in the reverse sense, i.e. applies "
        "a transform "
        "that would take zero-mean, unit-variance input and turn it into "
        "output with the "
        "stats of \"stats\".  This can be useful if you trained without CMVN "
        "but later want "
        "to correct a mismatch, so you would first apply CMVN and then do the "
        "\"reverse\" "
        "CMVN with the summed stats of your training data.",
        py::arg("stats"), py::arg("norm_vars"), py::arg("feats"),
        py::call_guard<py::gil_scoped_release>());
  m.def("FakeStatsForSomeDims", &FakeStatsForSomeDims,
        "Modify the stats so that for some dimensions (specified in \"dims\"), "
        "we "
        "replace them with \"fake\" stats that have zero mean and unit "
        "variance; this "
...
void pybind_compressed_transform_stats(py::module &m) {
  {
    ...
    auto compressed_affine_xform_stats =
        py::class_<PyClass>(m, "CompressedAffineXformStats");
    compressed_affine_xform_stats.def(py::init<>())
        .def(py::init<const AffineXformStats &>(), py::arg("input"))
...
void pybind_decodable_am_diag_gmm_regtree(py::module &m) {
  {
    ...
    auto decodable_am_diag_gmm_regtree_fmllr =
        py::class_<PyClass, DecodableAmDiagGmmUnmapped>(
            m, "DecodableAmDiagGmmRegtreeFmllr");
    decodable_am_diag_gmm_regtree_fmllr
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const Matrix<BaseFloat> &, const RegtreeFmllrDiagGmm &,
                      const RegressionTree &, BaseFloat, BaseFloat>(),
             py::arg("am"), py::arg("tm"), py::arg("feats"),
             py::arg("fmllr_xform"), py::arg("regtree"), py::arg("scale"),
  ...
  }
  {
    ...
    auto decodable_am_diag_gmm_regtree_mllr =
        py::class_<PyClass, DecodableAmDiagGmmUnmapped>(
            m, "DecodableAmDiagGmmRegtreeMllr");
    decodable_am_diag_gmm_regtree_mllr
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const Matrix<BaseFloat> &, const RegtreeMllrDiagGmm &,
                      const RegressionTree &, BaseFloat, BaseFloat>(),
             py::arg("am"), py::arg("tm"), py::arg("feats"),
             py::arg("mllr_xform"), py::arg("regtree"), py::arg("scale"),
             py::arg("log_sum_exp_prune") = -1.0)
        .def("LogLikelihood", &PyClass::LogLikelihood, py::arg("frame"),
             py::arg("tid"))
        .def("NumFramesReady", &PyClass::NumFramesReady)
...
void pybind_fmllr_diag_gmm(py::module &m) {
      ...
      .def_readwrite("num_iters", &FmllrOptions::num_iters);
  {
    ...
    auto fmllr_diag_gmm_accs =
        py::class_<PyClass, AffineXformStats>(m, "FmllrDiagGmmAccs");
    fmllr_diag_gmm_accs
        .def(py::init<const FmllrOptions &>(), py::arg("opts") = FmllrOptions())
        .def(py::init<int32, const FmllrOptions &>(), py::arg("dim"),
             py::arg("opts") = FmllrOptions())
        .def(py::init<const FmllrDiagGmmAccs &>(), py::arg("other"))
        .def(py::init<const DiagGmm &, const AccumFullGmm &>(), py::arg("gmm"),
             py::arg("fgmm_accs"))
        .def("Init", &PyClass::Init, py::arg("dim"))
        .def("Read", &PyClass::Read, py::arg("in"), py::arg("binary"),
             py::arg("add"), py::call_guard<py::gil_scoped_release>())
        .def("AccumulateForGmm", &PyClass::AccumulateForGmm, py::arg("gmm"),
             py::arg("data"), py::arg("weight"),
             py::call_guard<py::gil_scoped_release>())
        .def("AccumulateForGmmPreselect", &PyClass::AccumulateForGmmPreselect,
             py::arg("gmm"), py::arg("gselect"), py::arg("data"),
             py::arg("weight"))
        .def("AccumulateFromPosteriors", &PyClass::AccumulateFromPosteriors,
             py::arg("gmm"), py::arg("data"), py::arg("posteriors"),
             py::call_guard<py::gil_scoped_release>())
        .def(
            "accumulate_from_pdf_post",
            [](PyClass &spk_stats, const Posterior &pdf_post,
               const AmDiagGmm &am_gmm, const Matrix<BaseFloat> &feats) {
        ...
        .def(
            "accumulate_from_gpost",
            [](PyClass &spk_stats, const GaussPost &gpost,
               const AmDiagGmm &am_gmm, const Matrix<BaseFloat> &feats) {
              py::gil_scoped_release gil_release;
            ...
            py::arg("gpost"), py::arg("am_gmm"), py::arg("feats"))
        .def(
            "accumulate_from_alignment",
            [](PyClass &spk_stats, const TransitionModel &alignment_trans_model,
               const AmDiagGmm &alignment_am_gmm,
               const TransitionModel &trans_model, const AmDiagGmm &am_gmm,
               const Matrix<BaseFloat> &feats, const std::vector<int32> &ali,
               const ConstIntegerSet<int32> &silence_set,
               BaseFloat silence_scale, BaseFloat rand_prune = 0.0,
               bool distributed = false, bool two_models = false) {
              py::gil_scoped_release gil_release;
              Posterior posterior;
              ...
              ConvertPosteriorToPdfs(alignment_trans_model, posterior,
                                     &pdf_posterior);
                  ...
                  for (size_t j = 0; j < pdf_posterior[i].size(); j++) {
                    ...
                    if (rand_prune > 0.0)
                      for (int32 k = 0; k < this_post_vec.Dim(); k++)
                        this_post_vec(k) =
                            RandPrune(this_post_vec(k), rand_prune);
            ...
            py::arg("alignment_trans_model"), py::arg("alignment_am_gmm"),
            py::arg("trans_model"), py::arg("am_gmm"), py::arg("feats"),
            py::arg("ali"), py::arg("silence_set"), py::arg("silence_scale"),
            py::arg("rand_prune") = 0.0, py::arg("distributed") = false,
            py::arg("two_models") = false)
        .def(
            "accumulate_from_lattice",
            [](PyClass *spk_stats, const TransitionModel &alignment_trans_model,
               const AmDiagGmm &alignment_am_gmm,
               const TransitionModel &trans_model, const AmDiagGmm &am_gmm,
               const Matrix<BaseFloat> &feats, Lattice &lat,
               const ConstIntegerSet<int32> &silence_set,
               BaseFloat silence_scale, BaseFloat lm_scale = 1.0,
               BaseFloat acoustic_scale = 1.0, BaseFloat rand_prune = 0.0,
               bool distributed = false, bool two_models = false) {
              py::gil_scoped_release gil_release;
                ...
                for (size_t i = 0; i < gpost.size(); i++) {
                  for (size_t j = 0; j < gpost[i].size(); j++) {
                    ...
                    spk_stats->AccumulateFromPosteriors(
                        am_gmm.GetPdf(pdf_id), feats.Row(i), posterior);
                  }
                }
              }
            },
            py::arg("alignment_trans_model"), py::arg("alignment_am_gmm"),
            py::arg("trans_model"), py::arg("am_gmm"), py::arg("feats"),
            py::arg("lat"), py::arg("silence_set"), py::arg("silence_scale"),
            py::arg("lm_scale") = 1.0, py::arg("acoustic_scale") = 1.0,
            py::arg("rand_prune") = 0.0, py::arg("distributed") = false,
            py::arg("two_models") = false)
        .def("AccumulateFromPosteriorsPreselect",
             &PyClass::AccumulateFromPosteriorsPreselect, py::arg("gmm"),
             py::arg("gselect"), py::arg("data"), py::arg("posteriors"))
        .def("Update", &PyClass::Update, py::arg("opts"), py::arg("fmllr_mat"),
             py::arg("objf_impr"), py::arg("count"))
        .def(
            "compute_transform",
            ...
            py::arg("am_gmm"), py::arg("fmllr_opts"));
  }
  m.def("InitFmllr", &InitFmllr,
        "Initializes the FMLLR matrix to its default values.", py::arg("dim"),
        py::arg("out_fmllr"));
  m.def("ComputeFmllrLogDet", &ComputeFmllrLogDet, py::arg("fmllr_mat"));
  m.def("ComputeFmllrMatrixDiagGmmFull", &ComputeFmllrMatrixDiagGmmFull,
        ...
        py::arg("in_xform"), py::arg("stats"), py::arg("num_iters"),
        py::arg("out_xform"));
  m.def("ComputeFmllrMatrixDiagGmmDiagonal", &ComputeFmllrMatrixDiagGmmDiagonal,
        "This does diagonal fMLLR (i.e. only estimate an offset and scale per "
        "dimension).  The format of the output is the same as for the full "
        "case.  Of "
        "course, these statistics are unnecessarily large for this case.  "
        "Returns the "
        "objective function improvement, not normalized by number of frames.",
        py::arg("in_xform"), py::arg("stats"), py::arg("out_xform"));
  m.def("ComputeFmllrMatrixDiagGmmOffset", &ComputeFmllrMatrixDiagGmmOffset,
        "This does offset-only fMLLR, i.e. it only estimates an offset.",
        py::arg("in_xform"), py::arg("stats"), py::arg("out_xform"));
  m.def("ComputeFmllrMatrixDiagGmm", &ComputeFmllrMatrixDiagGmm,
        "This function internally calls ComputeFmllrMatrixDiagGmm{Full, "
        "Diagonal, Offset}, "
        "depending on \"fmllr_type\".",
        py::arg("in_xform"), py::arg("stats"), py::arg("fmllr_type"),
        py::arg("num_iters"), py::arg("out_xform"));
  m.def("FmllrAuxFuncDiagGmm",
        py::overload_cast<const MatrixBase<float> &, const AffineXformStats &>(
            &FmllrAuxFuncDiagGmm),
        "Returns the (diagonal-GMM) FMLLR auxiliary function value given the "
        "transform "
        "and the stats.",
        py::arg("xform"), py::arg("stats"));
  m.def("FmllrAuxFuncDiagGmm",
        py::overload_cast<const MatrixBase<double> &, const AffineXformStats &>(
            &FmllrAuxFuncDiagGmm),
        "Returns the (diagonal-GMM) FMLLR auxiliary function value given the "
        "transform "
        "and the stats.",
        py::arg("xform"), py::arg("stats"));
  m.def("FmllrAuxfGradient", &FmllrAuxfGradient,
        "Returns the (diagonal-GMM) FMLLR auxiliary function value given the "
        "transform "
        "and the stats.",
        py::arg("xform"), py::arg("stats"), py::arg("grad_out"));
  m.def("ApplyFeatureTransformToStats", &ApplyFeatureTransformToStats,
        ...
        py::arg("xform"), py::arg("stats"));
  m.def(
      "ApplyModelTransformToStats", &ApplyModelTransformToStats,
      "ApplyModelTransformToStats takes a transform \"xform\", which must be "
      "diagonal "
      "(i.e. of the form T = [ D; b ] where D is diagonal), and applies it to "
      "the "
      "stats as if we had made it a model-space transform (note that the "
      "transform "
      "applied to the model means is the inverse transform of T).  Thus, if we "
      "are "
      "estimating a transform T U, and we get stats valid for estimating T U "
      "and we "
      "estimate T, we can then call this function (treating T as a model-space "
      "transform) and will get stats valid for estimating U.  This only works "
      "if T is "
      "diagonal, because otherwise the standard stats format is not valid.  "
      "xform must "
      "be of dimension d x d+1",
      py::arg("xform"), py::arg("stats"));
  m.def("FmllrInnerUpdate", &FmllrInnerUpdate,
        ...
        py::arg("inv_G"), py::arg("k"), py::arg("beta"), py::arg("row"),
        py::arg("transform"));
}
...
void pybind_fmllr_raw(py::module &m) {
      ...
      .def_readwrite("num_iters", &FmllrRawOptions::num_iters);
  {
    ...
    auto fmllr_raw_accs = py::class_<PyClass>(m, "FmllrRawAccs");
    fmllr_raw_accs.def(py::init<>())
        .def(py::init<int32, int32, const Matrix<BaseFloat> &>(),
             py::arg("raw_dim"), py::arg("model_dim"),
        ...
        .def(
            "AccumulateForGmm", &PyClass::AccumulateForGmm,
            "Accumulate stats for a single GMM in the model; returns log "
            "likelihood. "
            "Here, \"data\" will typically be of larger dimension than the "
            "model. "
            "Note: \"data\" is the original, spliced features-- before "
            "LDA+MLLT. "
            "Returns log-like for this data given this GMM, including rejected "
            "dimensions (not multiplied by weight).",
            py::arg("gmm"), py::arg("data"), py::arg("weight"))
        .def("AccumulateFromPosteriors", &PyClass::AccumulateFromPosteriors,
             "Accumulate stats for a GMM, given supplied posteriors.  Note: "
             "\"data\" is "
             "the original, spliced features-- before LDA+MLLT. ",
             py::arg("gmm"), py::arg("data"), py::arg("posteriors"))
        .def(
            "Update", &PyClass::Update,
            "Update \"raw_fmllr_mat\"; it should have the correct dimension "
            "and "
            "reasonable values at entry (see the function InitFmllr in "
            "fmllr-diag-gmm.h "
            "for how to initialize it.) "
            "The only reason this function is not const is because we may have "
            "to call CommitSingleFrameStats().",
            py::arg("opts"), py::arg("fmllr_mat"), py::arg("objf_impr"),
            py::arg("count"))
...
void pybind_fmpe(py::module &m) {
  py::class_<FmpeOptions>(m, "FmpeOptions")
      ...
      .def("Write", &FmpeOptions::Write, py::arg("os"), py::arg("binary"),
           py::call_guard<py::gil_scoped_release>())
      .def("Read", &FmpeOptions::Read, py::arg("is"), py::arg("binary"),
           py::call_guard<py::gil_scoped_release>());
  ...
  py::class_<FmpeStats>(m, "FmpeStats")
      .def(py::init<>())
      .def(py::init<const Fmpe &>(), py::arg("fmpe"))
      .def("Init", &FmpeStats::Init)
      .def("Write", &FmpeStats::Write, py::arg("os"), py::arg("binary"),
           py::call_guard<py::gil_scoped_release>())
      .def("Read", &FmpeStats::Read, py::arg("is"), py::arg("binary"),
           py::arg("add") = false, py::call_guard<py::gil_scoped_release>())
      .def("DerivPlus", &FmpeStats::DerivPlus)
      .def("DerivMinus", &FmpeStats::DerivMinus)
      .def("AccumulateChecks", &FmpeStats::AccumulateChecks,
           "If we're using the indirect differential, accumulates certain "
           "quantities "
           "that will be used in the update phase to verify that the "
           "computation "
           "of the indirect differential was done correctly",
           py::arg("feats"), py::arg("direct_deriv"), py::arg("indirect_deriv"))
      .def("DoChecks", &FmpeStats::DoChecks);
  {
    using PyClass = Fmpe;
    auto fmpe = py::class_<PyClass>(m, "Fmpe");
    fmpe.def(py::init<>())
        .def(py::init<const DiagGmm &, const FmpeOptions &>(), py::arg("gmm"),
             py::arg("config"))
        ...
        .def("ComputeFeatures", &PyClass::ComputeFeatures,
             ...
             py::arg("feat_in"), py::arg("gselect"), py::arg("feat_out"))
        .def(
            "AccStats", &PyClass::AccStats,
            ...
            py::arg("feat_in"), py::arg("gselect"),
            py::arg("direct_feat_deriv"), py::arg("indirect_feat_deriv"),
            py::arg("stats"))
        .def("Write", &PyClass::Write, py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
        ...
        .def("Update", &PyClass::Update,
             ...
             py::arg("config"), py::arg("stats"));
  }

  m.def("ComputeAmGmmFeatureDeriv", &ComputeAmGmmFeatureDeriv,
        ...
        "case, not fMPE.",
        py::arg("am_gmm"), py::arg("trans_model"), py::arg("posterior"),
        py::arg("features"), py::arg("direct_deriv"),
        py::arg("model_diff") = NULL, py::arg("indirect_deriv") = NULL);
}
...
void pybind_lda_estimate(py::module &m) {
  py::class_<LdaEstimateOptions>(m, "LdaEstimateOptions")
      ...
      .def_readwrite("within_class_factor",
                     &LdaEstimateOptions::within_class_factor);
  {
    using PyClass = LdaEstimate;
    auto lda_estimate = py::class_<PyClass>(m, "LdaEstimate");
    lda_estimate.def(py::init<>())
        .def("Init", &PyClass::Init, py::arg("num_classes"),
             py::arg("dimension"))
        ...
        .def("Accumulate", &PyClass::Accumulate, py::arg("data"),
             py::arg("class_id"), py::arg("weight") = 1.0)
        .def("Estimate", &PyClass::Estimate,
             ...
             "The \"remove_offset\" argument is new and should be set to false "
             "for back "
             "compatibility.",
        ...
        .def(
            "Add",
            [](PyClass &lda, const PyClass &other) {
              py::gil_scoped_release gil_release;
        ...
        .def(
            "acc_lda",
            [](PyClass &lda, const TransitionModel &trans_model,
               const std::vector<int32> &ali, const Matrix<BaseFloat> &feats,
               const ConstIntegerSet<int32> &silence_set,
               BaseFloat rand_prune = 0.0, BaseFloat silence_weight = 0.0) {
              py::gil_scoped_release gil_release;

              Posterior post;
              AlignmentToPosterior(ali, &post);
              WeightSilencePost(trans_model, silence_set, silence_weight,
                                &post);
              Posterior pdf_post;
              ...
              for (int32 i = 0; i < feats.NumRows(); i++) {
                ...
                for (size_t j = 0; j < pdf_post[i].size(); j++) {
                  int32 pdf_id = pdf_post[i][j].first;
                  BaseFloat weight =
                      RandPrune(pdf_post[i][j].second, rand_prune);
                  if (weight != 0.0) {
            ...
            py::arg("trans_model"), py::arg("ali"), py::arg("feats"),
            py::arg("silence_set"), py::arg("rand_prune") = 0.0,
...
void pybind_lvtln(py::module &m) {
  {
    ...
    auto linear_vtln = py::class_<PyClass>(m, "LinearVtln");
    linear_vtln.def(py::init<>())
        .def(py::init<int32, int32, int32>(), py::arg("dim"),
             py::arg("num_classes"), py::arg("default_class"))
        .def("SetTransform", &PyClass::SetTransform,
             "SetTransform is used when we initialize it as \"normal\" VTLN. "
             "It's not necessary to ever call this function.  \"transform\" is "
             "\"A\", "
             "the square part of the transform matrix.",
             py::arg("i"), py::arg("transform"))
        .def("SetWarp", &PyClass::SetWarp, py::arg("i"), py::arg("warp"))
        .def("GetWarp", &PyClass::GetWarp, py::arg("i"))
        .def("GetTransform", &PyClass::GetTransform, py::arg("i"),
             py::arg("transform"))
        .def("ComputeTransform", &PyClass::ComputeTransform,
             "Compute the transform for the speaker.", py::arg("accs"),
             py::arg("norm_type"), py::arg("logdet_scale"), py::arg("Ws"),
             py::arg("class_idx"), py::arg("logdet_out"),
             py::arg("objf_impr") = NULL, py::arg("count") = NULL)
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
...
void pybind_mllt(py::module &m) {
  {
    ...
    auto mllt_accs = py::class_<PyClass>(m, "MlltAccs");
    mllt_accs.def(py::init<>())
        .def(py::init<int32, BaseFloat>(), py::arg("dim"),
        ...
        .def(
            "Update",
            static_cast<void (PyClass::*)(MatrixBase<BaseFloat> *, BaseFloat *,
                                          BaseFloat *) const>(&PyClass::Update),
            "The Update function does the ML update; it requires that M has "
            "the "
            "right size. "
            " @param [in, out] M  The output transform, will be of dimension "
            "Dim() x Dim(). "
            "                  At input, should be the unit transform (the "
            "objective function "
            "                  improvement is measured relative to this "
            "value). "
            " @param [out] objf_impr_out  The objective function improvement "
            " @param [out] count_out  The data-count",
            py::arg("M"), py::arg("objf_impr_out"), py::arg("count_out"))
        .def(
            "update",
            [](PyClass &mllt_accs) {
        ...
        .def(
            "Add",
            [](PyClass &accs, const PyClass &other) {
              py::gil_scoped_release gil_release;
        ...
        .def_static(
            "Update_",
            py::overload_cast<double, const std::vector<SpMatrix<double>> &,
                              MatrixBase<BaseFloat> *, BaseFloat *,
                              BaseFloat *>(&PyClass::Update),
            "A static version of the Update function, so it can "
            "be called externally, given the right stats.",
            py::arg("beta"), py::arg("G"), py::arg("M"),
            py::arg("objf_impr_out"), py::arg("count_out"))
        .def("AccumulateFromGmm", &PyClass::AccumulateFromGmm, py::arg("gmm"),
             py::arg("data"), py::arg("weight"))
        ...
        .def(
            "gmm_acc_mllt",
            [](PyClass &mllt_accs, const AmDiagGmm &am_gmm,
               const TransitionModel &trans_model,
               const std::vector<int32> &ali, const Matrix<BaseFloat> &mat,
               const ConstIntegerSet<int32> &silence_set,
              ...
              for (size_t i = 0; i < pdf_posterior.size(); i++) {
                for (size_t j = 0; j < pdf_posterior[i].size(); j++) {
                  ...
                  tot_like_this_file +=
                      mllt_accs.AccumulateFromGmm(am_gmm.GetPdf(pdf_id),
                                                  mat.Row(i), weight) *
                      weight;
                  tot_weight += weight;
                }
            ...
            py::arg("gmm"), py::arg("trans_model"), py::arg("ali"),
            py::arg("mat"), py::arg("silence_set"),
            py::arg("silence_weight") = 0.0)
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
...
void pybind_regression_tree(py::module &m) {
  {
    ...
    auto regression_tree = py::class_<PyClass>(m, "RegressionTree");
    regression_tree.def(py::init<>())
        .def("BuildTree", &PyClass::BuildTree,
             "Top-down clustering of the Gaussians in a model based on their "
             "means. "
             "If sil_indices is nonempty, will put silence in a special class "
             "using a top-level split.",
             py::arg("state_occs"), py::arg("sil_indices"), py::arg("am"),
             py::arg("randmax_clusters_prune"))
        .def("GatherStats", &PyClass::GatherStats,
             "Parses the regression tree and finds the nodes whose occupancies "
             "(read "
             "from stats_in) are greater than min_count. The regclass_out "
             "vector has "
             "size equal to number of baseclasses, and contains the regression "
             "class "
             "index for each baseclass. The stats_out vector has size equal to "
             "number "
             "of regression classes. Return value is true if at least one "
             "regression "
             "class passed the count cutoff, false otherwise.",
             py::arg("stats_in"), py::arg("min_count"),
             py::arg("regclasses_out"), py::arg("stats_out"))
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::arg("am"), py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("NumBaseclasses", &PyClass::NumBaseclasses)
        .def("GetBaseclass", &PyClass::GetBaseclass, py::arg("bclass"))
        .def("Gauss2BaseclassId", &PyClass::Gauss2BaseclassId,
             py::arg("pdf_id"), py::arg("gauss_id"));
  }
}
...
void pybind_regtree_fmllr_diag_gmm(py::module &m) {
...

  {
    ...
    auto regtree_fmllr_diag_gmm = py::class_<PyClass>(
        ...
        "(constrained MLLR) is an affine transformation of the feature "
        "vectors. "
        "This class supports multiple transforms, and a regression tree. "
        "For a single, feature-level transformation see "
        "fmllr-diag-gmm-global.h "
        "Note: the \"regression classes\" are the classes after "
        "tree-clustering, "
        "which are smaller in number than the \"base classes\"  (these "
        "correspond "
        "to the leaves of the tree).");
    regtree_fmllr_diag_gmm.def(py::init<>())
        .def(py::init<const RegtreeFmllrDiagGmm &>(), py::arg("other"))
        .def("Init", &PyClass::Init, py::arg("num_xforms"), py::arg("dim"))
        ...
        .def("GetXformMatrix", &PyClass::GetXformMatrix, py::arg("xform_index"),
             py::arg("out"))
        .def("GetLogDets", &PyClass::GetLogDets, py::arg("out"))
        .def("Base2RegClass", &PyClass::Base2RegClass, py::arg("bclass"))
        .def("SetParameters", &PyClass::SetParameters, py::arg("mat"),
             py::arg("regclass"))
  ...
  }
  {
    ...
    auto regtree_fmllr_diag_gmm_accs = py::class_<PyClass>(
        ...
        "Class for computing the accumulators needed for the "
        "maximum-likelihood "
        "estimate of FMLLR transforms for an acoustic model that uses diagonal "
        "Gaussian mixture models as emission densities.");
    regtree_fmllr_diag_gmm_accs.def(py::init<>())
        .def("Init", &PyClass::Init, py::arg("num_bclass"), py::arg("dim"))
        .def("SetZero", &PyClass::SetZero)
        .def("AccumulateForGmm", &PyClass::AccumulateForGmm,
             ...
             "do a 2nd pass of regression-tree fMLLR estimation, which as I "
             "write "
             "(Dan, 2016) I'm not sure that this framework even supports.",
             py::arg("regtree"), py::arg("am"), py::arg("data"),
             py::arg("pdf_index"), py::arg("weight"))
        .def("AccumulateForGaussian", &PyClass::AccumulateForGaussian,
             "Accumulate stats for a single Gaussian component in the model.",
             py::arg("regtree"), py::arg("am"), py::arg("data"),
             py::arg("pdf_index"), py::arg("gauss_index"), py::arg("weight"))
        .def("Update", &PyClass::Update, py::arg("regtree"), py::arg("opts"),
             py::arg("out_fmllr"), py::arg("auxf_impr"), py::arg("tot_t"))
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::arg("add"), py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
...
void pybind_regtree_mllr_diag_gmm(py::module &m) {
      ...
      .def_readwrite("use_regtree", &RegtreeMllrOptions::use_regtree);
  {
    ...
    auto regtree_mllr_diag_gmm = py::class_<PyClass>(
        ...
        "estimate of FMLLR transforms for an acoustic model that uses diagonal "
        "Gaussian mixture models as emission densities.");
    regtree_mllr_diag_gmm.def(py::init<>())
        .def("Init", &PyClass::Init, py::arg("num_xforms"), py::arg("dim"))
        .def("SetUnit", &PyClass::SetUnit)
        .def("TransformModel", &PyClass::TransformModel, py::arg("regtree"),
             py::arg("am"))
        .def("GetTransformedMeans", &PyClass::GetTransformedMeans,
             "Get all the transformed means for a given pdf.",
             py::arg("regtree"), py::arg("am"), py::arg("pdf_index"),
             py::arg("out"))
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
  ...
  }
  {
    ...
    auto regtree_mllr_diag_gmm_accs =
        py::class_<PyClass>(m, "RegtreeMllrDiagGmmAccs",
                            "Class for computing the maximum-likelihood "
                            "estimates of the parameters of "
                            "an acoustic model that uses diagonal Gaussian "
                            "mixture models as emission "
                            "densities.");
    regtree_mllr_diag_gmm_accs.def(py::init<>())
        .def("Init", &PyClass::Init, py::arg("num_bclass"), py::arg("dim"))
        .def("SetZero", &PyClass::SetZero)
        .def("AccumulateForGmm", &PyClass::AccumulateForGmm,
             "Accumulate stats for a single GMM in the model; returns log "
        ...
        .def("AccumulateForGaussian", &PyClass::AccumulateForGaussian,
             ...
             py::arg("pdf_index"), py::arg("gauss_index"), py::arg("weight"))
        .def("Update", &PyClass::Update, py::arg("regtree"), py::arg("opts"),
             py::arg("out_mllr"), py::arg("auxf_impr"), py::arg("t"))
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::arg("add"), py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
...
void pybind_transform_common(py::module &m) {
  {
    ...
    auto affine_xform_stats = py::class_<PyClass>(m, "AffineXformStats");
    affine_xform_stats.def(py::init<>())
        .def(py::init<const AffineXformStats &>(), py::arg("other"))
        ...
        .def("Dim", &PyClass::Dim)
        .def("SetZero", &PyClass::SetZero)
        .def("CopyStats", &PyClass::CopyStats, py::arg("other"))
        .def("Add", &PyClass::Add, py::call_guard<py::gil_scoped_release>(),
             py::arg("other"))
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::arg("add"), py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
  ...
  m.def("ComposeTransforms", &ComposeTransforms, py::arg("a"), py::arg("b"),
        py::arg("b_is_affine"), py::arg("c"));
  m.def(
      "compose_transforms",
      ...
      py::arg("a"), py::arg("b"), py::arg("b_is_affine"));
  m.def("ApplyAffineTransform", &ApplyAffineTransform,
        "Applies the affine transform 'xform' to the vector 'vec' and "
        "overwrites the "
        "contents of 'vec'.",
        py::arg("xform"), py::arg("vec"));
}
...
void init_transform(py::module &_m) {
  ...
  m.def(
      ...
      [](const TransitionModel &trans_model, const AmDiagGmm &am_gmm,
         const Matrix<BaseFloat> &feats, const std::vector<int32> &ali,
         const ConstIntegerSet<int32> &silence_set, FmllrDiagGmmAccs *spk_stats,
         BaseFloat silence_scale, BaseFloat rand_prune = 0.0,
         bool distributed = false, bool two_models = false) {
        py::gil_scoped_release gil_release;
        Posterior pdf_post;
        ...
        AlignmentToPosterior(ali, &post);
        if (distributed)
          WeightSilencePostDistributed(trans_model, silence_set, silence_scale,
                                       &post);
        else
          WeightSilencePost(trans_model, silence_set, silence_scale, &post);
        ConvertPosteriorToPdfs(trans_model, post, &pdf_post);
        if (!two_models) {
          for (size_t i = 0; i < pdf_post.size(); i++) {
            for (size_t j = 0; j < pdf_post[i].size(); j++) {
      ...
      py::arg("trans_model"), py::arg("am_gmm"), py::arg("feats"),
      py::arg("ali"), py::arg("silence_set"), py::arg("spk_stats"),
      py::arg("silence_scale"), py::arg("rand_prune") = 0.0,
      py::arg("distributed") = false, py::arg("two_models") = false);

  m.def(
      "compute_fmllr_transform",
      [](FmllrDiagGmmAccs *f, int32 dim, const FmllrOptions &fmllr_opts) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("f"), py::arg("am_gmm"), py::arg("fmllr_opts"));
}
