...
void pybind_hmm_topology(py::module &m) {
  {
    ...
    py::class_<State>(
        ...
        .def("__eq__",
             [](const State &s1, const State &s2) { return s1 == s2; })
    ...
    hmm.def(py::init<>())
        ...
        .def(
            "TopologyForPhone", &PyClass::TopologyForPhone,
...
void pybind_hmm_utils(py::module &m) {
  py::class_<HTransducerConfig>(m, "HTransducerConfig")
      ...
      .def_readwrite(
          "nonterm_phones_offset", &HTransducerConfig::nonterm_phones_offset,
          "The integer id of #nonterm_bos in phones.txt, if present. "
          "Only needs to be set if you are doing grammar decoding, "
          "see doc/grammar.dox.");

  m.def(
      "GetHmmAsFsa", &GetHmmAsFsa,
      "Called by GetHTransducer() and probably will not need to be called "
      "directly; "
      "it creates and returns the FST corresponding to the phone.  It's "
      "actually an "
      "acceptor (ilabels equal to olabels), which is why this is called "
      "\"Fsa\" not "
      "\"Fst\".  This acceptor does not include self-loops; you have to call "
      "AddSelfLoops() for that.  (We do that at a later graph compilation "
      "phase, "
      ...
      "  @param context_window  A vector representing the phonetic context; "
      "see "
      ...
      py::arg("context_window"), py::arg("ctx_dep"), py::arg("trans_model"),
      py::arg("config"), py::arg("cache") = NULL);

  m.def(
      "GetHmmAsFsaSimple", &GetHmmAsFsaSimple,
  ...
  m.def(
      "GetHTransducer", &GetHTransducer,
      "Returns the H tranducer; result owned by caller.  Caution: our version "
      "of "
      "the H transducer does not include self-loops; you have to add those "
      "later. "
      "See \\ref hmm_graph_get_h_transducer.  The H transducer has on the "
      "input transition-ids, and also possibly some disambiguation symbols, "
      "which "
      "will be put in disambig_syms.  The output side contains the identifiers "
      "that "
      "are indexes into \"ilabel_info\" (these represent phones-in-context or "
      "disambiguation symbols).  The ilabel_info vector allows GetHTransducer "
      "to map "
      "from symbols to phones-in-context (i.e. phonetic context windows).  Any "
      "singleton symbols in the ilabel_info vector which are not phones, will "
      "be "
      "treated as disambiguation symbols.  [Not all recipes use these].  The "
      "output "
      "\"disambig_syms_left\" will be set to a list of the disambiguation "
      "symbols on "
      "the input of the transducer (i.e. same symbol type as whatever is on "
      "the "
      "input of the transducer",
      py::arg("ilabel_info"), py::arg("ctx_dep"), py::arg("trans_model"),
      py::arg("config"), py::arg("disambig_syms_left"));

  m.def("GetIlabelMapping", &GetIlabelMapping,
        "GetIlabelMapping produces a mapping that's similar to HTK's "
        "logical-to-physical "
        "model mapping (i.e. the xwrd.clustered.mlist files).   It groups "
        "together "
        "\"logical HMMs\" (i.e. in our world, phonetic context windows) that "
        "share the "
        "same sequence of transition-ids.   This can be used in an "
        "optional graph-creation step that produces a remapped form of CLG "
        "that can be "
        "more productively determinized and minimized.  This is used in the "
        "command-line program "
        "make-ilabel-transducer.cc. "
        "@param ilabel_info_old [in] The original \\ref tree_ilabel "
        "\"ilabel_info\" vector "
        "@param ctx_dep [in] The tree "
        "@param trans_model [in] The transition-model object "
        "@param old2new_map [out] The output; this vector, which is of size "
        "equal to the "
        "      number of new labels, is a mapping to the old labels such that "
        "we could "
  ...
  /*m.def("AddSelfLoops",
        ...
        "For context, see \\ref hmm_graph_add_self_loops.  Expands an FST that
     has been " "built without self-loops, and adds the self-loops (it also
     needs to modify " "the probability of the non-self-loop ones, as the graph
     without self-loops " "was created in such a way that it was stochastic).
     Note that the " "disambig_syms will be empty in some recipes (e.g.  if you
     already removed " "the disambiguation symbols). " "This function will treat
     numbers over 10000000 (kNontermBigNumber) the " "same as disambiguation
     symbols, assuming they are special symbols for " "grammar decoding. "
          "\n"
          "@param trans_model [in] Transition model "
          "@param disambig_syms [in] Sorted, uniq list of disambiguation
     symbols, required " "      if the graph contains disambiguation symbols but
     only needed for sanity checks. "
          "@param self_loop_scale [in] Transition-probability scale for
     self-loops; c.f. " "                   \\ref hmm_scale "
          "@param reorder [in] If true, reorders the transitions (see \\ref
     hmm_reorder). " "                    You'll normally want this to be true.
     "
          "@param check_no_self_loops [in]  If true, it will check that there
     are no " "                     self-loops in the original graph; you'll
     normally want " "                     this to be true.  If false, it will
     allow them, and " "                     will add self-loops after the
     original self-loop " "                     transitions, assuming
     reorder==true... this happens to " "                     be what we want
     when converting normal to unconstrained " "                     chain
     examples.  WARNING: this was added in 2018; " "                     if you
     get a compilation error, add this as 'true', " "                     which
     emulates the behavior of older code. "
  ...
  m.def("AddTransitionProbs",
        py::overload_cast<const TransitionModel &, const std::vector<int32> &,
                          BaseFloat, BaseFloat, fst::VectorFst<fst::StdArc> *>(
            &AddTransitionProbs),
        "Adds transition-probs, with the supplied "
        "scales (see \\ref hmm_scale), to the graph. "
        "Useful if you want to create a graph without transition probs, then "
        "possibly "
        "train the model (including the transition probs) but keep the graph "
        "fixed, "
        "and add back in the transition probs.  It assumes the fst has "
        "transition-ids "
        "on it.  It is not an error if the FST has no states (nothing will be "
        "done). "
        "@param trans_model [in] The transition model "
        "@param disambig_syms [in] A list of disambiguation symbols, required "
        "if the "
        "                      graph has disambiguation symbols on its input "
        "but only "
        "                      used for checks. "
        "@param transition_scale [in] A scale on transition-probabilities "
        "apart from "
        "                     those involving self-loops; see \\ref hmm_scale. "
        "@param self_loop_scale [in] A scale on self-loop transition "
        "probabilities; "
        "                     see \\ref hmm_scale. "
        "@param  fst [in, out] The FST to be modified.",
        py::arg("trans_model"), py::arg("disambig_syms"),
        py::arg("transition_scale"), py::arg("self_loop_scale"), py::arg("fst"),
        py::call_guard<py::gil_scoped_release>());

  m.def("AddTransitionProbs",
        py::overload_cast<const TransitionModel &, BaseFloat, BaseFloat,
        ...
        "of the pair).",
        py::arg("trans_model"), py::arg("transition_scale"),
        py::arg("self_loop_scale"), py::arg("lat"),
        py::call_guard<py::gil_scoped_release>());

  m.def("GetPdfToTransitionIdTransducer", &GetPdfToTransitionIdTransducer,
        "Returns a transducer from pdfs plus one (input) to  transition-ids "
        "(output). "
  ...
  m.def(
      ...
      py::arg("trans_model"), py::arg("alignment"));

  m.def(
      "ConvertAlignment", &ConvertAlignment,
      ...
      "because the alignment was partial), or because some other error "
      "happened, "
      "such as we couldn't convert the alignment because there were too few "
      "frames "
      "for the new topology. "
      "\n"
      "@param old_trans_model [in]  The transition model that the original "
      "alignment "
      "                             used. "
      "@param new_trans_model [in]  The transition model that we want to use "
      "for the "
      "                             new alignment. "
      "@param new_ctx_dep     [in]  The new tree "
      "@param old_alignment   [in]  The alignment we want to convert "
      "@param subsample_factor [in] The frame subsampling factor... normally "
      "1, but "
      ...
      "                             [note: we actually do this by "
      "interpolating "
      "                             'subsample_factor' separately generated "
      "                             alignments, to keep the phone boundaries "
      "                             the same as the input where possible.] "
      "@param reorder [in]          True if you want the pdf-ids on the new "
      "alignment to "
      "                             be 'reordered'. (vs. the way they appear "
      "in "
      ...
      py::arg("old_trans_model"), py::arg("new_trans_model"),
      py::arg("new_ctx_dep"), py::arg("old_alignment"),
      py::arg("subsample_factor"), py::arg("repeat_frames"), py::arg("reorder"),
      py::arg("phone_map"), py::arg("new_alignment"));

  m.def("ConvertPhnxToProns", &ConvertPhnxToProns,
        ...
        "words in it.",
        py::arg("phnx"), py::arg("words"), py::arg("word_start_sym"),
        py::arg("word_end_sym"), py::arg("prons"));

  m.def("GetRandomAlignmentForPhone", &GetRandomAlignmentForPhone,
        "Generates a random alignment for this phone, of length equal to "
        "alignment->size(), which is required to be at least the MinLength() "
        "of the "
        "topology for this phone, or this function will crash. "
        "The alignment will be without 'reordering'.",
        py::arg("ctx_dep"), py::arg("trans_model"), py::arg("phone_window"),
        py::arg("alignment"));

  m.def(
      "ChangeReorderingOfAlignment", &ChangeReorderingOfAlignment,
      "If the alignment was non-reordered makes it reordered, and vice versa.",
      py::arg("trans_model"), py::arg("alignment"));

  m.def("GetPdfToPhonesMap", &GetPdfToPhonesMap,
        "GetPdfToPhonesMap creates a map which maps each pdf-id into its "
        "corresponding monophones.",
        py::arg("trans_model"), py::arg("pdf2phones"));
}
...
void pybind_posterior(py::module &m) {
...

  {
    ...
    auto posterior_holder = py::class_<PyClass>(m, "PosteriorHolder");
    ...
    posterior_holder.def(py::init<>())
        ...
        .def("Value", &PyClass::Value, py::call_guard<py::gil_scoped_release>())
        .def("Swap", &PyClass::Swap, py::arg("other"))
        .def("ExtractRange", &PyClass::ExtractRange, py::arg("other"),
             py::arg("range"));
  }

  m.def("WritePosterior", &WritePosterior,
        "stand-alone function for writing a Posterior.", py::arg("os"),
        py::arg("binary"), py::arg("post"));

  m.def("ReadPosterior", &ReadPosterior,
        "stand-alone function for reading a Posterior.", py::arg("os"),
        py::arg("binary"), py::arg("post"));

  {
    using PyClass = GaussPostHolder;

    auto gauss_posterior_holder = py::class_<PyClass>(m, "GaussPostHolder");
  ...
  pybind_sequential_table_reader<PosteriorHolder>(m,
                                                  "SequentialPosteriorReader");
  pybind_random_access_table_reader<PosteriorHolder>(
      m, "RandomAccessPosteriorReader");

  pybind_table_writer<GaussPostHolder>(m, "GaussPostWriter");
  pybind_sequential_table_reader<KalpyGaussPostHolder>(
      m, "SequentialGaussPostReader");
  pybind_random_access_table_reader<GaussPostHolder>(
      m, "RandomAccessGaussPostReader");

  m.def("ScalePosterior", &ScalePosterior,
        "Scales the BaseFloat (weight) element in the posterior entries.",
        py::arg("scale"), py::arg("post"),
        py::call_guard<py::gil_scoped_release>());

  m.def("TotalPosterior", &TotalPosterior,
        "Returns the total of all the weights in \"post\".", py::arg("post"));

  m.def("PosteriorEntriesAreDisjoint", &PosteriorEntriesAreDisjoint,
        "Returns true if the two lists of pairs have no common .first element.",
        py::arg("post_elem1"), py::arg("post_elem2"));

  m.def(
      "MergePosteriors", &MergePosteriors,
      "Merge two sets of posteriors, which must have the same length.  If "
      "\"merge\" "
      "is true, it will make a common entry whenever there are duplicated "
      "entries, "
      "adding up the weights.  If \"drop_frames\" is true, for frames where "
      "the "
      "two sets of posteriors were originally disjoint, makes no entries for "
      "that "
      "frame (relates to frame dropping, or drop_frames, see Vesely et al, "
      "ICASSP "
      "2013).  Returns the number of frames for which the two posteriors were "
      "disjoint (i.e. no common transition-ids or whatever index we are "
      "using).",
      py::arg("post1"), py::arg("post2"), py::arg("merge"),
      py::arg("drop_frames"), py::arg("post"));

  m.def(
      "VectorToPosteriorEntry", &VectorToPosteriorEntry,
      ...
      "\n"
      "It returns the log of the sum of the selected log-likes that "
      "contributed "
      "to the posterior.",
      py::arg("log_likes"), py::arg("num_gselect"), py::arg("min_post"),
      py::arg("post_entry"));
  ...
  m.def(
      ...
      "each entry).",
      py::arg("ali"), py::return_value_policy::reference);

  m.def("SortPosteriorByPdfs", &SortPosteriorByPdfs,
        "Sorts posterior entries so that transition-ids with same pdf-id are "
        "next to "
        "each other.",
        py::arg("tmodel"), py::arg("post"));

  m.def(
      "convert_posterior_to_pdfs",
      [](const TransitionModel &tmodel, const Posterior &post_in) {
        py::gil_scoped_release release;
        Posterior post_out;
      ...
      "over pdf-ids.",
      py::arg("tmodel"), py::arg("post_in"));

  m.def("ConvertPosteriorToPdfs", &ConvertPosteriorToPdfs,
        "Converts a posterior over transition-ids to be a posterior "
        "over pdf-ids.",
        py::arg("tmodel"), py::arg("post_in"), py::arg("post_out"));

  m.def("ConvertPosteriorToPhones", &ConvertPosteriorToPhones,
        "Converts a posterior over transition-ids to be a posterior "
        "over phones.",
        py::arg("tmodel"), py::arg("post_in"), py::arg("post_out"));

  m.def("WeightSilencePost", &WeightSilencePost,
        "Weight any silence phones in the posterior (i.e. any phones "
        "in the set \"silence_set\" by scale \"silence_scale\". "
        "The interface was changed in Feb 2014 to do the modification "
        "\"in-place\" rather than having separate input and output.",
        py::arg("trans_model"), py::arg("silence_set"),
        py::arg("silence_scale"), py::arg("post"));

  m.def(
      "weight_silence_post",
      [](const TransitionModel &trans_model,
         const ConstIntegerSet<int32> &silence_set, BaseFloat silence_scale,
         Posterior &post, bool distributed = false) {
        py::gil_scoped_release release;
        if (distributed)
      ...
      "The interface was changed in Feb 2014 to do the modification "
      "\"in-place\" rather than having separate input and output.",
      py::arg("trans_model"), py::arg("silence_set"), py::arg("silence_scale"),
      py::arg("post"), py::arg("distributed") = false);

  m.def(
      "ali_to_pdf_post",
      [](const std::vector<int32> &ali, const TransitionModel &trans_model,
         const ConstIntegerSet<int32> &silence_set, BaseFloat silence_scale,
         bool distributed = false) {
        py::gil_scoped_release release;
        Posterior post;
      ...
      py::arg("ali"), py::arg("trans_model"), py::arg("silence_set"),
      py::arg("silence_scale"), py::arg("distributed") = false);

  m.def("WeightSilencePostDistributed", &WeightSilencePostDistributed,
  ...
  m.def(
      "PosteriorToMatrix", &PosteriorToMatrix<double>,
      "This converts a Posterior to a Matrix. The number of matrix-rows is the "
      "same "
      "as the 'post.size()', the number of matrix-columns is defined by "
      "'post_dim'. "
      "The elements which are not specified in 'Posterior' are equal to zero.",
      py::arg("post"), py::arg("post_dim"), py::arg("mat"));

  m.def(
      "PosteriorToMatrix", &PosteriorToMatrix<float>,
      "This converts a Posterior to a Matrix. The number of matrix-rows is the "
      "same "
      "as the 'post.size()', the number of matrix-columns is defined by "
      "'post_dim'. "
  ...
  m.def(
      "PosteriorToPdfMatrix", &PosteriorToPdfMatrix<double>,
      "This converts a Posterior to a Matrix. The number of matrix-rows is the "
      "same "
      "as the 'post.size()', the number of matrix-columns is defined by "
      "'NumPdfs' "
      ...
      "The elements which are not specified in 'Posterior' are equal to zero.",
      py::arg("post"), py::arg("model"), py::arg("mat"));

  m.def(
      "PosteriorToPdfMatrix", &PosteriorToPdfMatrix<float>,
      "This converts a Posterior to a Matrix. The number of matrix-rows is the "
      "same "
      "as the 'post.size()', the number of matrix-columns is defined by "
      "'NumPdfs' "
...
void pybind_transition_model(py::module &m) {
  {
    ...
    py::class_<PyClass, TransitionInformation>(m, "TransitionModel")
        .def(py::init<>())
        .def(
            py::init<const ContextDependencyInterface &, const HmmTopology &>(),
        ...
        .def("TransitionStateToSelfLoopPdfClass",
             &PyClass::TransitionStateToSelfLoopPdfClass,
             py::arg("trans_state"))
        .def("TransitionStateToForwardPdf",
             &PyClass::TransitionStateToForwardPdf, py::arg("trans_state"))
        ...
        .def("MleUpdate", &PyClass::MleUpdate,
             "Does Maximum Likelihood estimation.  The stats are "
             "counts/weights, indexed "
             "by transition-id.  This was previously called Update().",
             py::arg("stats"), py::arg("cfg"), py::arg("objf_impr_out"),
             py::arg("count_out"))
        .def(
            "mle_update",
        ...
        .def("MapUpdate", &PyClass::MapUpdate,
             "Does Maximum A Posteriori (MAP) estimation.  The stats are "
             "counts/weights, "
        ...
        .def("Print", &PyClass::Print, py::arg("os"), py::arg("phone_names"),
             py::arg("occs") = nullptr)
        .def("InitStats", &PyClass::InitStats, py::arg("stats"))
        ...
        .def(
            "acc_stats",
            [](PyClass &trans_model, const std::vector<int32> &alignment,
            ...
            py::arg("alignment"), py::arg("transition_accs"))
        .def(
            "acc_twofeats",
            [](PyClass &trans_model, const Posterior &posterior,
               const Matrix<BaseFloat> &mat1, const Matrix<BaseFloat> &mat2,
               Vector<double> *transition_accs) {
              py::gil_scoped_release release;
            ...
            py::arg("posterior"), py::arg("mat1"), py::arg("mat2"),
    ...
    m.def(
        "GetPhonesForPdfs",
        [](const TransitionModel &trans_model, const std::vector<int32> &pdfs)
            -> std::pair<bool, std::vector<int>> {
...
void pybind_tree_accu(py::module &m) {
  {
    ...
    auto accumulate_tree_stats_options =
        py::class_<PyClass>(m, "AccumulateTreeStatsOptions");
...

  {
    ...
    auto accumulate_tree_stats_info =
        py::class_<PyClass>(m, "AccumulateTreeStatsInfo");
    accumulate_tree_stats_info
        .def(py::init<const AccumulateTreeStatsOptions &>(), py::arg("opts"))
        .def_readwrite("var_floor", &PyClass::var_floor)
  ...
  m.def("AccumulateTreeStats", &AccumulateTreeStats,
        "Accumulates the stats needed for training context-dependency trees "
        "(in the "
        "\"normal\" way).  It adds to 'stats' the stats obtained from this "
        "file.  Any "
        "new GaussClusterable* pointers in \"stats\" will be allocated with "
        "\"new\".",
        py::arg("trans_model"), py::arg("info"), py::arg("alignment"),
        py::arg("features"), py::arg("stats"),
        py::call_guard<py::gil_scoped_release>());
  m.def(
      ...
      [](const TransitionModel &trans_model,
        ...
        AccumulateTreeStats(trans_model, info, alignment, features,
                            &tree_stats);
      ...
      "Accumulates the stats needed for training context-dependency trees (in "
      "the "
      "\"normal\" way).  It adds to 'stats' the stats obtained from this file. "
      " Any "
      "new GaussClusterable* pointers in \"stats\" will be allocated with "
      "\"new\".",
      py::arg("trans_model"), py::arg("info"), py::arg("alignment"),
      py::arg("features"));
  m.def(
      "ReadPhoneMap", &ReadPhoneMap,
      "Read a mapping from one phone set to another.  The phone map file has "
      "lines "
      "of the form <old-phone> <new-phone>, where both entries are integers, "
      "usually "
      "nonzero (but this is not enforced).  This program will crash if the "
      "input is "
      "invalid, e.g. there are multiple inconsistent entries for the same old "
      "phone. "
...
void init_hmm(py::module &_m) {
  ...
  m.def(
      "make_h_transducer",
      [](const ContextDependency &ctx_dep, const TransitionModel &trans_model,
      ...
      py::arg("ctx_dep"), py::arg("trans_model"), py::arg("ilabel_info"));
  ...
  m.def(
      ...
      [](const TransitionModel &old_trans_model,
         ...
         int32 frame_subsampling_factor = 1, bool repeat_frames = false,
         bool reorder = true) {
        py::gil_scoped_release gil_release;
        std::vector<int32> new_alignment;

        ConvertAlignment(old_trans_model, new_trans_model, new_ctx_dep,
                         old_alignment, frame_subsampling_factor, repeat_frames,
                         reorder, NULL, &new_alignment);
        return new_alignment;
      },
      py::arg("old_trans_model"), py::arg("new_trans_model"),
      py::arg("new_ctx_dep"), py::arg("old_alignment"),
      py::arg("frame_subsampling_factor") = 1, py::arg("repeat_frames") = false,
      py::arg("reorder") = true);

  m.def(
      "phones_to_prons",
      [](const std::vector<int32> &phones, const std::vector<int32> &words,
         VectorFst<StdArc> *L, int32 word_start_sym, int32 word_end_sym) {
        py::gil_scoped_release gil_release;
        {
        ...
        VectorFst<StdArc> phones_alt_fst;
        {
          ...
          for (size_t i = 0; i < phones.size(); i++) {
            ...
            phones_alt_fst.AddArc(
                cur_s, StdArc(phones[i], phones[i], Weight::One(), next_s));
            cur_s = next_s;
          }
          for (StateId s = 0; s <= cur_s; s++) {
            phones_alt_fst.AddArc(
                s, StdArc(word_end_sym, word_end_sym, Weight::One(), s));
            phones_alt_fst.AddArc(
                s, StdArc(word_start_sym, word_start_sym, Weight::One(), s));
          }
      ...
      py::arg("phones"), py::arg("words"), py::arg("L"),
      py::arg("word_start_sym"), py::arg("word_end_sym"));
}
