...
void pybind_arctic_weight(py::module &m) {
  using namespace fst;
  {
    ...
    py::class_<PyClass>(m, "ArcticWeight")
        ...
        .def("Quantize", &PyClass::Quantize, py::arg("delta"))
...
void pybind_compose_lattice_pruned(py::module &m) {
  {
    ...
    compose_lattice_pruned_options.def(py::init<>())
        ...
        .def_readwrite("max_arcs", &PyClass::max_arcs,
                       "'max_arcs' is the maximum number of arcs that we are "
                       "willing to expand per "
                       "lattice; once this limit is reached, we terminate the "
                       "composition (however, "
                       "this limit is not applied until at least one path to a "
                       "final-state has been "
                       "produced).")
        .def_readwrite("initial_num_arcs", &PyClass::initial_num_arcs,
                       "'initial_num_arcs' is the number of arcs we use on the "
                       "first outer "
                       "iteration of the algorithm.  This is so unimportant "
                       "that we do not expose "
                       "it on the command line.")
        .def_readwrite("growth_ratio", &PyClass::growth_ratio,
                       "'growth_ratio' determines how much we allow the "
                       "num-arcs to grow on each "
                       "outer iteration of the algorithm.  1.5 is a reasonable "
                       "value; if it is set "
                       "too small, too much time will be taken in "
                       "RecomputePruningInfo(), and if "
                       "too large, the paths searched may be less optimal than "
                       "they could be (the "
                       "heuristics will be less accurate)..");
  }
  m.def(
      "ComposeCompactLatticePruned", &ComposeCompactLatticePruned,
      "Does pruned composition of a lattice 'clat' with a "
      "DeterministicOnDemandFst "
      "'det_fst'; implements LM rescoring. "
      "\n"
      "@param [in] opts Class containing options "
      "@param [in] clat   The input lattice, which is expected to already have "
      "a "
      "                reasonable acoustic scale applied (e.g. 0.1 if it's a "
      "normal "
      "                cross-entropy system, but 1.0 for a chain system); this "
      "scale "
      "                affects the pruning. "
      "@param [in] det_fst   The on-demand FST that we are composing with; its "
      "                ilabels will correspond to words and it should be an "
      "acceptor "
      "                in practice (ilabel == olabel).  Will often contain a "
      "                weighted difference of language model scores, with "
      "scores "
      "                of the form alpha * new - alpha * old, where alpha "
      "                is the interpolation weight for the 'new' language "
      "model "
      "                (e.g. 0.5 or 0.8).  It's non-const because 'det_fst' is "
      "                on-demand. "
      "@param [out] composed_clat  The output, which is a result of composing "
      "                'clat' with '*det_fst'.  Notionally, '*det_fst' is on "
      "the "
      "                right, although both are acceptors so it doesn't really "
      "                matter in practice. "
      "                Although the two FSTs are of different types, the code "
      "                manually does the conversion.  The weights in "
      "'*det_fst' "
      "                will be interpreted as graph weights (Value1()) in the "
      "                lattice semiring.",
      py::arg("opts"), py::arg("clat"), py::arg("det_fst"),
      py::arg("composed_clat"));
}

void pybind_confidence(py::module &m) {
  m.def(
      "SentenceLevelConfidence",
      py::overload_cast<const CompactLattice &, int32 *, std::vector<int32> *,
                        std::vector<int32> *>(&SentenceLevelConfidence),
      "Caution: this function is not the only way to get confidences in Kaldi. "
      "This only gives you sentence-level (utterance-level) confidence.  You "
      "can "
      "get word-by-word confidence within a sentence, along with Minimum Bayes "
      "Risk "
      "decoding, by looking at sausages.h. "
      "Caution: confidences estimated using this type of method are not very "
      "accurate. "
      "This function will return the difference between the best path in clat "
      "and "
      ...
      "was only one path in \"clat\".  It will output to \"num_paths\" (if "
      "non-NULL) "
      "a number n = 0, 1 or 2 saying how many n-best paths (up to two) were "
      "found. "
      "If n >= 1 it outputs to \"best_sentence\" (if non-NULL) the best "
      "word-sequence; "
      "if n == 2 it outputs to \"second_best_sentence\" (if non-NULL) the "
      "second best "
      "word-sequence (this may be useful for testing whether the two best word "
      "sequences are somehow equivalent for the task at hand).  If you need "
      "more "
      "information than this or want to look deeper inside the n-best list, "
      "then "
      "look at the implementation of this function. "
      "This function requires that distinct paths in \"lat\" have distinct "
      "word "
      "sequences; this will automatically be the case if you generated "
      "\"clat\" "
      ...
      py::arg("clat"), py::arg("num_paths"), py::arg("best_sentence"),
      py::arg("second_best_sentence"));
  m.def(
      "SentenceLevelConfidence",
      py::overload_cast<const Lattice &, int32 *, std::vector<int32> *,
                        std::vector<int32> *>(&SentenceLevelConfidence),
      "Caution: this function is not the only way to get confidences in Kaldi. "
      "This only gives you sentence-level (utterance-level) confidence.  You "
      "can "
...
void pybind_kaldi_functions_transition_model(py::module &m) {
  m.def(
      "LatticeForwardBackwardMmi", &LatticeForwardBackwardMmi,
      "This function can be used to compute posteriors for MMI, with a "
      "positive contribution "
      "for the numerator and a negative one for the denominator.  This "
      "function is not actually "
      "used in our normal MMI training recipes, where it's instead done using "
      "various command "
      "line programs that each do a part of the job.  This function was "
      "written for use in "
      ...
      "@param [in] drop_frames   If \"drop_frames\" is true, it will not "
      "compute any "
      "                      posteriors on frames where the num and den have "
      "disjoint "
      "                      pdf-ids. "
      "@param [in] convert_to_pdf_ids   If \"convert_to_pdfs_ids\" is true, it "
      "will "
      "                      convert the output to be at the level of pdf-ids, "
      "not "
      "                      transition-ids. "
      "@param [in] cancel   If \"cancel\" is true, it will cancel out any "
      "positive and "
      ...
      "                      pdf-ids if convert_to_pdf_ids == true) at each "
      "frame "
      ...
      py::arg("trans"), py::arg("lat"), py::arg("num_ali"),
      py::arg("drop_frames"), py::arg("convert_to_pdf_ids"), py::arg("cancel"),
      py::arg("arc_post"));
  m.def(
      "CompactLatticeToWordProns", &CompactLatticeToWordProns,
      ...
      "been "
      "processed so that the arcs in the CompactLattice align correctly with "
      "the "
      ...
      "(sequence "
      "of phones).  This is done even for zero words, corresponding to "
      "optional "
      ...
      "linear).",
      py::arg("tmodel"), py::arg("clat"), py::arg("words"),
      py::arg("begin_times"), py::arg("lengths"), py::arg("prons"),
...
void pybind_lat_kaldi_functions(py::module &m) {
  m.def(
      ...
      "lattice (which we refer to as an 'nbest' lattice elsewhere in Kaldi "
      "code). "
      "The dimension of *per_frame_loglikes will be set to the "
      "number of input symbols in 'nbest'.  The elements of "
      "'*per_frame_loglikes' will be set to the .Value2() elements of the "
      "lattice "
      ...
      "preceding input symbol, or the following input symbol for "
      "input-epsilons "
      "encountered prior to any input symbol.  If 'nbest' has no input "
      "symbols, "
      "'per_frame_loglikes' will be set to the empty vector.",
      py::arg("nbest"));

  m.def("LatticeStateTimes", &LatticeStateTimes,
        "This function iterates over the states of a topologically sorted "
        "lattice and "
        "counts the time instance corresponding to each state. The times are "
        "returned "
        "in a vector of integers 'times' which is resized to have a size equal "
        "to the "
        "number of states in the lattice. The function also returns the "
        "maximum time "
        "in the lattice (this will equal the number of frames in the file).",
        py::arg("lat"), py::arg("times"));

  m.def(
      "CompactLatticeStateTimes", &CompactLatticeStateTimes,
      ...
      "in the final-prob.",
      py::arg("clat"), py::arg("times"));

  m.def(
      "LatticeForwardBackward", &LatticeForwardBackward,
      "This function does the forward-backward over lattices and computes the "
      "posterior probabilities of the arcs. It returns the total "
      "log-probability "
      ...
      py::arg("lat"), py::arg("arc_post"), py::arg("acoustic_like_sum") = NULL);

  m.def("ComputeCompactLatticeAlphas", &ComputeCompactLatticeAlphas,
        "This function is something similar to LatticeForwardBackward(), but "
        "it is on "
        "the CompactLattice lattice format. Also we only need the alpha in the "
        "forward "
        "path, not the posteriors.",
        py::arg("lat"), py::arg("alpha"));

  m.def("ComputeCompactLatticeBetas", &ComputeCompactLatticeBetas,
        "A sibling of the function CompactLatticeAlphas()... We compute the "
        "beta from "
        "the backward path here.",
        py::arg("lat"), py::arg("beta"));
  m.def(
      "ComputeLatticeAlphasAndBetas",
      py::overload_cast<const kaldi::Lattice &, bool, std::vector<double> *,
                        std::vector<double> *>(
          &ComputeLatticeAlphasAndBetas<kaldi::Lattice>),
      ...
      py::arg("lat"), py::arg("viterbi"), py::arg("alpha"), py::arg("beta"));
  m.def(
      "ComputeLatticeAlphasAndBetas",
      py::overload_cast<const kaldi::CompactLattice &, bool,
                        std::vector<double> *, std::vector<double> *>(
          &ComputeLatticeAlphasAndBetas<kaldi::CompactLattice>),
      "Computes (normal or Viterbi) alphas and betas; returns (total-prob, or "
      "best-path negated cost) Note: in either case, the alphas and betas are "
      "negated costs.  Requires that lat be topologically sorted.  This code "
      "will work for either CompactLattice or Lattice.",
      py::arg("lat"), py::arg("viterbi"), py::arg("alpha"), py::arg("beta"));
  m.def("TopSortCompactLatticeIfNeeded", &TopSortCompactLatticeIfNeeded,
        ...
        py::arg("clat"));
  m.def("TopSortLatticeIfNeeded", &TopSortLatticeIfNeeded,
        "Topologically sort the compact lattice if not already topologically "
        "sorted. "
        "Will crash if the lattice cannot be topologically sorted.",
        py::arg("clat"));
  m.def("CompactLatticeDepth", &CompactLatticeDepth,
        ...
        py::arg("clat"), py::arg("num_frames") = NULL);
  m.def("CompactLatticeDepthPerFrame", &CompactLatticeDepthPerFrame,
        "This function returns, for each frame, the number of arcs crossing "
        "that frame.",
        py::arg("clat"), py::arg("depth_per_frame"));
  m.def(
      "CompactLatticeLimitDepth", &CompactLatticeLimitDepth,
      "This function limits the depth of the lattice, per frame: that means, "
      "it "
      "does not allow more than a specified number of arcs active on any given "
      "frame.  This can be used to reduce the size of the \"very deep\" "
      "portions of "
      "the lattice.",
      py::arg("max_arcs_per_frame"), py::arg("clat"));
  m.def("LatticeActivePhones", &LatticeActivePhones,
        ...
        "phones in this list.",
        py::arg("lat"), py::arg("trans"), py::arg("sil_phones"),
        py::arg("active_phones"));
  m.def("ConvertLatticeToPhones", &ConvertLatticeToPhones,
        "Given a lattice, and a transition model to map pdf-ids to phones, "
        ...
        py::arg("trans_model"), py::arg("lat"));
  m.def(
      "PruneLattice",
      py::overload_cast<BaseFloat, kaldi::Lattice *>(
          &PruneLattice<kaldi::Lattice>),
      "Prunes a lattice or compact lattice.  Returns true on success, false if "
      "there was some kind of failure.",
      py::arg("beam"), py::arg("lat"));
  m.def(
      "PruneLattice",
      py::overload_cast<BaseFloat, kaldi::CompactLattice *>(
          &PruneLattice<kaldi::CompactLattice>),
      "Prunes a lattice or compact lattice.  Returns true on success, false if "
      "there was some kind of failure.",
      py::arg("beam"), py::arg("lat"));
  m.def("ConvertCompactLatticeToPhones", &ConvertCompactLatticeToPhones,
        "Given a lattice, and a transition model to map pdf-ids to phones, "
        ...
        py::arg("trans_model"), py::arg("clat"));
  m.def(
      "LatticeBoost", &LatticeBoost,
      ...
      "frame. "
      "This is used in \"margin-inspired\" discriminative training, esp. "
      "Boosted MMI. "
      "The TransitionInformation is used to map transition-ids in the lattice "
      "input-side to phones; the phones appearing in "
      "\"silence_phones\" are treated specially in that we replace the frame "
      "error f "
      ...
      py::arg("trans"), py::arg("alignment"), py::arg("silence_phones"),
      py::arg("b"), py::arg("max_silence_error"), py::arg("lat"));
  m.def(
      "LatticeForwardBackwardMpeVariants", &LatticeForwardBackwardMpeVariants,
      ...
      "criterion of SMBR criterion for this utterance, and outputs the "
      "posteriors (which "
      "may be positive or negative) into \"post\". "
      "\n"
      "@param [in] trans    The transition model. Used to map the "
      "                      transition-ids to phones or pdfs. "
      "@param [in] silence_phones   A list of integer ids of silence phones. "
      "The "
      ...
      "@param [in] lat      The denominator lattice "
      "@param [in] num_ali  The numerator alignment "
      "@param [in] criterion    The objective function. Must be \"mpfe\" or "
      "\"smbr\" "
      ...
      "behavior, "
      "                      where the silence frames (according to num_ali) "
      "are "
      "                      treated as incorrect. However, this means that "
      "the "
      "                      insertions are not penalized by the objective. "
      "                      Setting this to true gives the new behaviour, "
      "where we "
      "                      treat silence as any other phone, except that all "
      "pdfs "
      "                      of silence phones are collapsed into a single "
      "class for "
      "                      the frame-error computation. This can possible "
      "reduce "
      "                      the insertions in the trained model. This is "
      "closer to "
      "                      the WER metric that we actually care about, since "
      "WER is "
      "                      generally computed after filtering out noises, "
      "but "
      "                      does penalize insertions. "
      "  @param [out] post   The \"MBR posteriors\" i.e. derivatives w.r.t to "
      "the "
      "                      pseudo log-likelihoods of states at each frame.",
      py::arg("trans"), py::arg("silence_phones"), py::arg("lat"),
      py::arg("num_ali"), py::arg("criterion"), py::arg("one_silence_class"),
      py::arg("post"));
  ...
  m.def(
      ...
      [](const CompactLattice &clat) {
        ...
        bool ans =
            CompactLatticeToWordAlignment(clat, &words, &times, &lengths);
      ...
      "linear).",
      py::arg("clat"));
  m.def("CompactLatticeShortestPath", &CompactLatticeShortestPath,
        "A form of the shortest-path/best-path algorithm that's specially "
        "coded for "
        "CompactLattice.  Requires that clat be acyclic.",
        py::arg("clat"), py::arg("shortest_path"));
  m.def(
      "ExpandCompactLattice", &ExpandCompactLattice,
      ...
      "splitted.",
      py::arg("clat"), py::arg("epsilon"), py::arg("expand_clat"));
  m.def("CompactLatticeBestCostsAndTracebacks",
        &CompactLatticeBestCostsAndTracebacks,
        "For each state, compute forward and backward best (viterbi) costs and "
        "its "
        "traceback states (for generating best paths later). The forward best "
        "cost "
        "for a state is the cost of the best path from the start state to the "
        "state. "
        "The traceback state of this state is its predecessor state in the "
        "best path. "
        "The backward best cost for a state is the cost of the best path from "
        "the "
        "state to a final one. Its traceback state is the successor state in "
        "the best "
        ...
        py::arg("clat"), py::arg("forward_best_cost_and_pred"),
        py::arg("backward_best_cost_and_pred"));
  m.def(
      "AddNnlmScoreToCompactLattice", &AddNnlmScoreToCompactLattice,
      "This function adds estimated neural language model scores of words in a "
      "minimal list of hypotheses that covers a lattice, to the graph scores "
      "on the "
      "arcs. The list of hypotheses are generated by "
      "latbin/lattice-path-cover.",
      py::arg("nnlm_scores"), py::arg("clat"));
  m.def("AddWordInsPenToCompactLattice", &AddWordInsPenToCompactLattice,
        "This function add the word insertion penalty to graph score of each "
        "word "
        "in the compact lattice",
        py::arg("word_ins_penalty"), py::arg("clat"));
  m.def("RescoreCompactLattice", &RescoreCompactLattice,
        ...
        "should "
        "use ScaleCompactLattice to first set the acoustic scores to zero.  "
        "Returns "
        "true on success, false on error (typically some kind of mismatched "
        "inputs).",
        py::arg("decodable"), py::arg("clat"));
  m.def(
  ...
  m.def(
      ...
      py::arg("lat"));
  m.def(
      "RescoreCompactLatticeSpeedup", &RescoreCompactLatticeSpeedup,
      "This function is like RescoreCompactLattice, but it is modified to "
      "avoid "
      "computing probabilities on most frames where all the pdf-ids are the "
      "same. "
      "(it needs the transition-model to work out whether two transition-ids "
      "map to "
      "the same pdf-id, and it assumes that the lattice has transition-ids on "
      "it). "
      "The naive thing would be to just set all probabilities to zero on "
      "frames "
      ...
      "imagine speedup_factor = 100 (it must be >= 1.0)... with probability "
      "(1.0 / "
      ...
      "expected probability so our corpus-level diagnostics will be about "
      "right.",
      py::arg("tmodel"), py::arg("speedup_factor"), py::arg("decodable"),
      py::arg("clat"));
  m.def(
  ...
  m.def("ComposeCompactLatticeDeterministic",
        ...
        py::arg("clat"), py::arg("det_fst"), py::arg("composed_clat"));
  m.def(
      "ComputeAcousticScoresMap", &ComputeAcousticScoresMap,
      ...
      "                    But if not, we will take the average of the "
      "acoustic "
      ...
      "that "
      "                    frame.",
      py::arg("lat"), py::arg("acoustic_scores"));
  m.def(
      "ReplaceAcousticScoresFromMap", &ReplaceAcousticScoresFromMap,
      ...
      "                     A map from the pair (frame-index, transition-id) "
      "to a "
      "                     pair (sum-of-acoustic-scores, num-of-occurences) "
      "of  "
      ...
      py::arg("acoustic_scores"), py::arg("lat"));
}
...
void pybind_lat_minimize_lattice(py::module &m) {
  ...
  m.def(
      ...
      "(see ./push-lattice.h).  If the lattice is not determinized and pushed "
      "this "
      "function will not combine as many states as it could, but it won't "
      "crash. "
      "Returns true on success, and false if it failed due to topological "
      "sorting "
      "failing. "
      "The output will be topologically sorted.",
      py::arg("clat"), py::arg("delta") = fst::kDelta);
}
...
void pybind_lat_phone_align_lattice(py::module &m) {
  {
    ...
    auto phone_align_lattice_options =
        py::class_<PyClass>(m, "PhoneAlignLatticeOptions");
    phone_align_lattice_options.def(py::init<>())
        ...
        .def_readwrite("replace_output_symbols",
                       &PyClass::replace_output_symbols);
  }
  m.def(
      "PhoneAlignLattice", &PhoneAlignLattice,
      ...
      "false, but an arc may have >1 phone on it, but the boundaries will "
      "still "
      "correspond with the boundaries between phones.  Note: it's possible "
      "to have arcs with words on them but no transition-ids at all.  Returns "
      "true if "
      "everything was OK, false if some kind of error was detected (e.g. the "
      "\"reorder\" option was incorrectly specified.)",
      py::arg("lat"), py::arg("tmodel"), py::arg("opts"), py::arg("lat_out"));
}
...
void pybind_lat_push_lattice(py::module &m) {
  ...
  m.def(
      ...
      "This function pushes the transition-ids as far towards the start as "
      "they "
      ...
      "which should never happen, but we handle it gracefully by just leaving "
      "the "
      "lattice the same. "
  ...
  m.def(
      ...
      "This function pushes the weights in the CompactLattice so that all "
      "states "
      "except possibly the start state, have Weight components (of type "
      "LatticeWeight) that \"sum to one\" in the LatticeWeight (i.e. "
      "interpreting the "
      "weights as negated log-probs).  It returns true on success, false if it "
      "failed due to TopSort failing, which should never happen, but we handle "
      "it "
...
void pybind_lat_sausages(py::module &m) {
  {
    ...
    auto minimum_bayes_risk_options =
        py::class_<PyClass>(m, "MinimumBayesRiskOptions");
    minimum_bayes_risk_options
        .def(py::init<>())

        .def_readwrite("decode_mbr", &PyClass::decode_mbr,
                       "Boolean configuration parameter: if true, we actually "
                       "update the hypothesis "
                       "to do MBR decoding (if false, our output is the MAP "
                       "decoded output, but we "
                       "output the stats too (i.e. the confidences)).")
        .def_readwrite("print_silence", &PyClass::print_silence,
                       "Boolean configuration parameter: if true, the 1-best "
                       "path will 'keep' the <eps> bins,");
  }
  {
    using PyClass = MinimumBayesRisk;

    auto minimum_bayes_risk = py::class_<PyClass>(m, "MinimumBayesRisk");
    minimum_bayes_risk
        .def(py::init<const CompactLattice &, MinimumBayesRiskOptions>(),
             py::arg("clat"), py::arg("opts") = MinimumBayesRiskOptions())
        .def(py::init<const CompactLattice &, const std::vector<int32> &,
                      MinimumBayesRiskOptions>(),
             py::arg("clat"), py::arg("words"),
             py::arg("opts") = MinimumBayesRiskOptions())
        .def(py::init<const CompactLattice &, const std::vector<int32> &,
                      const std::vector<std::pair<BaseFloat, BaseFloat>> &,
                      MinimumBayesRiskOptions>(),
             py::arg("clat"), py::arg("words"), py::arg("times"),
             py::arg("opts") = MinimumBayesRiskOptions())
...
void pybind_lat_word_align_lattice_lexicon(py::module &m) {
  m.def("ReadLexiconForWordAlign", &ReadLexiconForWordAlign,
        ...
        "alignment.  This "
        "is mainly useful when the lattice may have no symbol for the "
        "optional-silence arcs "
        "(so <old-word-id> would equal zero), but we want it to be output with "
        "a symbol on those "
        "arcs (so <new-word-id> would be nonzero). "
        "If the silence should not be added to the lattice, both <old-word-id> "
        "and <new-word-id> "
        "may be zero. "
        "\n"
        "This function is very simple: it just reads in a series of lines from "
        "a text file, "
        "each with at least two integers on them.",
        py::arg("is"), py::arg("lexicon"));
  {
    using PyClass = WordAlignLatticeLexiconInfo;

    auto word_align_lattice_lexicon_info =
        py::class_<PyClass>(m, "WordAlignLatticeLexiconInfo");
    word_align_lattice_lexicon_info
        ...
        .def("IsValidEntry", &PyClass::IsValidEntry,
             ...
             "(output-word phone1 phone2 ...).  This is just used in testing "
             "code.",
             py::arg("entry"))
        .def("EquivalenceClassOf", &PyClass::EquivalenceClassOf,
             "Purely for the testing code, we map words into equivalence "
             "classes derived "
             "from the mappings in the first two fields of each line in the "
             "lexicon.  This "
             "function maps from each word-id to the lowest member of its "
             "equivalence class.",
             py::arg("word"));
  }
  {
    ...
    auto word_align_lattice_lexicon_opts =
        py::class_<PyClass>(m, "WordAlignLatticeLexiconOpts");
  ...
  m.def(
      "WordAlignLatticeLexicon", &WordAlignLatticeLexicon,
      ...
      py::arg("lat"), py::arg("tmodel"), py::arg("lexicon_info"),
      py::arg("opts"), py::arg("lat_out"));
}
...
void pybind_lat_word_align_lattice(py::module &m) {
  {
    ...
    word_boundary_info_opts.def(py::init<>())
        ...
        .def_readwrite("partial_word_label", &PyClass::partial_word_label)
        .def_readwrite("reorder", &PyClass::reorder)
        .def_readwrite("silence_may_be_word_internal",
                       &PyClass::silence_may_be_word_internal)
  ...
  }
  {
    ...
    auto word_boundary_info = py::class_<PyClass>(m, "WordBoundaryInfo");
    word_boundary_info
        .def(py::init<const WordBoundaryInfoNewOpts &>(), py::arg("opts"))
        .def(py::init<const WordBoundaryInfoNewOpts &, std::string>(),
             py::arg("opts"), py::arg("word_boundary_file"))
        .def("Init", &PyClass::Init, py::arg("stream"))
        .def("TypeOfPhone", &PyClass::TypeOfPhone, py::arg("p"));
    py::enum_<WordBoundaryInfo::PhoneType>(word_boundary_info, "PhoneType")
        ...
        .value("kSkipNGkWordBeginAndEndPhoneram",
               WordBoundaryInfo::PhoneType::kWordBeginAndEndPhone)
        .value("kWordInternalPhone",
               WordBoundaryInfo::PhoneType::kWordInternalPhone)
        .value("kNonWordPhone", WordBoundaryInfo::PhoneType::kNonWordPhone)
        .export_values();
  }
  m.def("WordAlignLattice", &WordAlignLattice,
        "Align lattice so that each arc has the transition-ids on it "
        "that correspond to the word that is on that arc.  [May also have "
        "epsilon arcs for optional silences.] "
        ...
        "lattice out.",
        py::arg("lat"), py::arg("tmodel"), py::arg("info"),
        py::arg("max_states"), py::arg("lat_out"));
}
...
void pybind_determinize_lattice_pruned(py::module &m) {
  ...
  m.def("DeterminizeLatticePruned",
        py::overload_cast<const ExpandedFst<kaldi::LatticeArc> &, double,
                          MutableFst<kaldi::LatticeArc> *,
                          DeterminizeLatticePrunedOptions>(
            &DeterminizeLatticePruned<kaldi::LatticeWeight>),
        ...
        "the "
        "first one has an epsilon on the input side.  It also prunes using the "
        "beam "
        "in the \"prune\" parameter.  The input FST must be topologically "
        "sorted in order "
        "for the algorithm to work. For efficiency it is recommended to sort "
        "ilabel as well. "
        "Returns true on success, and false if it had to terminate the "
        "determinization "
        "earlier than specified by the \"prune\" beam-- that is, if it "
        "terminated because "
        ...
        "CompactLattice.",
        py::arg("ifst"), py::arg("prune"), py::arg("ofst"),
  ...
  m.def(
      ...
      py::overload_cast<const kaldi::TransitionInformation &,
                        const ExpandedFst<kaldi::LatticeArc> &, double,
                        MutableFst<kaldi::CompactLatticeArc> *,
                        DeterminizeLatticePhonePrunedOptions>(
          &DeterminizeLatticePhonePruned<kaldi::LatticeWeight, int32>),
      ...
      "first "
      "calls DeterminizeLatticePhonePrunedFirstPass() to do the initial pass "
      "of "
      "determinization on the phone + word lattices. If --word-determinize is "
      "set "
      "true, it then does a second pass of determinization on the word "
      "lattices by "
      "calling DeterminizeLatticePruned(). If both are set to false, then it "
      "gives "
      ...
      "lattice "
      "due to the max-mem constraint.  The result should be the same as "
      "word-level "
      "determinization in general, but for deeper lattices it is a bit faster, "
      "despite the fact that we now have two passes of determinization by "
      "default.",
      py::arg("trans_model"), py::arg("ifst"), py::arg("prune"),
      py::arg("ofst"),
      py::arg("opts") = DeterminizeLatticePhonePrunedOptions());
  m.def(
      "DeterminizeLatticePhonePruned",
      py::overload_cast<const kaldi::TransitionInformation &,
                        MutableFst<kaldi::LatticeArc> *, double,
                        MutableFst<kaldi::CompactLatticeArc> *,
                        DeterminizeLatticePhonePrunedOptions>(
          &DeterminizeLatticePhonePruned<kaldi::LatticeWeight, int32>),
      "This function is a wrapper of DeterminizeLatticePhonePrunedFirstPass() "
  ...
  /*m.def("DeterminizeLatticeInsertPhones",
     ...
     The " "returning value is the starting index of the phone label. Typically
     we pick "
        "(maximum_output_label_index + 1) as this value. The inserted phones are
     then " "mapped to (returning_value + original_phone_label) in the new
     lattice. The " "returning value will be used by
     DeterminizeLatticeDeletePhones() where it " "works out the phones according
     to this value.", py::arg("trans_model"), py::arg("fst"));
      */
  m.def("DeterminizeLatticeDeletePhones",
        ...
        "\"phones\" "
        "here are actually any label that is larger than first_phone_label "
        "because "
        "when we insert phones into the lattice, we map the original phone "
        "label to "
        "(first_phone_label + original_phone_label). It is supposed to be used "
        "together with DeterminizeLatticeInsertPhones()",
        py::arg("first_phone_label"), py::arg("fst"));
  m.def("DeterminizeLatticePhonePrunedWrapper",
        &DeterminizeLatticePhonePrunedWrapper,
        "This function is a wrapper of DeterminizeLatticePhonePruned() that "
        "works for "
        ...
        "requires \"ifst\" to have transition-id's on the input side and words "
        "on the "
        "output side. "
...
void pybind_kaldi_lattice(py::module &m) {
  ...
  pybind_random_access_table_reader<LatticeHolder>(m,
...
void init_lat(py::module &_m) {
  ...
  m.def(
      "linear_to_lattice",
      [](const std::vector<int32> &ali, const std::vector<int32> &words,
         BaseFloat lm_cost = 0.0, BaseFloat ac_cost = 0.0) -> CompactLattice {
        Lattice lat_out;
        CompactLattice clat;
      ...
      py::arg("ali"), py::arg("words"), py::arg("lm_cost") = 0.0,
      py::arg("ac_cost") = 0.0);

  m.def(
      "word_align_lattice_lexicon",
      [](const CompactLattice &clat, const TransitionModel &tmodel,
      ...
      py::arg("clat"), py::arg("tmodel"), py::arg("lexicon_info"),
      py::arg("opts"));

  m.def(
      "lattice_best_path",
      [](CompactLattice &clat, BaseFloat lm_scale = 1.0,
      ...
      py::arg("clat"), py::arg("lm_scale") = 1.0,
      py::arg("acoustic_scale") = 1.0);

  m.def(
      "lattice_to_post",
      [](CompactLattice &clat, BaseFloat beam = 10.0,
         BaseFloat acoustic_scale = 1.0, bool minimize = false) {
      ...
      py::arg("clat"), py::arg("beam") = 10.0, py::arg("acoustic_scale") = 1.0,
      py::arg("minimize") = false, py::return_value_policy::take_ownership);

  m.def(
      "lattice_determinize_pruned",
      [](const Lattice &lat) {
        Posterior post;
        double lat_like = LatticeForwardBackward(lat, &post);
        return py::make_tuple(post, lat_like);
      },
      py::arg("lat"), py::return_value_policy::take_ownership);

  m.def(
      "lm_rescore",
      [](CompactLattice &clat, VectorFst<StdArc> *lm_to_subtract_fst,
         VectorFst<StdArc> *lm_to_add_fst,
         const ComposeLatticePrunedOptions &compose_opts,
         BaseFloat lm_scale = 1.0, BaseFloat acoustic_scale = 1.0) {
        if (lm_to_subtract_fst->Properties(fst::kAcceptor, true) == 0) {
          // If it's not already an acceptor, project on the output, i.e. copy
          // olabels to ilabels.  Generally the G.fst's on disk will have the
          // disambiguation symbol #0 on the input symbols of the backoff arc,
          // and projection will replace them with epsilons which is what is on
          // the output symbols of those arcs.
        ...
        fst::BackoffDeterministicOnDemandFst<StdArc> lm_to_add(*lm_to_add_fst);
        ...
        ComposeCompactLatticePruned(compose_opts, clat, &combined_lms,
                                    &composed_clat);
      ...
      py::arg("clat"), py::arg("lm_to_subtract_fst"), py::arg("lm_to_add_fst"),
      py::arg("compose_opts"), py::arg("lm_scale") = 1.0,
      py::arg("acoustic_scale") = 1.0);
  ...
  m.def(
      ...
      py::arg("clat"), py::arg("lm_to_subtract_fst"), py::arg("const_arpa"),
      py::arg("compose_opts"), py::arg("lm_scale") = 1.0,
      py::arg("acoustic_scale") = 1.0);

  m.def(
      "load_fst_for_subtract",
      [](VectorFst<StdArc> *lm_to_subtract_fst, BaseFloat lm_scale = 1.0) {
        if (lm_to_subtract_fst->Properties(fst::kAcceptor, true) == 0) {
          // If it's not already an acceptor, project on the output, i.e. copy
          // olabels to ilabels.  Generally the G.fst's on disk will have the
          // disambiguation symbol #0 on the input symbols of the backoff arc,
      ...
      py::arg("lm_to_subtract_fst"), py::arg("lm_scale") = 1.0);
