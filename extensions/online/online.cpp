...
void pybind_online_audio_source(py::module &m) {
  {
    ...
    py::class_<PyClass>(m, "OnlineAudioSourceItf")
        .def("Read", &PyClass::Read,
             "Reads from the audio source, and writes the samples converted to "
             "BaseFloat "
             "into the vector pointed by \"data\". "
             "The user sets data->Dim() as a way of requesting that many "
             "samples. "
             "The function returns true if there may be more data, and false "
             "if it "
             "knows we are at the end of the stream. "
             "In case an unexpected and unrecoverable error occurs the "
             "function throws "
             "an exception of type KaldiFatalError (by using KALDI_ERR macro). "
             "\n"
             "NOTE: The older version of this interface had a second parameter "
             "- \"timeout\". "
             "      We decided to remove it, because we don't envision usage "
             "scenarios, "
             "      where \"timeout\" will need to be changed dynamically from "
             "call to call. "
             "      If the particular audio source can experience timeouts for "
             "some reason "
             "      (e.g. the samples are received over a network connection) "
             "      we encourage the implementors to configure timeout using a "
             "      constructor parameter. "
             "      The suggested semantics are: if timeout is used and is "
             "greater than 0, "
             "      this method has to wait no longer than \"timeout\" "
             "milliseconds before "
             "      returning data-- by that time, it will return as much data "
             "as it has.",
             py::arg("data"));
  }
#ifndef KALDI_NO_PORTAUDIO
  {
    ...
    py::class_<OnlinePaSource, OnlineAudioSourceItf>(m, "OnlineAudioSourceItf")
        .def(py::init<const uint32, const uint32, const uint32, const uint32>(),
             py::arg("timeout"), py::arg("sample_rate"), py::arg("rb_size"),
        ...
        .def("TimedOut", &PyClass::TimedOut,
             "Returns True if the last call to Read() failed to read the "
             "requested "
  ...
  m.def(
      ...
      py::arg("input"), py::arg("output"), py::arg("frame_count"),
      py::arg("time_info"), py::arg("status_flags"), py::arg("user_data"));

#endif
  {
    using PyClass = OnlineVectorSource;
    py::class_<OnlineVectorSource, OnlineAudioSourceItf>(
        m, "OnlineVectorSource",
        "Simulates audio input, by returning data from a Vector. "
        "This class is mostly meant to be used for online decoder testing "
        "using "
        "pre-recorded audio")
        .def(py::init<const VectorBase<BaseFloat> &>(), py::arg("input"))
        .def("Read", &PyClass::Read,
             "Reads from the audio source, and writes the samples converted to "
             "BaseFloat "
             "into the vector pointed by \"data\". "
...
void pybind_online_decodable(py::module &m) {
  {
    ...
    py::class_<OnlineDecodableDiagGmmScaled, DecodableInterface>(
        m, "OnlineDecodableDiagGmmScaled",
        "A decodable, taking input from an OnlineFeatureInput object on-demand")
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const BaseFloat, OnlineFeatureMatrix *>(),
...
void pybind_online_faster_decoder(py::module &m) {
  {
    ...
    py::class_<OnlineFasterDecoderOpts, FasterDecoderOptions>(
        m, "OnlineFasterDecoderOpts",
        "Extends the definition of FasterDecoder's options to include "
        "additional "
        ...
        .def_readwrite(
            "max_utt_len_", &PyClass::max_utt_len_,
  ...
  }
  {
    ...
    auto online_faster_decoder = py::class_<OnlineFasterDecoder, FasterDecoder>(
        m, "OnlineFasterDecoder");
    online_faster_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
                      const OnlineFasterDecoderOpts &,
                      const std::vector<int32> &, const TransitionModel &>(),
             py::arg("fst"), py::arg("opts"), py::arg("sil_phones"),
             py::arg("trans_model"))
        ...
        .def("EndOfUtterance", &PyClass::EndOfUtterance,
             "Returns \"true\" if the best current hypothesis ends with long "
             "enough silence")
        .def("frame", &PyClass::frame);
    py::enum_<OnlineFasterDecoder::DecodeState>(online_faster_decoder,
                                                "DecodeState")
...
void pybind_online_feat_input(py::module &m) {
  {
    ...
    py::class_<PyClass>(m, "OnlineFeatInputItf")
        .def("Compute", &PyClass::Compute,
             ...
             "The features may be e.g. extracted from an audio samples, "
             "received and/or "
             "transformed from another OnlineFeatInput class etc. "
             "\n"
             "\"output\" - a matrix to store the extracted feature vectors in "
             "its rows. "
             "           The number of rows (NumRows()) of \"output\" when the "
             "function is "
             "           called, is treated as a hint of how many frames the "
             "user wants, "
             "           but this function does not promise to produce exactly "
             "that many: "
             "           it may be slightly more, less, or even zero, on a "
             "given call. "
             "           Zero frames may be returned because we timed out or "
             "because "
             "           we're at the beginning of the file and some buffering "
             "is going on. "
             "           In that case you should try again.  The function will "
             "return \"false\" "
             "           when it knows the stream is finished, but if it "
             "returns nothing "
             "           several times in a row you may want to terminate "
             "processing the "
             "           stream. "
             "\n"
             "Note: similar to the OnlineAudioSourceItf::Read(), Compute() "
             "previously "
             "      had a second argument - \"timeout\". Again we decided "
             "against including "
             "      this parameter in the interface specification. Instead we "
             "are "
             "      considering time out handling to be implementation detail, "
             "and if needed "
             "      it should be configured, through the descendant class' "
             "constructor, "
             "      or by other means. "
             "      For consistency, we recommend 'timeout' values greater "
             "than zero "
             "      to mean that Compute() should not block for more than that "
             "number "
             "      of milliseconds, and to return whatever data it has, when "
             "the timeout "
             "      period is exceeded. "
             "\n"
             "Returns \"false\" if we know the underlying data source has no "
             "more data, and "
...

  {
    ...
    py::class_<OnlineCmnInput, OnlineFeatInputItf>(m, "OnlineCmnInput")
        .def(py::init<OnlineFeatInputItf *, int32, int32>(), py::arg("input"),
             py::arg("cmn_window"), py::arg("min_window"))
        .def("Compute", &PyClass::Compute,
             "Produces feature vectors in some way. "
             "The features may be e.g. extracted from an audio samples, "
             "received and/or "
...

  {
    ...
    py::class_<OnlineCacheInput, OnlineFeatInputItf>(m, "OnlineCacheInput")
        .def(py::init<OnlineFeatInputItf *>(), py::arg("input"))
        .def("Compute", &PyClass::Compute,
             "The Compute function just forwards to the previous member of the "
             "chain, except that we locally accumulate the result, and "
             "GetCachedData() will return the entire input up to the current "
             "time.",
             py::arg("output"))
        .def("GetCachedData", &PyClass::GetCachedData, py::arg("output"))
        .def("Dim", &PyClass::Dim)
...

  {
    ...
    py::class_<OnlineUdpInput, OnlineFeatInputItf>(m, "OnlineUdpInput")
        .def(py::init<int32, int32 feature_dim>(), py::arg("port"),
             py::arg("feature_dim"))
        .def("Compute", &PyClass::Compute, py::arg("output"))
        .def("Dim", &PyClass::Dim)
...

  {
    ...
    py::class_<OnlineLdaInput, OnlineFeatInputItf>(m, "OnlineLdaInput")
        .def(py::init<OnlineFeatInputItf *, const Matrix<BaseFloat> &, int32,
                      int32>(),
             py::arg("input"), py::arg("transform"), py::arg("left_context"),
...

  {
    ...
    py::class_<OnlineDeltaInput, OnlineFeatInputItf>(m, "OnlineDeltaInput")
        .def(py::init<const DeltaFeaturesOptions &, OnlineFeatInputItf *>(),
             py::arg("delta_opts"), py::arg("input"))
        .def("Compute", &PyClass::Compute,
             "The Compute function just forwards to the previous member of the "
             "chain, except that we locally accumulate the result, and "
             "GetCachedData() will return the entire input up to the current "
  ...
  }
  {
    ...
    auto online_feature_matrix_options =
        py::class_<PyClass>(m, "OnlineFeatureMatrixOptions");
  ...
  }
  {
    ...
    auto online_feature_matrix = py::class_<PyClass>(m, "OnlineFeatureMatrix");
    online_feature_matrix
        .def(py::init<const OnlineFeatureMatrixOptions &,
                      OnlineFeatInputItf *>(),
             py::arg("opts"), py::arg("input"))
        .def("IsValidFrame", &PyClass::IsValidFrame, py::arg("frame"))
        .def("Dim", &PyClass::Dim)
...
void pybind_online_tcp_source(py::module &m) {
#ifndef KALDI_NO_PORTAUDIO
  {
    ...
    auto online_tcp_vector_source =
        py::class_<PyClass, OnlineAudioSourceItf>(m, "OnlineTcpVectorSource");
    online_tcp_vector_source.def(py::init<int32>(), py::arg("socket"))
        .def("Read", &PyClass::Read, py::arg("data"))
