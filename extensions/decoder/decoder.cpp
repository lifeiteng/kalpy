...
void pybind_decoder_biglm_faster_decoder(py::module &m) {
  {
    ...
    auto biglm_faster_decoder_options =
        py::class_<BiglmFasterDecoderOptions, FasterDecoderOptions>(
  ...
  }
  {
    ...
    auto biglm_faster_decoder = py::class_<PyClass>(m, "BiglmFasterDecoder");
    biglm_faster_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
             ...
             py::arg("fst"), py::arg("opts"), py::arg("lm_diff_fst"))
        .def("SetOptions", &PyClass::SetOptions, py::arg("opts"))
        .def("Decode", &PyClass::Decode, py::arg("decodable"))
        .def("ReachedFinal", &PyClass::ReachedFinal)
        .def("GetBestPath", &PyClass::GetBestPath,
             "GetBestPath gets the decoding output.  If \"use_final_probs\" is "
             "true "
             "AND we reached a final state, it limits itself to final states; "
             "otherwise it gets the most likely token not taking into "
             "account final-probs.  fst_out will be empty (Start() == "
             "kNoStateId) if "
             "nothing was available.  It returns true if it got output (thus, "
             "fst_out "
             "will be nonempty).",
             py::arg("fst_out"), py::arg("use_final_probs") = true);
  }
}
...
void pybind_decoder_decodable_mapped(py::module &m) {
  {
    ...
    auto decodable_mapped =
        py::class_<DecodableMapped, DecodableInterface>(m, "DecodableMapped");
    decodable_mapped.def(
        py::init<const std::vector<int32> &, DecodableInterface *>(),
        py::arg("index_map"), py::arg("d"));
  }
}
...
void pybind_decoder_decodable_sum(py::module &m) {
  {
    ...
    auto decodable_sum =
        py::class_<DecodableSum, DecodableInterface>(m, "DecodableSum");
    decodable_sum
        .def(py::init<DecodableInterface *, BaseFloat, DecodableInterface *,
                      BaseFloat>(),
             py::arg("d1"), py::arg("w1"), py::arg("d2"), py::arg("w2"))
        .def(py::init<const std::vector<
                 std::pair<DecodableInterface *, BaseFloat>> &>(),
             py::arg("decodables"));
  }
  {
    using PyClass = DecodableSumScaled;

    auto decodable_sum_scaled =
        py::class_<DecodableSumScaled, DecodableSum>(m, "DecodableSumScaled");
    decodable_sum_scaled
        ...
        .def(
            py::init<
                ...
                BaseFloat>(),
            py::arg("decodables"), py::arg("scale"));
  }
}
...
void pybind_decoder_wrappers(py::module &m) {
  {
    ...
    auto align_config = py::class_<PyClass>(m, "AlignConfig");
    align_config.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              ...
              return py::make_tuple(p.beam, p.retry_beam, p.careful);
            },
  ...
  m.def("AlignUtteranceWrapper", &AlignUtteranceWrapper,
        "AlignUtteranceWapper is a wrapper for alignment code used in "
        "training, that "
        "is called from many different binaries, e.g. gmm-align, "
        "gmm-align-compiled, "
        "sgmm-align, etc.  The writers for alignments and words will only be "
        "written "
        "to if they are open.  The num_done, num_error, num_retried, tot_like "
        "and "
        "frame_count pointers will (if non-NULL) be incremented or added to, "
        "not set, "
        "by this function.",
        py::arg("config"), py::arg("utt"), py::arg("acoustic_scale"),
        py::arg("fst"), py::arg("decodable"), py::arg("alignment_writer"),
        py::arg("scores_writer"), py::arg("num_done"), py::arg("num_error"),
        py::arg("num_retried"), py::arg("tot_like"), py::arg("frame_count"),
        py::arg("per_frame_acwt_writer") = NULL);

  m.def("ModifyGraphForCarefulAlignment", &ModifyGraphForCarefulAlignment,
        "This function modifies the decoding graph for what we call \"careful "
        "alignment\".  The problem we are trying to solve is that if the "
        "decoding eats "
        "up the words in the graph too fast, it can get stuck at the end, and "
        "produce "
        "what looks like a valid alignment even though there was really a "
        "failure. "
        "So what we want to do is to introduce, after the final-states of the "
        "graph, "
        "a \"blind alley\" with no final-probs reachable, where the decoding "
        "can go to "
        "get lost.  Our basic idea is to append the decoding-graph to itself "
        "using "
        "the fst Concat operation; but in order that there should be "
        "final-probs at the end of "
        "the first but not the second FST, we modify the right-hand argument "
        "to the "
        "Concat operation so that it has none of the original final-probs, and "
        "add "
        "a \"pre-initial\" state that is final.",
        py::arg("fst"), py::call_guard<py::gil_scoped_release>());
  {
    ...
    decode_utterance_lattice_faster_class
        .def(py::init<LatticeFasterDecoder *, DecodableInterface *,
                      const TransitionInformation &, const fst::SymbolTable *,
                      const std::string &, BaseFloat, bool, bool,
                      Int32VectorWriter *, Int32VectorWriter *,
                      CompactLatticeWriter *, LatticeWriter *, double *,
                      int64 *, int32 *, int32 *, int32 *>(),
             py::arg("decoder"), py::arg("decodable"), py::arg("trans_model"),
             py::arg("word_syms"), py::arg("utt"), py::arg("acoustic_scale"),
             py::arg("determinize"), py::arg("allow_partial"),
             py::arg("alignments_writer"), py::arg("words_writer"),
             py::arg("compact_lattice_writer"), py::arg("lattice_writer"),
             py::arg("like_sum"), py::arg("frame_sum"), py::arg("num_done"),
             py::arg("num_err"), py::arg("num_partial"))
        .def("__call__", &PyClass::operator());
  }

  m.def("DecodeUtteranceLatticeSimple", &DecodeUtteranceLatticeSimple,
        "This function DecodeUtteranceLatticeSimple is used in several "
        "decoders, and "
        "we have moved it here.  Note: this is really \"binary-level\" code as "
        "it "
        "involves table readers and writers; we've just put it here as there "
        "is no "
...
void pybind_decoder_grammar_fst(py::module &m) {
  {
    ...
    auto grammar_fst_arc = py::class_<PyClass>(m, "GrammarFstArc");
    grammar_fst_arc.def(py::init<>())
        .def(py::init<PyClass::Label, PyClass::Label, PyClass::Weight,
                      PyClass::StateId>(),
  ...
  }
  {
    ...
    auto grammar_fst = py::class_<PyClass>(m, "VectorGrammarFst");
    grammar_fst.def(py::init<>())
        .def(py::init<int32, std::shared_ptr<StdVectorFst>,
                      const std::vector<
                          std::pair<int32, std::shared_ptr<StdVectorFst>>> &>(),
             py::arg("nonterm_phones_offset"), py::arg("top_fst"),
             py::arg("ifsts"))
        .def(py::init<const VectorGrammarFst &>(), py::arg("other"))
        .def("Write", &PyClass::Write,
             "This Write function allows you to dump a GrammarFst to disk as a "
             "single "
             "object.  It only supports binary mode, but the option is allowed "
             "for "
             "compatibility with other Kaldi read/write functions (it will "
             "crash if "
             "binary == false).",
             py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Read", &PyClass::Read,
             "Reads the format that Write() outputs.  Will crash if binary == "
             "false.",
             py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        ...
        .def("NumInputEpsilons", &PyClass::NumInputEpsilons,
             "This is called in LatticeFasterDecoder.  As an implementation "
             "shortcut, if "
             "the state is an expanded state, we return 1, meaning 'yes, there "
             "are input "
             "epsilons'; the calling code doesn't actually care about the "
             "exact number.",
             py::arg("s"))
        .def("Type", &PyClass::Type)
        .def_readwrite(
            "instances_", &PyClass::instances_,
            "The FST instances.  Initially it is a vector with just one "
            "element "
            "representing top_fst_, and it will be populated with more "
            "elements on "
            "demand.  An instance_id refers to an index into this vector.")
        .def_readwrite(
            "nonterm_phones_offset_", &PyClass::nonterm_phones_offset_,
            "The integer id of the symbol #nonterm_bos in phones.txt.")
        .def_readwrite("top_fst_", &PyClass::top_fst_,
                       "The top-level FST passed in by the user; contains the "
                       "start state and "
                       "final-states, and may invoke FSTs in 'ifsts_' (which "
                       "can also invoke "
                       "each other recursively).")
        .def_readwrite(
            "ifsts_", &PyClass::ifsts_,
            "A list of pairs (nonterm, fst), where 'nonterm' is a user-defined "
            "nonterminal symbol as numbered in phones.txt (e.g. #nonterm:foo), "
            "and "
            "'fst' is the corresponding FST.")
        .def_readwrite("nonterminal_map_", &PyClass::nonterminal_map_,
                       "Maps from the user-defined nonterminals like "
                       "#nonterm:foo as numbered in "
                       "phones.txt, to the corresponding index into 'ifsts_', "
                       "i.e. the ifst_index.")
        .def_readwrite(
            "entry_arcs_", &PyClass::entry_arcs_,
            "entry_arcs_ will have the same dimension as ifsts_.  Each "
            "entry_arcs_[i] "
            "is a map from left-context phone (i.e. either a phone-index or "
            "#nonterm_bos) to the corresponding arc-index leaving the "
            "start-state in "
            "the FST 'ifsts_[i].second'. "
            "We populate this only on demand as each one is needed (except for "
            "the "
            "first one, which we populate immediately as a kind of sanity "
            "check). "
            "Doing it on-demand prevents this object's initialization from "
            "being "
            "nontrivial in the case where there are a lot of nonterminals.");

    py::class_<VectorGrammarFst::ExpandedState,
               std::shared_ptr<VectorGrammarFst::ExpandedState>>(
        grammar_fst, "ExpandedState",
        "Represents an expanded state in an FstInstance.  We expand states "
        "whenever "
        "we encounter states with a final-cost equal to "
        "KALDI_GRAMMAR_FST_SPECIAL_WEIGHT (4096.0).  The function "
        "PrepareGrammarFst() makes sure to add this special final-cost on "
        "states "
        "that have special arcs leaving them.")
        .def(py::init<>())
        .def_readwrite(
            "dest_fst_instance", &PyClass::ExpandedState::dest_fst_instance,
            "The final-prob for expanded states is always zero; to avoid "
            "corner cases, we ensure this via adding epsilon arcs where "
            "needed. "
            "\n"
            "fst-instance index of destination state (we will have ensured "
            "previously "
            "that this is the same for all outgoing arcs).")
        .def_readwrite(
            "arcs", &PyClass::ExpandedState::arcs,
            "List of arcs out of this state, where the 'nextstate' element "
            "will be the "
            "lower-order 32 bits of the destination state and the higher order "
            "bits "
            "will be given by 'dest_fst_instance'.  We do it this way, instead "
            "of "
            "constructing a vector<Arc>, in order to simplify the ArcIterator "
            "code and "
            "avoid unnecessary branches in loops over arcs. "
            "We guarantee that this 'arcs' array will always be nonempty; this "
            "is to avoid certain hassles on Windows with automated "
            "bounds-checking.");

    py::class_<VectorGrammarFst::FstInstance>(grammar_fst, "FstInstance")
        .def(py::init<>())
        .def_readwrite("ifst_index", &PyClass::FstInstance::ifst_index,
                       "ifst_index is the index into the ifsts_ vector that "
                       "corresponds to this "
                       "FST instance, or -1 if this is the top-level instance.")
        .def_readwrite(
            "fst", &PyClass::FstInstance::fst,
            "Pointer to the FST corresponding to this instance: it will equal "
            "top_fst_ "
            "if ifst_index == -1, or ifsts_[ifst_index].second otherwise.")
        .def_readwrite(
            "expanded_states", &PyClass::FstInstance::expanded_states,
            "'expanded_states', which will be populated on demand as states in "
            "this "
            "FST instance are accessed, will only contain entries for states "
            "in this "
            "FST that the final-prob's value equal to "
            "KALDI_GRAMMAR_FST_SPECIAL_WEIGHT.  (That final-prob value is used "
            "as a "
            "kind of signal to this code that the state needs expansion).")
        .def_readwrite(
            "child_instances", &PyClass::FstInstance::child_instances,
            "'child_instances', which is populated on demand as states in this "
            "FST "
            "instance are accessed, is logically a map from pair "
            "(nonterminal_index, "
            "return_state) to instance_id.  When we encounter an arc in our "
            "FST with a "
            "user-defined nonterminal indexed 'nonterminal_index' on its "
            "ilabel, and "
            "with 'return_state' as its nextstate, we look up that pair "
            "(nonterminal_index, return_state) in this map to see whether "
            "there "
            "already exists an FST instance for that.  If it exists then the "
            "transition goes to that FST instance; if not, then we create a "
            "new one. "
            "The 'return_state' that's part of the key in this map would be "
            "the same "
            "as the 'parent_state' in that child FST instance, and of course "
            "the "
            "'parent_instance' in that child FST instance would be the "
            "instance_id of "
            "this instance. "
            "\n"
            "In most cases each return_state would only have a single "
            "nonterminal_index, making the 'nonterminal_index' in the key "
            "*usually* "
        ...
        .def_readwrite("parent_instance",
                       ...
                       "The instance-id of the FST we return to when we are "
                       "done with this one "
                       "(or -1 if this is the top-level FstInstance so there "
                       "is nowhere to "
                       "return).")
        .def_readwrite("parent_state", &PyClass::FstInstance::parent_state,
                       "The state in the FST of 'parent_instance' at which we "
                       "expanded this FST "
                       "instance, and to which we return (actually we return "
                       "to the next-states "
                       "of arcs out of 'parent_state').")
        .def_readwrite(
            "parent_reentry_arcs", &PyClass::FstInstance::parent_reentry_arcs,
            "'parent_reentry_arcs' is a map from left-context-phone (i.e. "
            "either a "
            "phone index or #nonterm_bos), to an arc-index, which we could use "
            "to "
            "Seek() in an arc-iterator for state parent_state in the "
            "FST-instance "
            "'parent_instance'.  It's set up when we create this FST instance. "
            " (The "
            "arcs used to enter this instance are not located here, they can "
            "be "
            "located in entry_arcs_[instance_id]).  We make use of "
            "reentry_arcs when "
            "we expand states in this FST that have #nonterm_end on their "
            "arcs, "
  ...
  }
  {
    ...
    auto grammar_fst = py::class_<PyClass>(m, "ConstGrammarFst");
    grammar_fst.def(py::init<>())
        .def(py::init<int32, std::shared_ptr<const ConstFst<StdArc>>,
                      const std::vector<std::pair<
                          int32, std::shared_ptr<const ConstFst<StdArc>>>> &>(),
             py::arg("nonterm_phones_offset"), py::arg("top_fst"),
             py::arg("ifsts"))
    ...
    py::class_<PyClass::FstInstance>(grammar_fst, "FstInstance")
        .def(py::init<>())
        .def_readwrite("ifst_index", &PyClass::FstInstance::ifst_index,
                       "ifst_index is the index into the ifsts_ vector that "
                       "corresponds to this "
  ...
  m.def(
      "PrepareForGrammarFst", &PrepareForGrammarFst,
      "This function prepares 'ifst' for use in GrammarFst: it ensures that it "
      "has "
      "the expected properties, changing it slightly as needed.  'ifst' is "
      "expected "
      "to be a fully compiled HCLG graph that is intended to be used in "
      "GrammarFst. "
      "The user will most likely want to copy it to the ConstFst type after "
      "calling "
      "this function. "
      "\n"
      ...
      "\n"
      "     - To keep the ArcIterator code simple (to avoid branches in "
      "loops), even "
      "     for expanded states we store the destination fst-instance index "
      "     separately per state, not per arc.  This requires that any "
      "transitions "
      "     across FST boundaries from a single FST must be to a single "
      "destination "
      "     FST (for a given source state).  We fix this problem by "
      "introducing "
      "     epsilon arcs and new states whenever we find a state that would "
      "cause a "
      "     problem for the above. "
      "     - In order to signal to the GrammarFst code that a particular "
      "state has "
      "     cross-FST-boundary transitions, we set the final-prob to a nonzero "
      "value "
      "     on that state.  Specifically, we use a weight with Value() == "
      "4096.0. "
      "     When the GrammarFst code sees that value it knows that it was not "
      "a "
      "     'real' final-prob.  Prior to doing this we ensure, by adding "
      "epsilon "
      "     transitions as needed, that the state did not previously have a "
      "     final-prob. "
      "     - For arcs that are final arcs in an FST that represents a "
      "nonterminal "
      "     (these arcs would have #nonterm_exit on them), we ensure that the "
      "     states that they transition to have unit final-prob (i.e. "
      "final-prob "
      "     equal to One()), by incorporating any final-prob into the arc "
      "itself. "
      "     This avoids the GrammarFst code having to inspect those "
      "final-probs "
      ...
      py::arg("nonterm_phones_offset"), py::arg("fst"));
}
...
void pybind_decoder_faster_decoder(py::module &m) {
  ...
  }
  {
    ...
    auto faster_decoder = py::class_<PyClass>(m, "FasterDecoder");
    faster_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
        ...
        .def(
            ...
            "GetBestPath gets the decoding traceback. If \"use_final_probs\" "
            "is true "
            "AND we reached a final state, it limits itself to final states; "
            "otherwise it gets the most likely token not taking into account "
            "final-probs. Returns true if the output best path was not the "
            "empty "
        ...
        .def(
            "AdvanceDecoding", &PyClass::AdvanceDecoding,
...
void pybind_decoder_lattice_biglm_faster_decoder(py::module &m) {
  {
    ...
    auto lattice_biglm_faster_decoder = py::class_<PyClass>(
        ...
        "FST that represents the difference between the language model you "
        "want "
    ...
    lattice_biglm_faster_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
                      const LatticeBiglmFasterDecoderConfig &,
                      fst::DeterministicOnDemandFst<fst::StdArc> *>(),
             py::arg("fst"), py::arg("config"), py::arg("lm_diff_fst"))
        .def("SetOptions", &PyClass::SetOptions, py::arg("config"))
        .def("GetOptions", &PyClass::GetOptions)
        .def("Decode", &PyClass::Decode,
             "Returns true if any kind of traceback is available (not "
             "necessarily from "
             "a final state).",
             py::arg("decodable"))
        .def("ReachedFinal", &PyClass::ReachedFinal,
             ...
             "was not, the "
             "lattice (or traceback) will end with states that are not "
             "final-states.")
        .def(
            "GetBestPath",
        ...
        .def("GetLattice", &PyClass::GetLattice,
             "This function is now deprecated, since now we do determinization "
             "from "
             "outside the LatticeBiglmFasterDecoder class. "
             "Outputs an FST corresponding to the lattice-determinized"
             "lattice (one path per word sequence).",
             py::arg("ofst"), py::arg("use_final_probs") = true);
  }
}
...
void pybind_decoder_lattice_faster_online_decoder(py::module &m) {
  {
    ...
    auto lattice_faster_online_decoder = py::class_<PyClass>(
        ...
        "BestPathEnd()), which is useful in endpointing and in situations "
        "where you "
        "might want to frequently access the best path. "
        "\n"
        "This is only templated on the FST type, since the Token type is "
        "required to "
        ...
        "indirectly via "
        "this child class.");
    lattice_faster_online_decoder
        .def(
            py::init<const fst::StdFst &, const LatticeFasterDecoderConfig &>(),
            py::arg("fst"), py::arg("config"))
        .def(py::init<const LatticeFasterDecoderConfig &, fst::StdFst *>(),
             py::arg("config"), py::arg("fst"))
        .def(
            "GetBestPath",
            [](const PyClass &decoder,
               bool use_final_probs = true) -> std::pair<bool, Lattice> {
            ...
            "lattice. "
            "This is quite efficient because it doesn't get the entire raw "
            "lattice and find "
            "the best path through it; instead, it uses the BestPathEnd and "
            "BestPathIterator "
            "so it basically traces it back through the lattice. "
            "Returns true if result is nonempty (using the return status is "
            "deprecated, "
            "it will become void).  If \"use_final_probs\" is true AND we "
            "reached the "
            "final-state of the graph then it will include those as "
            "final-probs, else "
            "it will treat all final-probs as one.",
            py::arg("use_final_probs") = true)
        .def(
            "TestGetBestPath", &PyClass::TestGetBestPath,
            "This function does a self-test of GetBestPath().  Returns true on "
            "success; returns false and prints a warning on failure.",
            py::arg("use_final_probs") = true)
        .def("BestPathEnd", &PyClass::BestPathEnd,
             "This function returns an iterator that can be used to trace back "
             "the best path.  If use_final_probs == true and at least one "
             "final state "
             "survived till the end, it will use the final-probs in working "
             "out the best "
             "final Token, and will output the final cost to *final_cost (if "
             "non-NULL), "
             "else it will use only the forward likelihood, and will put zero "
             "in "
             "*final_cost (if non-NULL). "
             "Requires that NumFramesDecoded() > 0.",
             py::arg("use_final_probs"), py::arg("final_cost") = NULL)
        .def("TraceBackBestPath", &PyClass::TraceBackBestPath,
             "This function can be used in conjunction with BestPathEnd() to "
             "trace back "
             "the best path one link at a time (e.g. this can be useful in "
             "endpoint "
             "detection).  By \"link\" we mean a link in the graph; not all "
             "links cross "
             "frame boundaries, but each time you see a nonzero ilabel you can "
             "interpret "
             "that as a frame.  The return value is the updated iterator.  It "
             "outputs "
             "the ilabel and olabel, and the (graph and acoustic) weight to "
             "the \"arc\" pointer, "
             "while leaving its \"nextstate\" variable unchanged.",
             py::arg("iter"), py::arg("arc"))
        .def(
            "GetRawLatticePruned", &PyClass::GetRawLatticePruned,
            ...
            "return essentially the same thing as GetRawLattice, but more "
            "slowly.",
            py::arg("ofst"), py::arg("use_final_probs"), py::arg("beam"));

    py::class_<LatticeFasterOnlineDecoder::BestPathIterator>(
        lattice_faster_online_decoder, "BestPathIterator")
        .def(py::init<void *, int32>(), py::arg("t"), py::arg("f"))
        .def_readwrite("tok", &PyClass::BestPathIterator::tok)
        .def_readwrite(
            "frame", &PyClass::BestPathIterator::frame,
            "note, \"frame\" is the frame-index of the frame you'll get the "
            "transition-id for next time, if you call TraceBackBestPath on "
            "this "
            "iterator (assuming it's not an epsilon transition).  Note that "
            "this "
            "is one less than you might reasonably expect, e.g. it's -1 for "
            "the nonemitting transitions before the first frame.")
        .def("Done", &PyClass::BestPathIterator::Done);
  }
}
...
void pybind_decoder_lattice_incremental_decoder(py::module &m) {
  {
    ...
    auto lattice_incremental_decoder_config =
        py::class_<PyClass>(m, "LatticeIncrementalDecoderConfig");
    lattice_incremental_decoder_config.def(py::init<>())
        ...
        .def_readwrite("determinize_min_chunk_size",
                       &PyClass::determinize_min_chunk_size)
        .def_readwrite("determinize_max_active",
                       &PyClass::determinize_max_active)
        .def("Check", &PyClass::Check);
  }
  {
    using PyClass = LatticeIncrementalDeterminizer;

    auto lattice_incremental_determinizer =
        py::class_<PyClass>(m, "LatticeIncrementalDeterminizer");
    lattice_incremental_determinizer
        .def(py::init<const TransitionInformation &,
                      const LatticeIncrementalDecoderConfig &>(),
             py::arg("trans_model"), py::arg("config"))
        .def("Init", &PyClass::Init)
        .def("GetDeterminizedLattice", &PyClass::GetDeterminizedLattice)
        .def(
            "InitializeRawLatticeChunk", &PyClass::InitializeRawLatticeChunk,
            "Starts the process of creating a raw lattice chunk.  (Search the "
            "glossary "
            "for \"raw lattice chunk\").  This just sets up the initial states "
            "and "
            "redeterminized-states in the chunk.  Relates to sec. 5.2 in the "
            "paper, "
            "specifically the initial-state i and the redeterminized-states. "
            "\n"
            "After calling this, the caller would add the remaining arcs and "
            "states "
            ...
            "          in the arcs in final_arcs_; values are states in "
            "`olat`. "
            "          See the last bullet point before Sec. 5.3 in the paper.",
            py::arg("olat"), py::arg("token_label2state"))
        .def("AcceptRawLatticeChunk", &PyClass::AcceptRawLatticeChunk,
             "This function accepts the raw FST (state-level lattice) "
             "corresponding to a "
             "single chunk of the lattice, determinizes it and appends it to "
             "this->clat_. "
             "Unless this was the "
             "\n"
             "Note: final-probs in `raw_fst` are treated specially: they are "
             "used to "
             "guide the pruned determinization, but when you call GetLattice() "
             "it will be "
             "-- except for pruning effects-- as if all nonzero final-probs in "
             "`raw_fst` "
             ...
             "          otherwise. "
             "\n"
             "@return returns false if determinization finished earlier than "
             "the beam "
             "or the determinized lattice was empty; true otherwise. "
             "\n"
             "NOTE: if this is not the final chunk, you will probably want to "
             "call "
             "SetFinalCosts() directly after calling this.",
             py::arg("raw_fst"))
        .def(
            "SetFinalCosts", &PyClass::SetFinalCosts,
            "Sets final-probs in `clat_`.  Must only be called if the final "
            "chunk "
            "has not been processed.  (The final chunk is whenever "
            "GetLattice() is "
            "called with finalize == true). "
            "\n"
            "The reason this is a separate function from "
            "AcceptRawLatticeChunk() is that "
            "there may be situations where a user wants to get the latice with "
            "final-probs in it, after previously getting it without "
            "final-probs; or "
            "vice versa.  By final-probs, we mean the Final() probabilities in "
            "the "
            "HCLG (decoding graph; this->fst_). "
            "\n"
            "     @param [in] token_label2final_cost   A map from the "
            "token-label "
            "          corresponding to Tokens active on the final frame of "
            "the "
            "          lattice in the object, to the final-cost we want to use "
            "for "
            "          those tokens.  If NULL, it means all Tokens should be "
            "treated "
            "          as final with probability One().  If non-NULL, and a "
            "particular "
            "          token-label is not a key of this map, it means that "
            "Token "
            "          corresponded to a state that was not final in HCLG; and "
            "          such tokens will be treated as non-final.  However, "
            "          if this would result in no states in the lattice being "
            "final, "
            "          we will treat all Tokens as final with probability "
            "One(), "
  ...
  }
  {
    ...
    auto lattice_incremental_decoder =
        py::class_<PyClass>(m, "LatticeIncrementalDecoder");
    lattice_incremental_decoder
        .def(py::init<const fst::StdFst &, const TransitionInformation &,
                      const LatticeIncrementalDecoderConfig &>(),
             py::arg("fst"), py::arg("trans_model"), py::arg("config"))
        .def(py::init<const LatticeIncrementalDecoderConfig &, fst::StdFst *,
                      const TransitionInformation &>(),
             py::arg("config"), py::arg("fst"), py::arg("trans_model"))
        .def("SetOptions", &PyClass::SetOptions, py::arg("config"))
        .def("GetOptions", &PyClass::GetOptions)
        .def("Decode", &PyClass::Decode,
             "CAUTION: it's unlikely that you will ever want to call this "
             "function.  In a "
             "scenario where you have the entire file and just want to decode "
             "it, there "
             "is no point using this decoder. "
             "\n"
             "An example of how to do decoding together with incremental "
             "determinization. It decodes until there are no more frames left "
             "in the "
             "\"decodable\" object. "
             "\n"
             "In this example, config_.determinize_max_delay, "
             "config_.determinize_min_chunk_size "
             "and config_.determinize_max_active are used to determine the "
             "time to "
             "call GetLattice(). "
             "\n"
             "Users will probably want to use appropriate combinations of "
             "AdvanceDecoding() and GetLattice() to build their application; "
             "this just "
             "gives you some idea how. "
             "\n"
             "The function returns true if any kind of traceback is available "
             "(not "
        ...
        .def(
            ...
            "the end. "
            "\n"
            "@param [in] num_frames_to_include  The number of frames that you "
            "want "
            ...
            "               NumFramesDecoded().  Must be set to true if you "
            "have "
            "               previously called FinalizeDecoding(). "

            "               (If no state was final on frame "
            "`num_frames_to_include`, the "
            "               final-probs won't be included regardless of "
            "               `use_final_probs`; you can test whether this "
            "               was the case by calling ReachedFinal(). "
            "\n"
            "@return clat   The CompactLattice representing what has been "
            "decoded "
            "               up until `num_frames_to_include` (e.g., "
            "LatticeStateTimes() "
            "               on this lattice would return "
            "`num_frames_to_include`). "
            "\n"
        ...
        .def(
            "NumFramesInLattice", &PyClass::NumFramesInLattice,
            "Returns the number of frames in the currently-determinized part "
            "of the "
            "lattice which will be a number in [0, NumFramesDecoded()].  It "
            "will "
            "be the largest number that GetLattice() was called with, but note "
            "that GetLattice() may be called from "
            "UpdateLatticeDeterminization(). "
            "\n"
            "Made available in case the user wants to give that same number to "
            "GetLattice().")
        .def("InitDecoding", &PyClass::InitDecoding,
             "InitDecoding initializes the decoding, and should only be used "
             "if you "
             "intend to call AdvanceDecoding().  If you call Decode(), you "
             "don't need to "
             "call this.  You can also call InitDecoding if you have already "
             "decoded an "
             "utterance and want to start with a new utterance.")
        .def("AdvanceDecoding", &PyClass::AdvanceDecoding,
             "This will decode until there are no more frames ready in the "
             "decodable "
             "object.  You can keep calling it each time more frames become "
             "available "
             "(this is the normal pattern in a real-time/online decoding "
             "scenario). "
             "If max_num_frames is specified, it specifies the maximum number "
             "of frames "
             "the function will decode before returning.",
             py::arg("decodable"), py::arg("max_num_frames") = -1)
        .def(
            "FinalRelativeCost", &PyClass::FinalRelativeCost,
            "FinalRelativeCost() serves the same purpose as ReachedFinal(), "
            "but gives "
            "more information.  It returns the difference between the best "
            "(final-cost "
            "plus cost) of any token on the final frame, and the best cost of "
            "any token "
            "on the final frame.  If it is infinity it means no final-states "
            "were "
            "present on the final frame.  It will usually be nonnegative.  If "
            "it not "
            "too positive (e.g. < 5 is my first guess, but this is not tested) "
            "you can "
        ...
        .def("FinalizeDecoding", &PyClass::FinalizeDecoding,
             "Finalizes the decoding, doing an extra pruning step on the last "
             "frame "
...
void pybind_decoder_lattice_incremental_online_decoder(py::module &m) {
  {
    ...
    auto lattice_incremental_online_decoder = py::class_<PyClass>(
        ...
        "LatticeIncrementalOnlineDecoderTpl is as LatticeIncrementalDecoderTpl "
        "but also "
        "supports an efficient way to get the best path (see the function "
        "BestPathEnd()), which is useful in endpointing and in situations "
        "where you "
        ...
        "required to "
        "be BackpointerToken.  Actually it only makes sense to instantiate "
        "LatticeIncrementalDecoderTpl with Token == BackpointerToken if you do "
        "so indirectly via "
        "this child class.");
    lattice_incremental_online_decoder
        .def(py::init<const fst::StdFst &, const TransitionInformation &,
                      const LatticeIncrementalDecoderConfig &>(),
             py::arg("fst"), py::arg("trans_model"), py::arg("config"))
        .def(py::init<const LatticeIncrementalDecoderConfig &, fst::StdFst *,
        ...
        .def("TraceBackBestPath", &PyClass::TraceBackBestPath,
             ...
             "the \"arc\" pointer, "
             "while leaving its \"nextstate\" variable unchanged.",
             py::arg("iter"), py::arg("final_arccost"));
...
void pybind_decoder_lattice_simple_decoder(py::module &m) {
  {
    ...
    auto lattice_simple_decoder_config =
  ...
  }
  {
    ...
    auto lattice_simple_decoder =
        py::class_<PyClass>(m, "LatticeSimpleDecoder");
    lattice_simple_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
        ...
        .def("InitDecoding", &PyClass::InitDecoding,
             ...
             "intend to call AdvanceDecoding().  If you call Decode(), you "
             "don't need "
             "to call this.  You can call InitDecoding if you have already "
             "decoded an "
             "utterance and want to start with a new utterance.")
        .def(
            "FinalizeDecoding", &PyClass::FinalizeDecoding,
            "This function may be optionally called after AdvanceDecoding(), "
            "when you "
            "do not plan to decode any further.  It does an extra pruning step "
            "that "
            "will help to prune the lattices output by GetLattice and "
            "(particularly) "
            "GetRawLattice more accurately, particularly toward the end of the "
            "utterance.  It does this by using the final-probs in pruning (if "
            "any "
            "final-state survived); it also does a final pruning step that "
            "visits all "
            "states (the pruning that is done during decoding may fail to "
            "prune states "
            "that are within kPruningScale = 0.1 outside of the beam).  If you "
            "call "
            "this, you cannot call AdvanceDecoding again (it will fail), and "
            "you "
        ...
        .def("GetLattice", &PyClass::GetLattice,
             ...
             py::arg("clat"), py::arg("use_final_probs") = true)
        .def("NumFramesDecoded", &PyClass::NumFramesDecoded);
  }
}
...
void pybind_decoder_simple_decoder(py::module &m) {
  {
    ...
    auto simple_decoder =
        py::class_<PyClass>(m, "SimpleDecoder",
                            "Simplest possible decoder, included largely for "
                            "didactic purposes and as a "
                            "means to debug more highly optimized decoders.  "
                            "See \\ref decoders_simple "
                            "for more information.");
    simple_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &, BaseFloat>(),
             py::arg("fst"), py::arg("beam"))
        .def("Decode", &PyClass::Decode,
             "Decode this utterance. "
             "Returns true if any tokens reached the end of the file "
             "(regardless of "
             "whether they are in a final state); query ReachedFinal() after "
             "Decode() "
        ...
        .def(
            ...
            "otherwise it gets the most likely token not taking into account "
            "final-probs. "
            "fst_out will be empty (Start() == kNoStateId) if nothing was "
            "available due to "
            "search error. "
            "If Decode() returned true, it is safe to assume GetBestPath will "
            "return true. "
            "It returns true if the output lattice was nonempty (i.e. had "
            "states in it); "
            "using the return value is deprecated.",
            py::arg("use_final_probs") = true)
        .def("FinalRelativeCost", &PyClass::FinalRelativeCost,
             "FinalRelativeCost() serves the same function as ReachedFinal(), "
             "but gives "
             "more information.  It returns the difference between the best "
             "(final-cost plus "
             "cost) of any token on the final frame, and the best cost of any "
             "token "
             "on the final frame.  If it is infinity it means no final-states "
             "were present "
...
void pybind_lattice_faster_decoder_impl(
  ...
  py::class_<PyClass>(m, class_name.c_str(), class_help_doc.c_str())
      ...
      .def("Decode", &PyClass::Decode,
           ...
           "necessarily from a final state).",
           py::arg("decodable"), py::call_guard<py::gil_scoped_release>())
      .def("ReachedFinal", &PyClass::ReachedFinal,
           "says whether a final-state was active on the last frame.  If it "
...
void pybind_training_graph_compiler(py::module &m) {
  {
    ...
    auto training_graph_compiler_options =
    ...
    training_graph_compiler_options.def(py::init<>())
        .def(py::init<BaseFloat, BaseFloat, bool>(),
             ...
             py::arg("self_loop_scale") = 1.0, py::arg("b") = true)
        ...
        .def(py::pickle(
            [](const TrainingGraphCompilerOptions &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.transition_scale, p.self_loop_scale,
                                    p.rm_eps, p.reorder);
            },
            [](py::tuple t) {  // __setstate__
  ...
  }
  {
    ...
    py::class_<PyClass>(m, "TrainingGraphCompiler", "TrainingGraphCompiler")
        ...
        .def(py::init([](const TransitionModel &trans_model,
                         const ContextDependency &ctx_dep, py::object fst,
          ...
          VectorFst<StdArc> *mf = down_cast<VectorFst<StdArc> *>(
              ptr->__pyx_base._mfst->GetMutableFst<StdArc>());
          TrainingGraphCompiler gc(trans_model, ctx_dep, mf, disambig_syms,
                                   opts);
          return gc;
        }))
        .def(py::init([](const TransitionModel &trans_model,
                         const ContextDependency &ctx_dep,
                         const std::string lex_rxfilename,
                         const std::vector<int32> &disambig_syms,
                         const TrainingGraphCompilerOptions &opts) {
               VectorFst<StdArc> *lex_fst = fst::ReadFstKaldi(lex_rxfilename);
               TrainingGraphCompiler *gc = new TrainingGraphCompiler(
                   trans_model, ctx_dep, lex_fst, disambig_syms, opts);
               return gc;
             }),
             py::return_value_policy::take_ownership)
        .def("CompileGraph", &PyClass::CompileGraph,
             ...
             "This function is not const for technical reasons involving the "
             "cache. "
        ...
        .def(
            ...
            "Same as `CompileGraph`, but uses an external LG fst.",
            py::arg("phone2word_fst"))
        .def("CompileGraphFromLG", &PyClass::CompileGraphFromLG,
             "Same as `CompileGraph`, but uses an external LG fst.",
             py::arg("phone2word_fst"), py::arg("out_fst"))
        .def("CompileGraphs", &PyClass::CompileGraphs,
             "CompileGraphs allows you to compile a number of graphs at the "
             "same "
        ...
        .def(
            ...
            "same "
            "time.  This consumes more memory but is faster.",
            py::arg("word_fsts"), py::return_value_policy::take_ownership)
        .def(
            "CompileGraphFromText", &PyClass::CompileGraphFromText,
        ...
        .def(
            ...
            py::arg("transcript"))
        .def(
            "CompileGraphsFromText", &PyClass::CompileGraphsFromText,
        ...
        .def(
            ...
            "This function creates FSTs from the text and calls CompileGraphs.",
            py::arg("transcripts"), py::return_value_policy::take_ownership);
  }
}
...
void init_decoder(py::module &_m) {
  ...
  m.def(
      ...
      [](LatticeFasterDecoder &decoder,  // not const but is really an input.
         ...
         const TransitionInformation &trans_model, double acoustic_scale,
         bool determinize, bool allow_partial) {
        double likelihood;
