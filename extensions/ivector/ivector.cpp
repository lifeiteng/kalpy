...
void pybind_agglomerative_clustering(py::module &m) {
  {
    ...
    auto ahc_cluster = py::class_<PyClass>(m, "AhcCluster");
    ahc_cluster.def(py::init<int32, int32, int32, std::vector<int32>>(),
                    py::arg("id"), py::arg("p1"), py::arg("p2"),
                    py::arg("utts"));
  }
  {
    using PyClass = AgglomerativeClusterer;

    auto agglomerative_clusterer =
        py::class_<PyClass>(m, "AgglomerativeClusterer");
    agglomerative_clusterer
        .def(py::init<const Matrix<BaseFloat> &, BaseFloat, int32, int32,
                      BaseFloat, std::vector<int32> *>(),
             py::arg("costs"), py::arg("threshold"), py::arg("min_clusters"),
             py::arg("first_pass_max_points"), py::arg("max_cluster_fraction"),
             py::arg("assignments_out"))
        .def("Cluster", &PyClass::Cluster,
             "Clusters points. Chooses single pass or two pass algorithm.")
        .def("ClusterSinglePass", &PyClass::ClusterSinglePass,
             "Clusters points using single pass algorithm.")
        .def("ClusterTwoPass", &PyClass::ClusterTwoPass,
  ...
  m.def(
      ...
      "\\code "
      "    while (num-clusters > min-clusters && smallest-merge-cost <= "
      "threshold) "
      ...
      "\n"
      "If the number of points to cluster is larger than "
      "first-pass-max-points, "
      "then clustering is done in two passes. In the first pass, input points "
      "are "
      "divided into contiguous subsets of size at most first-pass-max-points "
      "and "
...
void pybind_ivector_extractor(py::module &m) {
  {
    ...
    auto ivector_estimation_options =
        py::class_<PyClass>(m, "IvectorEstimationOptions");
    ivector_estimation_options.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              ...
              return py::make_tuple(p.acoustic_weight, p.max_count);
            },
...

  {
    ...
    auto ivector_extractor_utterance_stats =
        py::class_<PyClass>(m, "IvectorExtractorUtteranceStats");
    ivector_extractor_utterance_stats
        .def(py::init<int32, int32, bool>(), py::arg("num_gauss"),
             py::arg("feat_dim"), py::arg("need_2nd_order_stats"))
        .def("AccStats", &PyClass::AccStats, py::arg("feats"), py::arg("post"))
...

  {
    ...
    auto ivector_extractor_options =
        py::class_<PyClass>(m, "IvectorExtractorOptions");
    ivector_extractor_options.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.ivector_dim, p.num_iters, p.use_weights);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 2) throw std::runtime_error("Invalid state!");
...

  {
    ...
    auto ivector_extractor = py::class_<PyClass>(m, "IvectorExtractor");
    ivector_extractor.def(py::init<>())
        .def(py::init<const IvectorExtractorOptions &, const FullGmm &>(),
             py::arg("opts"), py::arg("fgmm"))
        .def("GetIvectorDistribution", &PyClass::GetIvectorDistribution,
             "Gets the distribution over ivectors (or at least, a Gaussian "
             "approximation "
             "to it).  The output \"var\" may be NULL if you don't need it.  "
             "\"mean\", and "
             "\"var\", if present, must be the correct dimension "
             "(this->IvectorDim()). "
             "If you only need a point estimate of the iVector, get the mean "
             "only.",
             py::arg("utt_stats"), py::arg("mean"), py::arg("var"))
        .def("PriorOffset", &PyClass::PriorOffset)
        .def("GetAuxf", &PyClass::GetAuxf,
             "Returns the log-likelihood objective function, summed over "
             "frames, "
             "for this distribution of iVectors (a point distribution, if var "
             "== NULL).",
             py::arg("utt_stats"), py::arg("mean"), py::arg("var"))
        .def("GetAcousticAuxf", &PyClass::GetAcousticAuxf,
             "Returns the data-dependent part of the log-likelihood objective "
             "function, "
             "summed over frames.  If variance pointer is NULL, uses point "
             "value.",
             py::arg("utt_stats"), py::arg("mean"), py::arg("var") = NULL)
        .def("GetPriorAuxf", &PyClass::GetPriorAuxf,
             "Returns the prior-related part of the log-likelihood objective "
             "function. "
             "Note: if var != NULL, this quantity is a *probability*, "
             "otherwise it is "
             "a likelihood (and the corresponding probability is zero).",
             py::arg("mean"), py::arg("var") = NULL)
        .def("GetAcousticAuxfVariance", &PyClass::GetAcousticAuxfVariance,
             "This returns just the part of the acoustic auxf that relates to "
             "the "
             "variance of the utt_stats (i.e. which would be zero if the "
             "utt_stats had "
             "zero variance.  This does not depend on the iVector, it's "
             "included as an "
             "aid to debugging.  We can only get this if we stored the S "
             "statistics.  If "
             "not we assume the variance is generated from the model.",
             py::arg("utt_stats"))
        .def("GetAcousticAuxfMean", &PyClass::GetAcousticAuxfMean,
             "This returns just the part of the acoustic auxf that relates to "
             "the "
             "variance of the utt_stats (i.e. which would be zero if the "
             "utt_stats had "
...

  {
    ...
    auto online_ivector_estimation_stats =
        py::class_<PyClass>(m, "OnlineIvectorEstimationStats");
    online_ivector_estimation_stats
        .def(py::init<int32, BaseFloat, BaseFloat>(), py::arg("ivector_dim"),
             py::arg("prior_offset"), py::arg("max_count"))
        .def(py::init<const OnlineIvectorEstimationStats &>(), py::arg("other"))
        .def(
            "AccStats",
            py::overload_cast<const IvectorExtractor &,
                              const VectorBase<BaseFloat> &,
                              const std::vector<std::pair<int32, BaseFloat>> &>(
                &PyClass::AccStats),
            "Accumulate stats for one frame.", py::arg("extractor"),
            py::arg("feature"), py::arg("gauss_post"))
        .def("AccStats",
             py::overload_cast<
                 const IvectorExtractor &, const MatrixBase<BaseFloat> &,
                 const std::vector<std::vector<std::pair<int32, BaseFloat>>> &>(
                 &PyClass::AccStats),
             "Accumulate stats for a sequence (or collection) of frames.",
             py::arg("extractor"), py::arg("features"), py::arg("gauss_post"))
        .def("IvectorDim", &PyClass::IvectorDim)
        .def("GetIvector", &PyClass::GetIvector, py::arg("num_cg_iters"),
             py::arg("ivector"))
        .def("NumFrames", &PyClass::NumFrames)
        .def("PriorOffset", &PyClass::PriorOffset)
        .def("ObjfChange", &PyClass::ObjfChange, py::arg("ivector"))
  ...
  m.def(
      "EstimateIvectorsOnline", &EstimateIvectorsOnline,
      "This code obtains periodically (for each \"ivector_period\" frames, "
      "e.g. 10 "
      "frames), an estimate of the iVector including all frames up to that "
      "point. "
      "This emulates what you could do in an online/streaming algorithm; its "
      "use is "
      ...
      "ivector-extract-online.cc, that calls this function, in any of the "
      "scripts.]. "
      "Caution: this program outputs the raw iVectors, where the first "
      "component "
      "will generally be very positive.  You probably want to subtract "
      "PriorOffset() "
      "from the first element of each row of the output before writing it out. "
      "For num_cg_iters, we suggest 15.  It can be a positive number (more -> "
      "more "
      "exact, less -> faster), or if it's negative it will do the optimization "
      "exactly each time which is slower. "
      "It returns the objective function improvement per frame from the "
      "\"default\" iVector to "
      "the last iVector estimated.",
      py::arg("feats"), py::arg("post"), py::arg("extractor"),
      py::arg("ivector_period"), py::arg("num_cg_iters"), py::arg("max_count"),
      py::arg("ivectors"));

  {
    using PyClass = IvectorExtractorStatsOptions;

    auto ivector_extractor_stats_options =
        py::class_<PyClass>(m, "IvectorExtractorStatsOptions");
    ivector_extractor_stats_options.def(py::init<>())
        ...
        .def_readwrite("num_samples_for_weights",
                       &PyClass::num_samples_for_weights)
        .def_readwrite("cache_size", &PyClass::cache_size)
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.update_variances, p.compute_auxf,
                                    p.num_samples_for_weights, p.cache_size);
            },
            [](py::tuple t) {  // __setstate__
  ...
  }
  {
    ...
    auto ivector_extractor_estimation_options =
        py::class_<PyClass>(m, "IvectorExtractorEstimationOptions");
    ivector_extractor_estimation_options.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.variance_floor_factor,
                                    p.gaussian_min_count, p.num_threads,
                                    p.diagonalize);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 4) throw std::runtime_error("Invalid state!");
  ...
  }
  {
    ...
    auto ivector_extractor_stats =
        py::class_<PyClass>(m, "IvectorExtractorStats");
    ivector_extractor_stats.def(py::init<>())
        .def(py::init<const IvectorExtractor &,
                      const IvectorExtractorStatsOptions &>(),
             py::arg("extractor"), py::arg("stats_opts"))
        .def("Add", &PyClass::Add, py::arg("other"))
        .def(
            "AccStatsForUtterance",
            py::overload_cast<const IvectorExtractor &,
                              const MatrixBase<BaseFloat> &, const Posterior &>(
                &PyClass::AccStatsForUtterance),
            py::arg("extractor"), py::arg("feats"), py::arg("post"),
            py::call_guard<py::gil_scoped_release>())
        .def("AccStatsForUtterance",
             py::overload_cast<const IvectorExtractor &,
                               const MatrixBase<BaseFloat> &, const FullGmm &>(
                 &PyClass::AccStatsForUtterance),
             py::arg("extractor"), py::arg("feats"), py::arg("fgmm"),
             py::call_guard<py::gil_scoped_release>())
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::arg("add") = false, py::call_guard<py::gil_scoped_release>())
        .def("Write", py::overload_cast<std::ostream &, bool>(&PyClass::Write),
             py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Update", &PyClass::Update,
             "Returns the objf improvement per frame.", py::arg("opts"),
             py::arg("extractor"),
             py::call_guard<py::gil_scoped_release>())
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
        ...
        .def(
            "update",
            [](PyClass &stats, IvectorExtractor &extractor,
               double variance_floor_factor = 0.1,
               double gaussian_min_count = 100.0, int32 num_threads = 1,
            ...
            "Returns the objf improvement per frame.", py::arg("extractor"),
            py::arg("variance_floor_factor") = 0.1,
            py::arg("gaussian_min_count") = 100.0, py::arg("num_threads") = 1,
            py::arg("diagonalize") = true)
        .def("AuxfPerFrame", &PyClass::AuxfPerFrame)
        .def("IvectorVarianceDiagnostic", &PyClass::IvectorVarianceDiagnostic,
...
void pybind_logistic_regression(py::module &m) {
  {
    ...
    auto logistic_regression_config =
        py::class_<PyClass>(m, "LogisticRegressionConfig");
    logistic_regression_config.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.max_steps, p.mix_up, p.normalizer,
                                    p.power);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 4) throw std::runtime_error("Invalid state!");
  ...
  }
  {
    ...
    auto logistic_regression = py::class_<PyClass>(m, "LogisticRegression");
    logistic_regression.def(py::init<>())
        .def("Train", &PyClass::Train,
             "xs and ys are the training data. Each row of xs is a vector "
             "corresponding to the class label in the same row of ys.",
             py::arg("xs"), py::arg("ys"), py::arg("conf"))
        .def("GetLogPosteriors",
             py::overload_cast<const Matrix<BaseFloat> &, Matrix<BaseFloat> *>(
                 &PyClass::GetLogPosteriors),
             "Calculates the log posterior of the class label given the input "
             "xs. "
             ...
             py::arg("xs"), py::arg("log_posteriors"))
        .def("GetLogPosteriors",
             py::overload_cast<const Vector<BaseFloat> &, Vector<BaseFloat> *>(
                 &PyClass::GetLogPosteriors),
             "Calculates the log posterior of the class label given the input "
             "x. "
             "The indices of log_posteriors are the class labels.",
             py::arg("x"), py::arg("log_posteriors"))
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("os"), py::arg("binary"))
        .def("ScalePriors", &PyClass::ScalePriors, py::arg("prior_scales"),
             py::call_guard<py::gil_scoped_release>());
  }
}
...
void pybind_plda(py::module &m) {
  {
    ...
    auto plda_config = py::class_<PyClass>(m, "PldaConfig");
    plda_config.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.normalize_length, p.simple_length_norm);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 2) throw std::runtime_error("Invalid state!");
  ...
  }
  {
    ...
    auto plda = py::class_<PyClass>(m, "Plda");
    plda.def(py::init<>())
        .def(py::init<const Plda &>(), py::arg("other"))
        .def(
            "transform_ivector",
            [](PyClass &plda, const Vector<double> &ivector,
               int32 num_examples) {
              py::gil_scoped_release gil_release;
              PldaConfig plda_config;
              Vector<double> *transformed_ivector =
                  new Vector<double>(plda.Dim());
              plda.TransformIvector(plda_config, ivector, num_examples,
                                    transformed_ivector);
              return transformed_ivector;
            },
            py::arg("ivector"), py::arg("num_examples"))
        .def(
            "transform_ivector",
            [](PyClass &plda, py::array_t<double> ivector, int32 num_examples) {
              py::gil_scoped_release gil_release;
              PldaConfig plda_config;
              Vector<double> *transformed_ivector =
                  new Vector<double>(plda.Dim());
              ...
              plda.TransformIvector(plda_config, ivector_dbl, num_examples,
                                    transformed_ivector);
              return transformed_ivector;
            },
            py::arg("ivector"), py::arg("num_examples"))
        .def(
            "classify_utterance",
            [](PyClass &plda, const Vector<double> &utterance_ivector,
              ...
              for (int32 i = 0; i < speaker_counts.size(); i++) {
                scores(i) = plda.LogLikelihoodRatio(
                    speaker_ivectors[i], speaker_counts[i], utterance_ivector);
              }
              int32 index;
              BaseFloat score = scores.Max(&index);
              return std::make_pair(index, score);
            },
            py::arg("utterance_ivector"), py::arg("speaker_ivectors"),
            py::arg("speaker_counts"))
        .def(
            "classify_utterance",
            [](PyClass &plda, py::array_t<double> utterance_ivector,
              ...
              for (int32 i = 0; i < r_counts.shape(0); i++) {
                ...
                scores(i) = plda.LogLikelihoodRatio(
                    speaker_ivector_dbl, r_counts(i), utterance_ivector_dbl);
              }
              int32 index;
              BaseFloat score = scores.Max(&index);
              return std::make_pair(index, score);
            ...
            py::arg("speaker_counts"))
        .def(
            "generate_affinity_matrix",
            [](PyClass &plda, const std::vector<Vector<double>> &ivectors) {
              py::gil_scoped_release gil_release;
              Matrix<float> scores;
              ...
              for (int32 i = 0; i < ivectors.size(); i++) {
                for (int32 j = 0; j < ivectors.size(); j++) {
                  scores(i, j) =
                      plda.LogLikelihoodRatio(ivectors[i], 1, ivectors[j]);
                }
              }
              return scores;
            },
        ...
        .def(
            "transform_ivectors",
            [](PyClass &plda, py::array_t<double> ivectors,
              ...
              std::vector<const Vector<double> *> transformed_ivectors(
                  r_counts.shape(0));
              ...
              for (int32 i = 0; i < r_counts.shape(0); i++) {
                ...
                for (py::size_t j = 0; j < r.shape(1); j++)
                  ivector(j) = r(i, j);
                plda.TransformIvector(plda_config, ivector, r_counts(i),
                                      transformed_ivector);
                transformed_ivectors[i] = transformed_ivector;
              }
              return transformed_ivectors;
            },
            py::arg("ivectors"), py::arg("num_examples"))
        .def(
            "transform_ivectors",
            [](PyClass &plda, const std::vector<Vector<double>> &ivectors,
              ...
              for (int32 i = 0; i < ivectors.size(); i++) {
                Vector<double> *transformed_ivector =
                    new Vector<double>(plda.Dim());
                plda.TransformIvector(plda_config, ivectors[i], num_examples[i],
                                      transformed_ivector);
                transformed_ivectors[i] = transformed_ivector;
              }
              return transformed_ivectors;
        ...
        .def(
            "score",
            [](PyClass &plda, const VectorBase<double> &utterance_ivector,
              ...
              for (int32 j = 0; j < transformed_enrolled_ivectors.size(); j++) {
                scores.push_back(plda.LogLikelihoodRatio(
                    utterance_ivector, num_enroll_utts[j],
        ...
        .def(
            "score",
            [](PyClass &plda, const VectorBase<float> &utterance_ivector,
               const std::vector<Vector<float>>
                   &transformed_enrolled_ivectors) {
              py::gil_scoped_release gil_release;
              PldaConfig plda_config;
              ...
              for (int32 j = 0; j < transformed_enrolled_ivectors.size(); j++) {
                Vector<double> ivector_two_dbl(
                    transformed_enrolled_ivectors[j]);
                scores.push_back(plda.LogLikelihoodRatio(ivector_one_dbl, 1,
                                                         ivector_two_dbl));
              }
              return scores;
            },
        ...
        .def(
            ...
            [](PyClass &plda, py::array_t<double> utterance_one_ivector,
              ...
              BaseFloat score = 1.0 / Exp(plda.LogLikelihoodRatio(
                                          ivector_one_dbl, 1, ivector_two_dbl));
              return score;
            },
            py::arg("utterance_one_ivector"), py::arg("utterance_two_ivector"))
        .def(
            "log_likelihood_distance_vectorized",
            [](PyClass &plda, py::array_t<double> utterance_one_ivector,
               py::array_t<double> utterance_two_ivector) {
              py::gil_scoped_release gil_release;
              py::buffer_info buf1 = utterance_one_ivector.request(),
                              buf2 = utterance_two_ivector.request();
              ...
              for (py::size_t i = 0; i < r_one.shape(0); i++) {
                ...
                ptr3[i] = 1.0 / Exp(plda.LogLikelihoodRatio(ivector_one_dbl, 1,
                                                            ivector_two_dbl));
                ;
              }
              return result;
        ...
        .def("TransformIvector",
             py::overload_cast<const PldaConfig &, const VectorBase<double> &,
                               int32, VectorBase<double> *>(
                 &PyClass::TransformIvector, py::const_),
             "Transforms an iVector into a space where the within-class "
             "variance "
             ...
             "\n"
             "If config.normalize_length == true, it will also normalize the "
             "iVector's "
             "length by multiplying by a scalar that ensures that ivector^T "
             "inv_var "
             "ivector = dim.  In this case, \"num_enroll_examples\" comes into "
             "play because it "
             "affects the expected covariance matrix of the iVector.  The "
             "normalization "
             "factor is returned, even if config.normalize_length == false, in "
             "which "
             "case the normalization factor is computed but not applied. "
             "If config.simple_length_normalization == true, then an "
             "alternative "
             "normalization factor is computed that causes the iVector length "
             "to be equal to the square root of the iVector dimension.",
             py::arg("config"), py::arg("ivector"),
             py::arg("num_enroll_examples"), py::arg("transformed_ivector"))
        .def("TransformIvector",
             py::overload_cast<const PldaConfig &, const VectorBase<float> &,
                               int32, VectorBase<float> *>(
                 &PyClass::TransformIvector, py::const_),
             "Transforms an iVector into a space where the within-class "
             "variance "
             "is unit and between-class variance is diagonalized.  The only "
             ...
             py::arg("config"), py::arg("ivector"),
             py::arg("num_enroll_examples"), py::arg("transformed_ivector"))
        .def("LogLikelihoodRatio", &PyClass::LogLikelihoodRatio,
             "Returns the log-likelihood ratio "
             "log (p(test_ivector | same) / p(test_ivector | different)). "
             "transformed_enroll_ivector is an average over utterances for "
             "that speaker.  Both transformed_enroll_vector and "
             "transformed_test_ivector "
             "are assumed to have been transformed by the function "
             "TransformIvector(). "
             "Note: any length normalization will have been done while "
             "computing "
             "the transformed iVectors.",
             py::arg("transformed_enroll_ivector"), py::arg("num_enroll_utts"),
        ...
        .def(
            ...
            [](
              ...
              return plda.LogLikelihoodRatio(ivector_one_dbl, num_enroll_utts,
                                             ivector_two_dbl);
            },
            "Returns the log-likelihood ratio "
            "log (p(test_ivector | same) / p(test_ivector | different)). "
        ...
        .def(
            ...
            [](
                ...
                PyClass &plda, py::array_t<double> &transformed_enroll_ivector,
                int32 num_enroll_utts,
        ...
        .def("SmoothWithinClassCovariance",
             ...
             "This function smooths the within-class covariance by adding to "
             "it, "
             "smoothing_factor (e.g. 0.1) times the between-class covariance "
             "(it's "
             "implemented by modifying transform_).  This is to compensate for "
             "situations where there were too few utterances per speaker get a "
             "good "
             "estimate of the within-class covariance, and where the leading "
             "elements of "
             "psi_ were as a result very large.",
             py::arg("smoothing_factor"))
        .def("ApplyTransform", &PyClass::ApplyTransform,
             "Apply a transform to the PLDA model.  This is mostly used for "
             "projecting the parameters of the model into a lower dimensional "
             "space, "
             "i.e. in_transform.NumRows() <= in_transform.NumCols(), typically "
             "for "
  ...
  }
  {
    ...
    auto plda_stats = py::class_<PyClass>(m, "PldaStats");
    plda_stats.def(py::init<>())
        .def("AddSamples", &PyClass::AddSamples,
             ...
             py::arg("weight"), py::arg("group"))
        .def("Dim", &PyClass::Dim)
        .def("Init", &PyClass::Init, py::arg("dim"))
        .def("Sort", &PyClass::Sort)
        .def("IsSorted", &PyClass::IsSorted);
  }
  {
    using PyClass = PldaEstimationConfig;

    auto plda_estimation_config =
        py::class_<PyClass>(m, "PldaEstimationConfig");
    plda_estimation_config.def(py::init<>())
        .def_readwrite("num_em_iters", &PyClass::num_em_iters)
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.num_em_iters);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 1) throw std::runtime_error("Invalid state!");
  ...
  }
  {
    ...
    auto plda_estimator = py::class_<PyClass>(m, "PldaEstimator");
    plda_estimator.def(py::init<const PldaStats &>(), py::arg("stats"))
        .def("Estimate", &PyClass::Estimate, py::arg("config"),
             py::arg("output"))
        .def(
            "estimate",
            ...
            py::arg("config"), py::return_value_policy::reference);
  }
  {
    using PyClass = PldaUnsupervisedAdaptorConfig;

    auto plda_unsupervised_adaptor_config =
        py::class_<PyClass>(m, "PldaUnsupervisedAdaptorConfig");
    plda_unsupervised_adaptor_config.def(py::init<>())
        ...
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(p.mean_diff_scale, p.within_covar_scale,
  ...
  }
  {
    ...
    auto plda_unsupervised_adaptor =
        py::class_<PyClass>(m, "PldaUnsupervisedAdaptor");
    plda_unsupervised_adaptor.def(py::init<>())
        .def("AddStats",
             py::overload_cast<double, const Vector<double> &>(
                 &PyClass::AddStats),
             "Add stats to this class.  Normally the weight will be 1.0.",
             py::arg("weight"), py::arg("ivector"))
        .def("AddStats",
             py::overload_cast<double, const Vector<float> &>(
                 &PyClass::AddStats),
             "Add stats to this class.  Normally the weight will be 1.0.",
             py::arg("weight"), py::arg("ivector"))
        .def("UpdatePlda", &PyClass::UpdatePlda,
             "Add stats to this class.  Normally the weight will be 1.0.",
             py::arg("config"), py::arg("plda"));
  }
}
...
void pybind_voice_activity_detection(py::module &m) {
  {
    ...
    auto vad_energy_options = py::class_<PyClass>(m, "VadEnergyOptions");
    vad_energy_options.def(py::init<>())
        ...
        .def_readwrite("vad_proportion_threshold",
                       &PyClass::vad_proportion_threshold)
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
              return py::make_tuple(
                  p.vad_energy_threshold, p.vad_energy_mean_scale,
                  p.vad_frames_context, p.vad_proportion_threshold);
            },
            [](py::tuple t) {  // __setstate__
              if (t.size() != 4) throw std::runtime_error("Invalid state!");
  ...
  m.def(
      ...
      py::arg("opts"), py::arg("input_features"));
}
...
void init_ivector(py::module &_m) {
  ...
  m.def(
      "ivector_extract",
      [](const DiagGmm &gmm, const IvectorExtractor &extractor,
         const Matrix<BaseFloat> &feats,

         double acoustic_weight = 1.0, double max_count = 0.0,
         int32 num_post = 50, BaseFloat min_post = 0.0) -> Vector<double> {
        ...
        for (int32 t = 0; t < T; t++) {
          log_like_this_file += VectorToPosteriorEntry(
              loglikes.Row(t), num_post, min_post, &(posterior[t]));
        }
        ...
        if (opts.max_count > 0 && this_t > opts.max_count) {
          ...
          KALDI_LOG << "Scaling stats for utterance by scale "
                    << max_count_scale
                    << " due to --max-count=" << opts.max_count;
        ...
        IvectorExtractorUtteranceStats utt_stats(
            extractor.NumGauss(), extractor.FeatDim(), need_2nd_order_stats);
      ...
      py::arg("gmm"), py::arg("extractor"), py::arg("feats"),
      py::arg("acoustic_weight") = 1.0, py::arg("max_count") = 0.0,
      py::arg("num_post") = 50, py::arg("min_post") = 0.0);

  m.def(
      "ivector_normalize_length",
      [](Vector<BaseFloat> *ivector, bool normalize = true,
         bool scaleup = true) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("ivector"), py::arg("normalize") = true,
      py::arg("scaleup") = true);

  m.def(
      "ivector_normalize_length",
      [](Vector<double> *ivector, bool normalize = true, bool scaleup = true) {
        py::gil_scoped_release gil_release;
  ...
  m.def(
      ...
      [](std::vector<Vector<float> *> &ivectors, bool normalize = true) {
        py::gil_scoped_release gil_release;
        Vector<double> sum;
      ...
      py::arg("ivectors"), py::arg("normalize") = true);

  m.def(
      "ivector_subtract_mean",
      [](std::vector<Vector<double> *> &ivectors, bool normalize = true) {
        py::gil_scoped_release gil_release;
        Vector<double> sum;
