...
void pybind_am_diag_gmm(py::module &m) {
  {
    ...
    auto am_diag_gmm = py::class_<PyClass>(m, "AmDiagGmm");
    am_diag_gmm.def(py::init<>())
        .def("Init", &PyClass::Init,
             "Initializes with a single \"prototype\" GMM.", py::arg("proto"),
             py::arg("num_pdfs"))
        .def(
            "AddPdf", &PyClass::AddPdf,
            "Adds a GMM to the model, and increments the total number of PDFs.",
            py::arg("gmm"))
        .def("CopyFromAmDiagGmm", &PyClass::CopyFromAmDiagGmm,
             "Copies the parameters from another model. Allocates necessary "
             "memory.",
             py::arg("other"))
        .def("SplitPdf", &PyClass::SplitPdf, py::arg("idx"),
             py::arg("target_components"), py::arg("perturb_factor"))
        .def(
            "transform_means",
            [](PyClass &am_gmm, const Matrix<BaseFloat> &mat) {
              ...
              for (int32 i = 0; i < am_gmm.NumPdfs(); i++) {
                ...
                if (mat.NumCols() == dim) {  // linear case
                  // Right-multiply means by mat^T (equivalent to
                  // left-multiplying each row by mat).
                  new_means.AddMatMat(1.0, means, kNoTrans, mat, kTrans, 0.0);
                } else {  // affine case
                  Matrix<BaseFloat> means_ext(means.NumRows(),
                                              means.NumCols() + 1);
                  ...
                  new_means.AddMatMat(1.0, means_ext, kNoTrans, mat, kTrans,
                                      0.0);
                }
            ...
            py::arg("mat"))
        .def(
            "SplitByCount", &PyClass::SplitByCount,
            "In SplitByCount we use the \"target_components\" and \"power\" "
            "to work out targets for each state (according to "
            "power-of-occupancy rule), "
            ...
            py::arg("state_occs"), py::arg("target_components"),
            py::arg("perturb_factor"), py::arg("power"), py::arg("min_count"))
        .def("MergeByCount", &PyClass::MergeByCount,
             "In MergeByCount we use the \"target_components\" and \"power\" "
             "to work out targets for each state (according to "
             "power-of-occupancy rule), "
             "and any state over its target gets mixed down.  If some states "
             "were under their target, this may take the #Gauss below the "
             "target.",
             py::arg("state_occs"), py::arg("target_components"),
             py::arg("power"), py::arg("min_count"))
        .def("ComputeGconsts", &PyClass::ComputeGconsts,
             "Sets the gconsts for all the PDFs. Returns the total number of "
             "Gaussians "
             "over all PDFs that are \"invalid\" e.g. due to zero weights or "
             "variances.")
        .def("LogLikelihood", &PyClass::LogLikelihood, py::arg("pdf_index"),
             py::arg("data"))
        ...
        .def("GetPdf",
             (const DiagGmm &(PyClass::*)(int32 pdf_index) const) &
                 PyClass::GetPdf,
             py::arg("pdf_index"))
        .def("GetPdf",
             (DiagGmm & (PyClass::*)(int32 pdf_index)) & PyClass::GetPdf,
             py::arg("pdf_index"))
        .def("GetGaussianMean", &PyClass::GetGaussianMean, py::arg("pdf_index"),
             py::arg("gauss"), py::arg("out"))
        .def("GetGaussianVariance", &PyClass::GetGaussianVariance,
             py::arg("pdf_index"), py::arg("gauss"), py::arg("out"))
        .def("SetGaussianMean", &PyClass::SetGaussianMean, py::arg("pdf_index"),
        ...
        .def(
            ...
            py::arg("trans_model"), py::arg("silence_phones"), py::arg("boost"))
        .def(
            "mle_update",
            [](PyClass &am_gmm, const AccumAmDiagGmm &gmm_accs, int32 mixup = 0,
               int32 mixdown = 0, BaseFloat perturb_factor = 0.01,
               BaseFloat power = 0.2, BaseFloat min_count = 20.0,
              ...
              if (mixup != 0 || mixdown != 0) {
                ...
                if (mixup != 0)
                  am_gmm.SplitByCount(pdf_occs, mixup, perturb_factor, power,
                                      min_count);
              }
              return py::make_tuple(objf_impr, count);
            },
            py::arg("gmm_accs"), py::arg("mixup") = 0, py::arg("mixdown") = 0,
            py::arg("perturb_factor") = 0.01, py::arg("power") = 0.2,
  ...
  }
  {
    ...
    auto ubm_clustering_options =
        py::class_<PyClass>(m, "UbmClusteringOptions");
    ubm_clustering_options.def(py::init<>())
        .def(py::init<int32, BaseFloat, int32, BaseFloat, int32>(),
             py::arg("ncomp"), py::arg("red"), py::arg("interm_gauss"),
             py::arg("vfloor"), py::arg("max_am_gauss"))
        .def_readwrite("ubm_num_gauss", &PyClass::ubm_num_gauss)
        .def_readwrite("reduce_state_factor", &PyClass::reduce_state_factor)
        .def_readwrite("intermediate_num_gauss",
                       &PyClass::intermediate_num_gauss)
  ...
  m.def("ClusterGaussiansToUbm", &ClusterGaussiansToUbm,
        "Clusters the Gaussians in an acoustic model to a single GMM with "
        "specified "
        "number of components. First the each state is mixed-down to a single "
        "Gaussian, then the states are clustered by clustering these Gaussians "
        "in a "
        "bottom-up fashion. Number of clusters is determined by "
        "reduce_state_factor. "
        "The Gaussians for each cluster of states are then merged based on the "
        "least "
        "likelihood reduction till there are intermediate_numcomp Gaussians, "
        "which "
        "are then merged into ubm_num_gauss Gaussians. "
        "This is the UBM initialization algorithm described in section 2.1 of "
        "Povey, "
        "et al., \"The subspace Gaussian mixture model - A structured model "
        "for speech "
        "recognition\", In Computer Speech and Language, April 2011.",
        py::arg("am"), py::arg("state_occs"), py::arg("opts"),
        py::arg("ubm_out"));
}
...
void pybind_decodable_am_diag_gmm(py::module &m) {
  {
    ...
    auto decodable_am_diag_gmm_unmapped =
        py::class_<DecodableAmDiagGmmUnmapped, DecodableInterface>(
    ...
    decodable_am_diag_gmm_unmapped
        .def(
            py::init<const AmDiagGmm &, const Matrix<BaseFloat> &, BaseFloat>(),
            "If you set log_sum_exp_prune to a value greater than 0 it will "
            "prune "
            "in the LogSumExp operation (larger = more exact); I suggest 5. "
            "This is advisable if it's spending a long time doing exp  "
            "operations.",
            py::arg("am"), py::arg("feats"), py::arg("log_sum_exp_prune"))
        .def(
            "LogLikelihood", &PyClass::LogLikelihood,
            "Note, frames are numbered from zero.  But state_index is numbered "
            "from one (this routine is called by FSTs).",
            py::arg("frame"), py::arg("state_index"))
  ...
  }
  {
    ...
    auto decodable_am_diag_gmm =
        py::class_<DecodableAmDiagGmm, DecodableAmDiagGmmUnmapped>(
            m, "DecodableAmDiagGmm");
    decodable_am_diag_gmm
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const Matrix<BaseFloat> &, BaseFloat>(),
             py::arg("am"), py::arg("tm"), py::arg("feats"),
             py::arg("log_sum_exp_prune"))
        .def("LogLikelihood", &PyClass::LogLikelihood,
             "Note, frames are numbered from zero.", py::arg("frame"),
             py::arg("tid"))
        .def("NumIndices", &PyClass::NumIndices,
             "Indices are one-based!  This is for compatibility with OpenFst.")
  ...
  }
  {
    ...
    auto decodable_am_diag_gmm_scaled =
        py::class_<DecodableAmDiagGmmScaled, DecodableAmDiagGmmUnmapped>(
            m, "DecodableAmDiagGmmScaled");
    decodable_am_diag_gmm_scaled
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const Matrix<BaseFloat> &, BaseFloat, BaseFloat>(),
             py::arg("am"), py::arg("tm"), py::arg("feats"), py::arg("scale"),
             py::arg("log_sum_exp_prune") = -1.0)
        .def(py::init<const AmDiagGmm &, const TransitionModel &, BaseFloat,
                      BaseFloat, Matrix<BaseFloat> *>(),
             py::arg("am"), py::arg("tm"), py::arg("scale"),
             py::arg("log_sum_exp_prune"), py::arg("feats"))
        .def("LogLikelihood", &PyClass::LogLikelihood,
             "Note, frames are numbered from zero but transition-ids from one.",
             py::arg("frame"), py::arg("tid"))
        .def("NumIndices", &PyClass::NumIndices,
             "Indices are one-based!  This is for compatibility with OpenFst.")
        .def("TransModel", &PyClass::TransModel);
  }
...
void pybind_diag_gmm_normal(py::module &m) {
  {
    ...
    auto diag_gmm_normal = py::class_<PyClass>(m, "DiagGmmNormal");
    diag_gmm_normal.def(py::init<>())
        .def(py::init<const DiagGmm &>(), py::arg("gmm"))
        .def("Resize", &PyClass::Resize,
             "Resizes arrays to this dim. Does not initialize data.",
             py::arg("nMix"), py::arg("dim"))
        .def("CopyFromDiagGmm", &PyClass::CopyFromDiagGmm,
             "Copies from given DiagGmm", py::arg("diaggmm"))
        .def("CopyToDiagGmm", &PyClass::CopyToDiagGmm,
             "Copies to DiagGmm the requested parameters", py::arg("diaggmm"),
             py::arg("flags"))
        .def("NumGauss", &PyClass::NumGauss)
        .def("Dim", &PyClass::Dim);
  }
...
void pybind_diag_gmm(py::module &m) {
  {
    ...
    auto diag_gmm = py::class_<PyClass>(m, "DiagGmm");
    diag_gmm.def(py::init<>())
        ...
        .def(py::init<const GaussClusterable &, BaseFloat>(),
             ...
             py::arg("gc"), py::arg("var_floor"))
        .def(py::init<int32, int32>(), py::arg("nMix"), py::arg("dim"))
        .def(py::init<
                 const std::vector<std::pair<BaseFloat, const DiagGmm *>> &>(),
             "Constructor that allows us to merge GMMs with weights.  Weights "
             "must sum "
             "to one, or this GMM will not be properly normalized (we don't "
             "check this). "
             "Weights must be positive (we check this).",
             py::arg("gmms"))
        .def("CopyFromNormal", &PyClass::CopyFromNormal,
        ...
        .def("CopyFromDiagGmm", &PyClass::CopyFromDiagGmm,
             "Copies from given DiagGmm", py::arg("diaggmm"))
        .def("CopyFromFullGmm", &PyClass::CopyFromFullGmm,
             "Copies from given FullGmm", py::arg("fullgmm"))
        .def("NumGauss", &PyClass::NumGauss,
        ...
        .def("LogLikelihoods",
             py::overload_cast<const VectorBase<BaseFloat> &,
                               Vector<BaseFloat> *>(&PyClass::LogLikelihoods,
                                                    py::const_),
             "Outputs the per-component log-likelihoods", py::arg("data"),
             py::arg("loglikes"))
        .def(
            "mle_update",
            [](PyClass &gmm, const AccumDiagGmm &gmm_accs, int32 mixup = 0,
              ...
              {  // Update GMMs.
                ...
                MleDiagGmmUpdate(gmm_opts, gmm_accs,
                                 StringToGmmFlags(update_flags_str), &gmm,
                                 &objf_impr, &count);
              }

              if (mixup != 0) gmm.Split(mixup, perturb_factor);
            },
            py::arg("gmm_accs"), py::arg("mixup") = 0,
        ...
        .def(
            "generate_post",
            [](PyClass &gmm, const MatrixBase<BaseFloat> &feats,
               int32 num_post = 50, BaseFloat min_post = 0.0) {
              ...
              for (int32 t = 0; t < T; t++) {
                log_like_this_file += VectorToPosteriorEntry(
                    loglikes.Row(t), num_post, min_post, &(post[t]));
              }
              py::gil_scoped_acquire acquire;
              return py::make_tuple(post, log_like_this_file);
            },
            py::arg("feats"), py::arg("num_post") = 50,
            py::arg("min_post") = 0.0)
        .def("LogLikelihoods",
             py::overload_cast<const MatrixBase<BaseFloat> &,
                               Matrix<BaseFloat> *>(&PyClass::LogLikelihoods,
                                                    py::const_),
             "Outputs the per-component log-likelihoods", py::arg("data"),
             py::arg("loglikes"))
        .def(
            "LogLikelihoodsPreselect", &PyClass::LogLikelihoodsPreselect,
            "Outputs the per-component log-likelihoods of a subset of mixture "
            "components.  Note: at output, loglikes->Dim() will equal "
            "indices.size(). "
            "loglikes[i] will correspond to the log-likelihood of the Gaussian "
            "indexed indices[i], including the mixture weight.",
            py::arg("data"), py::arg("indices"), py::arg("loglikes"))
        .def("GaussianSelection",
             py::overload_cast<const VectorBase<BaseFloat> &, int32,
                               std::vector<int32> *>(
                 &PyClass::GaussianSelection, py::const_),
             "Get gaussian selection information for one frame.  Returns "
             "log-like "
             "this frame.  Output is the best \"num_gselect\" indices, sorted "
             "from best to "
             "worst likelihood.  If \"num_gselect\" > NumGauss(), sets it to "
             "NumGauss().",
             py::arg("data"), py::arg("num_gselect"), py::arg("output"))
        .def(
            "gaussian_selection",
            [](PyClass &gmm, const MatrixBase<BaseFloat> &data,
               int32 num_gselect) {
              py::gil_scoped_release release;
              std::vector<std::vector<int32>> gselect(data.NumRows());
              double tot_like_this_file =
                  gmm.GaussianSelection(data, num_gselect, &gselect);
              py::gil_scoped_acquire acquire;
        ...
        .def("GaussianSelection",
             py::overload_cast<const MatrixBase<BaseFloat> &, int32,
             ...
             py::arg("data"), py::arg("num_gselect"), py::arg("output"))
        .def(
            "GaussianSelectionPreselect", &PyClass::GaussianSelectionPreselect,
            "Get gaussian selection information for one frame.  Returns "
            "log-like for "
            "this frame.  Output is the best \"num_gselect\" indices that were "
            "preselected, sorted from best to worst likelihood.  If "
            "\"num_gselect\" > "
            "NumGauss(), sets it to NumGauss().",
            py::arg("data"), py::arg("preselect"), py::arg("num_gselect"),
            py::arg("output"))
        .def(
            "ComponentPosteriors", &PyClass::ComponentPosteriors,
            ...
            py::arg("data"), py::arg("posteriors"))
        .def("ComponentLogLikelihood", &PyClass::ComponentLogLikelihood,
             "Computes the log-likelihood of a data point given a single "
             "Gaussian "
             "component. NOTE: Currently we make no guarantees about what "
             "happens if "
             "one of the variances is zero.",
             py::arg("data"), py::arg("comp_id"))
        .def("ComputeGconsts", &PyClass::ComputeGconsts,
             "Sets the gconsts.  Returns the number that are \"invalid\" e.g. "
             "because of "
             "zero weights or variances.")
        .def("Generate", &PyClass::Generate,
             "Generates a random data-point from this distribution.",
             py::arg("output"))
        .def("Split", &PyClass::Split,
             "Split the components and remember the order in which the "
             "components were split",
             py::arg("target_components"), py::arg("perturb_factor"),
             py::arg("history") = NULL)
        .def("Perturb", &PyClass::Perturb,
             "Perturbs the component means with a random vector multiplied by "
             "the "
             "pertrub factor.",
             py::arg("perturb_factor"))
        .def("Merge", &PyClass::Merge,
             "Merge the components and remember the order in which the "
             "components were "
             "merged (flat list of pairs)",
             py::arg("target_components"), py::arg("history") = NULL)
        .def("MergeKmeans", &PyClass::MergeKmeans,
        ...
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Interpolate",
             py::overload_cast<BaseFloat, const DiagGmm &, GmmFlagsType>(
                 &PyClass::Interpolate),
             "this = rho x source + (1-rho) x this", py::arg("rho"),
             py::arg("source"), py::arg("flags") = kGmmAll)
        .def("Interpolate",
             py::overload_cast<BaseFloat, const FullGmm &, GmmFlagsType>(
                 &PyClass::Interpolate),
             "this = rho x source + (1-rho) x this", py::arg("rho"),
             py::arg("source"), py::arg("flags") = kGmmAll)
        .def("gconsts", &PyClass::gconsts)
        .def("weights", &PyClass::weights)
        .def("means_invvars", &PyClass::means_invvars)
        .def("inv_vars", &PyClass::inv_vars)
        .def("valid_gconsts", &PyClass::valid_gconsts)
        .def("RemoveComponent", &PyClass::RemoveComponent,
             "Removes single component from model", py::arg("gauss"),
             py::arg("renorm_weights"))
        .def("RemoveComponents", &PyClass::RemoveComponents,
             "Removes multiple components from model; \"gauss\" must not have "
             "dups.",
             py::arg("gauss"), py::arg("renorm_weights"))
        .def("SetWeights", &PyClass::SetWeights<float>, py::arg("w"))
        .def("SetMeans", &PyClass::SetMeans<float>,
             "Use SetMeans to update only the Gaussian means (and not "
             "variances)",
             py::arg("m"))
        .def("SetInvVarsAndMeans", &PyClass::SetInvVarsAndMeans<float>,
             "Use SetInvVarsAndMeans if updating both means and (inverse) "
             "variances",
             py::arg("invvars"), py::arg("means"))
        .def("SetInvVars", &PyClass::SetInvVars<float>,
             "Set the (inverse) variances and recompute means_invvars_",
             py::arg("v"))
        .def("GetVars", &PyClass::GetVars<float>, "Accessor for covariances.",
             py::arg("v"))
        .def("GetMeans", &PyClass::GetMeans<float>, "Accessor for means.",
             py::arg("m"))
        .def("SetComponentMean", &PyClass::SetComponentMean<float>,
             "Set mean for a single component - internally multiplies with "
             "inv(var)",
             py::arg("gauss"), py::arg("in"))
        .def("SetComponentInvVar", &PyClass::SetComponentInvVar<float>,
             "Set inv-var for single component (recommend to do this before "
             "setting the mean, if doing both, for numerical reasons).",
             py::arg("gauss"), py::arg("in"))
        .def("SetComponentWeight", &PyClass::SetComponentWeight,
             "Set weight for single component.", py::arg("gauss"),
             py::arg("weight"))
        .def("GetComponentMean", &PyClass::GetComponentMean<float>,
             "Accessor for single component mean", py::arg("gauss"),
             py::arg("out"))
        .def("GetComponentVariance", &PyClass::GetComponentVariance<float>,
             "Accessor for single component variance", py::arg("gauss"),
             py::arg("out"))
        .def(
            "init_from_random_frames",
            [](PyClass *gmm, const Matrix<BaseFloat> &feats) {
              int32 num_gauss = gmm->NumGauss(), num_frames = feats.NumRows(),
                    dim = feats.NumCols();
              KALDI_ASSERT(num_frames >= 10 * num_gauss &&
            ...
            "We initialize the GMM parameters by setting the variance to the "
            "global "
            "variance of the features, and the means to distinct randomly "
            "chosen frames.",
            py::arg("feats"))
        .def(
            "train_one_iter",
            [](PyClass *gmm, const Matrix<BaseFloat> &feats,
              ...
              double tot_like;
              tot_like = gmm_acc.AccumulateFromDiagMultiThreaded(
                  *gmm, feats, frame_weights, num_threads);
              ...
              MleDiagGmmUpdate(gmm_opts, gmm_acc, kGmmAll, gmm, &objf_change,
                               &count);

              KALDI_LOG << "Objective-function change on iteration " << iter
                        << " was " << (objf_change / count) << " over " << count
                        << " frames.";
            },
            "We initialize the GMM parameters by setting the variance to the "
            "global "
            "variance of the features, and the means to distinct randomly "
            "chosen frames.",
            py::arg("feats"), py::arg("gmm_opts"), py::arg("iter"),
            py::arg("num_threads"));
  }
}
...
void pybind_ebw_diag_gmm(py::module &m) {
  {
    ...
    auto ebw_options = py::class_<PyClass>(m, "EbwOptions");
  ...
  }
  {
    ...
    auto ebw_weight_options = py::class_<PyClass>(m, "EbwWeightOptions");
  ...
  m.def("UpdateEbwDiagGmm", &UpdateEbwDiagGmm,
        "Update Gaussian parameters only (no weights) "
        "The pointer parameters auxf_change_out etc. are incremented, not set.",
        py::arg("num_stats"), py::arg("den_stats"), py::arg("flags"),
        py::arg("opts"), py::arg("gmm"), py::arg("auxf_change_out"),
        py::arg("count_out"), py::arg("num_floored_out"));
  m.def("UpdateEbwAmDiagGmm", &UpdateEbwAmDiagGmm, py::arg("num_stats"),
        py::arg("den_stats"), py::arg("flags"), py::arg("opts"),
        py::arg("am_gmm"), py::arg("auxf_change_out"), py::arg("count_out"),
        py::arg("num_floored_out"));
  m.def("UpdateEbwWeightsDiagGmm", &UpdateEbwWeightsDiagGmm,
        "Updates the weights using the EBW-like method described in Dan "
        "Povey's thesis "
        "(this method has no tunable parameters). "
        "The pointer parameters auxf_change_out etc. are incremented, not set.",
        py::arg("num_stats"), py::arg("den_stats"), py::arg("opts"),
        py::arg("gmm"), py::arg("auxf_change_out"), py::arg("count_out"));
  m.def("UpdateEbwWeightsAmDiagGmm", &UpdateEbwWeightsAmDiagGmm,
        py::arg("num_stats"), py::arg("den_stats"), py::arg("opts"),
        py::arg("am_gmm"), py::arg("auxf_change_out"), py::arg("count_out"));
  m.def("IsmoothStatsDiagGmm", &IsmoothStatsDiagGmm,
        "I-Smooth the stats.  src_stats and dst_stats do not have to be "
        "different.",
        py::arg("src_stats"), py::arg("tau"), py::arg("dst_stats"));
  m.def("DiagGmmToStats", &DiagGmmToStats,
        "Creates stats from the GMM.  Resizes them as needed.", py::arg("gmm"),
        py::arg("flags"), py::arg("state_occ"), py::arg("dst_stats"));
  m.def("IsmoothStatsAmDiagGmm", &IsmoothStatsAmDiagGmm,
        "Smooth \"dst_stats\" with \"src_stats\".  They don't have to be "
        "different.",
        py::arg("src_stats"), py::arg("tau"), py::arg("dst_stats"));
  m.def("IsmoothStatsAmDiagGmmFromModel", &IsmoothStatsAmDiagGmmFromModel,
        "This version of the I-smoothing function takes a model as input.",
        py::arg("src_model"), py::arg("tau"), py::arg("dst_stats"));
}
...
void pybind_full_gmm_normal(py::module &m) {
  {
    ...
    auto full_gmm_normal = py::class_<PyClass>(m, "FullGmmNormal");
    full_gmm_normal.def(py::init<>())
        .def(py::init<const FullGmm &>(), py::arg("gmm"))
        .def("Resize", &PyClass::Resize,
             "Resizes arrays to this dim. Does not initialize data.",
             py::arg("nMix"), py::arg("dim"))
        .def("CopyFromFullGmm", &PyClass::CopyFromFullGmm,
             "Copies from given FullGmm", py::arg("fullgmm"))
        .def("CopyToFullGmm", &PyClass::CopyToFullGmm, "Copies to FullGmm",
             py::arg("fullgmm"), py::arg("flags") = kGmmAll)
        .def("Rand", &PyClass::Rand,
             "Generates random features from the model.", py::arg("feats"));
  }
}
...
void pybind_full_gmm(py::module &m) {
  {
    ...
    auto full_gmm = py::class_<PyClass>(m, "FullGmm");
    full_gmm.def(py::init<>())
        ...
        .def(
            ...
            "Returns the log-likelihood of a data point (vector) given the GMM",
            py::arg("data"))
        .def("LogLikelihoods", &PyClass::LogLikelihoods,
             "Outputs the per-component contributions to the log-likelihood",
             py::arg("data"), py::arg("loglikes"))
        .def(
            "LogLikelihoodsPreselect", &PyClass::LogLikelihoodsPreselect,
            "Outputs the per-component log-likelihoods of a subset of mixture "
            "components. Note: indices.size() will equal loglikes->Dim() at "
            "output. "
            "loglikes[i] will correspond to the log-likelihood of the Gaussian "
            "indexed indices[i].",
            py::arg("data"), py::arg("indices"), py::arg("loglikes"))
        .def("GaussianSelection", &PyClass::GaussianSelection,
             "Get gaussian selection information for one frame.  Returns "
             "log-like for "
             "this frame.  Output is the best \"num_gselect\" indices, sorted "
             "from best to "
        ...
        .def(
            ...
            "given "
            "a data point. Returns the log-likehood of the data given the GMM.",
            py::arg("data"), py::arg("posterior"))
        .def("ComponentLogLikelihood", &PyClass::ComponentLogLikelihood,
             "Computes the contribution log-likelihood of a data point from a "
             "single "
             "Gaussian component. NOTE: Currently we make no guarantees about "
             "what "
        ...
        .def("Merge", &PyClass::Merge,
             ...
             "merged (flat list of pairs)",
             py::arg("target_components"), py::arg("history") = NULL)
        .def("MergePreselect", &PyClass::MergePreselect,
             "Merge the components and remember the order in which the "
             "components were "
             "merged (flat list of pairs); this version only considers merging "
             "pairs in \"preselect_pairs\" (or their descendants after "
             "merging). "
             "This is for efficiency, for large models.  Returns the delta "
             "likelihood.",
             py::arg("target_components"), py::arg("preselect_pairs"))
        .def("Write", &PyClass::Write, py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Interpolate", &PyClass::Interpolate,
             "this = rho x source + (1-rho) x this", py::arg("rho"),
             py::arg("source"), py::arg("flags") = kGmmAll)
        .def("gconsts", &PyClass::gconsts)
        .def("weights", &PyClass::weights)
        ...
        //  &PyClass::inv_covars)
        .def("SetWeights", &PyClass::SetWeights<float>, py::arg("w"))
        .def("SetWeights", &PyClass::SetWeights<double>, py::arg("w"))
        .def("SetMeans", &PyClass::SetMeans<float>,
             "Use SetMeans to update only the Gaussian means (and not "
             "variances)",
             py::arg("m"))
        .def("SetMeans", &PyClass::SetMeans<double>,
             "Use SetMeans to update only the Gaussian means (and not "
             "variances)",
             py::arg("m"))
        .def("SetInvCovarsAndMeans", &PyClass::SetInvCovarsAndMeans<float>,
             "Use SetInvCovarsAndMeans if updating both means and (inverse) "
             "variances",
             py::arg("invcovars"), py::arg("means"))
        .def("SetInvCovarsAndMeans", &PyClass::SetInvCovarsAndMeans<double>,
             "Use SetInvCovarsAndMeans if updating both means and (inverse) "
             "variances",
             py::arg("invcovars"), py::arg("means"))
        .def("SetInvCovarsAndMeansInvCovars",
             ...
             py::arg("invcovars"), py::arg("means_invcovars"))
        .def("SetInvCovarsAndMeansInvCovars",
             &PyClass::SetInvCovarsAndMeansInvCovars<double>,
             "Use this if setting both, in the class's native format.",
             py::arg("invcovars"), py::arg("means_invcovars"))
        .def("SetInvCovars", &PyClass::SetInvCovars<float>,
             "Set the (inverse) covariances and recompute means_invcovars_",
             py::arg("v"))
        .def("SetInvCovars", &PyClass::SetInvCovars<double>,
             "Set the (inverse) covariances and recompute means_invcovars_",
             py::arg("v"))
        .def("GetCovars", &PyClass::GetCovars<float>,
             "Accessor for covariances.", py::arg("v"))
        .def("GetCovars", &PyClass::GetCovars<double>,
             "Accessor for covariances.", py::arg("v"))
        .def("GetMeans", &PyClass::GetMeans<float>, "Accessor for means.",
             py::arg("m"))
        .def("GetMeans", &PyClass::GetMeans<double>, "Accessor for means.",
             py::arg("m"))
        .def("GetCovarsAndMeans", &PyClass::GetCovarsAndMeans<float>,
             "Accessor for covariances and means", py::arg("covars"),
             py::arg("m"))
        .def("GetCovarsAndMeans", &PyClass::GetCovarsAndMeans<double>,
             "Accessor for covariances and means", py::arg("covars"),
             py::arg("m"))
        .def("RemoveComponent", &PyClass::RemoveComponent,
             "Removes single component from model", py::arg("gauss"),
        ...
        .def("GetComponentMean", &PyClass::GetComponentMean<float>,
             "Accessor for single component mean", py::arg("gauss"),
             py::arg("out"))
        .def("GetComponentMean", &PyClass::GetComponentMean<double>,
             "Accessor for single component mean", py::arg("gauss"),
             py::arg("out"));
  }
}
...
void pybind_indirect_diff_diag_gmm(py::module &m) {
  m.def(
      "GetStatsDerivative", &GetStatsDerivative,
      "This gets the derivative of the (MMI or MPE) objective function w.r.t. "
      "the "
      "statistics for ML update, assuming we're doing an ML update-- as "
      "described in "
      "the original fMPE paper.  This is used in fMPE/fMMI, for the \"indirect "
      "differential\".  This derivative is represented as class AccumDiagGmm, "
      "as "
      "derivatives w.r.t. the x and x^2 stats directly (not w.r.t. the mean "
      "and "
      "variance). "
      "\n"
      "If the parameter \"rescaling\" is true, this function will assume that "
      "instead "
      "of the ML update, you will do a \"rescaling\" update as in the function "
      "DoRescalingUpdate(). "
      "\n"
      "CAUTION: for fMPE (as opposed to fMMI), to get the right answer, you "
      "would have "
      "to pre-scale the num and den accs by the acoustic scale (e.g. 0.1).",
      py::arg("gmm"), py::arg("num_accs"), py::arg("den_accs"),
      py::arg("ml_accs"), py::arg("min_variance"),
      py::arg("min_gaussian_occupancy"), py::arg("out_accs"));
  m.def(
      "DoRescalingUpdate", &DoRescalingUpdate,
      "This function \"DoRescalingUpdate\" updates the GMMs in a special way-- "
      "it "
      "first works out how the Gaussians differ from the old stats (in terms "
      "of an "
      "offset on the mean, a scale on the variance, and a factor on the "
      "weights), "
      "and it updates the model so that it will differ in the same way from "
      "the new "
      "stats. "
      "\n"
      "The idea here is that the original model may have been discriminatively "
      "trained, but we may have changed the features or the domain or "
      "something "
      "of that nature, and we want to update the model but preserve the "
      "discriminative "
      "training (viewed as an offset).",
      py::arg("old_ml_accs"), py::arg("new_ml_accs"), py::arg("min_variance"),
      py::arg("min_gaussian_occupancy"), py::arg("gmm"));
}
...
void pybind_mle_am_diag_gmm(py::module &m) {
  {
    ...
    auto accum_am_diag_gmm = py::class_<PyClass>(m, "AccumAmDiagGmm");
    accum_am_diag_gmm.def(py::init<>())
        .def("Read", &PyClass::Read, py::arg("in_stream"), py::arg("binary"),
             py::arg("add") = false, py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("out_stream"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Init",
             py::overload_cast<const AmDiagGmm &, GmmFlagsType>(&PyClass::Init),
             "Initializes accumulators for each GMM based on the number of "
             "components "
             "and dimension.",
             py::arg("model"), py::arg("flags"))
        .def(
            "init",
            [](PyClass &gmm_accs, const AmDiagGmm &am_gmm) {
        ...
        .def("AccumulateFromPosteriors", &PyClass::AccumulateFromPosteriors,
             "Accumulates stats for a single GMM in the model using "
             "pre-computed "
             "Gaussian posteriors.",
             py::arg("model"), py::arg("data"), py::arg("gmm_index"),
             py::arg("posteriors"), py::call_guard<py::gil_scoped_release>())
        .def("AccumulateForGaussian", &PyClass::AccumulateForGaussian,
             ...
             py::call_guard<py::gil_scoped_release>())
        .def("NumAccs", py::overload_cast<>(&PyClass::NumAccs))
        .def("TotStatsCount", &PyClass::TotStatsCount)
        .def("TotCount", &PyClass::TotCount)
        .def("TotLogLike", &PyClass::TotLogLike)
        .def("GetAcc", py::overload_cast<int32>(&PyClass::GetAcc),
             py::arg("index"))
        ...
        .def(
            ...
            py::arg("am_gmm"), py::arg("trans_model"), py::arg("alignment"),
            py::arg("mat"))
        .def(
            "acc_twofeats",
            [](PyClass &gmm_accs, const AmDiagGmm &am_gmm,
               const Posterior &pdf_posterior, const Matrix<BaseFloat> &mat1,
              ...
              for (size_t i = 0; i < pdf_posterior.size(); i++) {
                ...
                for (size_t j = 0; j < pdf_posterior[i].size(); j++) {
                  ...
                  tot_like_this_file +=
                      weight *
                      gmm_accs.AccumulateForGmmTwofeats(
                          am_gmm, mat1.Row(i), mat2.Row(i), pdf_id, weight);
                  tot_weight_this_file += weight;
                }
              }
              return tot_like_this_file;
            },
            py::arg("am_gmm"), py::arg("pdf_posterior"), py::arg("mat1"),
            py::arg("mat2"))
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
  ...
  m.def("MleAmDiagGmmUpdate", &MleAmDiagGmmUpdate,
        "for computing the maximum-likelihood estimates of the parameters of "
        "an acoustic model that uses diagonal Gaussian mixture models as "
        "emission densities.",
        py::arg("config"), py::arg("amdiaggmm_acc"), py::arg("flags"),
        py::arg("am_gmm"), py::arg("obj_change_out"), py::arg("count_out"),
        py::call_guard<py::gil_scoped_release>());
  m.def("MapAmDiagGmmUpdate", &MapAmDiagGmmUpdate,
        "Maximum A Posteriori update.", py::arg("config"),
        py::arg("diag_gmm_acc"), py::arg("flags"), py::arg("gmm"),
        py::arg("obj_change_out"), py::arg("count_out"),
        py::call_guard<py::gil_scoped_release>());
}
...
void pybind_mle_diag_gmm(py::module &m) {
  {
    ...
    auto mle_diag_gmm_options = py::class_<PyClass>(m, "MleDiagGmmOptions");
    mle_diag_gmm_options.def(py::init<>())
        .def_readwrite(
            "variance_floor_vector", &PyClass::variance_floor_vector,
            "Variance floor for each dimension [empty if not supplied]. "
            "It is in double since the variance is computed in double "
            "precision.")
        .def_readwrite(
            "min_gaussian_weight", &PyClass::min_gaussian_weight,
            "Minimum weight below which a Gaussian is not updated (and is "
            "removed, if remove_low_count_gaussians == true);")
        .def_readwrite(
            "min_gaussian_occupancy", &PyClass::min_gaussian_occupancy,
            "Minimum count below which a Gaussian is not updated (and is "
            "removed, if remove_low_count_gaussians == true).")
        .def_readwrite(
            "min_variance", &PyClass::min_variance,
  ...
  }
  {
    ...
    auto map_diag_gmm_options = py::class_<PyClass>(m, "MapDiagGmmOptions");
  ...
  }
  {
    ...
    auto accum_diag_gmm = py::class_<PyClass>(m, "AccumDiagGmm");
    accum_diag_gmm.def(py::init<>())
        .def(py::init<const DiagGmm &, GmmFlagsType>(), py::arg("gmm"),
             py::arg("flags"))
        .def(py::init<const AccumDiagGmm &>(), py::arg("other"))
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::arg("add") = false, py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        ...
        .def("AccumulateForComponent", &PyClass::AccumulateForComponent,
        ...
        .def("AccumulateFromDiagMultiThreaded",
             ...
             py::arg("gmm"), py::arg("data"), py::arg("frame_weights"),
             py::arg("num_threads"))
        .def("AddStatsForComponent", &PyClass::AddStatsForComponent,
             ...
             "by \"occ\"",
             py::arg("comp_id"), py::arg("occ"), py::arg("x_stats"),
             py::arg("x2_stats"))
        .def("Add", &PyClass::Add,
             "Increment with stats from this other accumulator (times scale)",
             py::arg("scale"), py::arg("acc"),
             py::call_guard<py::gil_scoped_release>())
        .def("SmoothStats", &PyClass::SmoothStats,
             "Smooths the accumulated counts by adding 'tau' extra frames. An "
             "example "
             "use for this is I-smoothing for MMIE.   Calls SmoothWithAccum.",
             py::arg("tau"), py::call_guard<py::gil_scoped_release>())
        .def("SmoothWithAccum", &PyClass::SmoothWithAccum,
             "Smooths the accumulated counts using some other accumulator. "
             "Performs a "
             "weighted sum of the current accumulator with the given one. An "
             "example use "
             "for this is I-smoothing for MMI and MPE. Both accumulators must "
             "have the "
             "same dimension and number of components.",
             py::arg("tau"), py::arg("src_acc"))
        .def(
            "SmoothWithModel", &PyClass::SmoothWithModel,
            "Smooths the accumulated counts using the parameters of a given "
            "model. "
            "An example use of this is MAP-adaptation. The model must have the "
            "same dimension and number of components as the current "
            "accumulator.",
            py::arg("tau"), py::arg("src_gmm"))
        .def("Flags", &PyClass::Flags)
        .def("occupancy", &PyClass::occupancy)
        .def("mean_accumulator", &PyClass::mean_accumulator)
        .def("variance_accumulator", &PyClass::variance_accumulator)
        .def("AssertEqual", &PyClass::AssertEqual, "used in testing.")
        .def(
            "accumulate_from_gselect",
            [](PyClass &gmm_accs, const DiagGmm &gmm,
              ...
              for (int32 i = 0; i < file_frames; i++) {
                ...
                for (int32 j = 0; j < loglikes.Dim(); j++)
                  gmm_accs.AccumulateForComponent(data, this_gselect[j],
                                                  loglikes(j));
              }
              return file_like;
            },
            py::arg("gmm"), py::arg("gselect"),
            py::arg("features"))
        .def(py::pickle(
            [](const PyClass &p) {  // __getstate__
              /* Return a tuple that fully encodes the state of the object */
  ...
  m.def("AugmentGmmFlags", &AugmentGmmFlags,
        "Returns \"augmented\" version of flags: e.g. if just updating means, "
        "need "
        "weights too.",
        py::arg("f"));
  m.def(
      "MleDiagGmmUpdate", &MleDiagGmmUpdate,
      "for computing the maximum-likelihood estimates of the parameters of "
      ...
      py::arg("gmm"), py::arg("obj_change_out"), py::arg("count_out"),
      py::arg("floored_elements_out") = NULL,
      py::arg("floored_gauss_out") = NULL, py::arg("removed_gauss_out") = NULL);
  m.def("MapDiagGmmUpdate", &MapDiagGmmUpdate,
        "Maximum A Posteriori estimation of the model.", py::arg("config"),
        py::arg("diag_gmm_acc"), py::arg("flags"), py::arg("gmm"),
        py::arg("obj_change_out"), py::arg("count_out"));
  m.def("MlObjective",
        py::overload_cast<const DiagGmm &, const AccumDiagGmm &>(&MlObjective),
        "Calc using the DiagGMM exponential form", py::arg("gmm"),
        py::arg("diaggmm_acc"));
}
...
void pybind_mle_full_gmm(py::module &m) {
  {
    ...
    auto mle_full_gmm_options = py::class_<PyClass>(m, "MleFullGmmOptions");
  ...
  }
  {
    ...
    auto accum_full_gmm = py::class_<PyClass>(m, "AccumFullGmm");
    accum_full_gmm.def(py::init<>())
        .def(py::init<int32, int32, GmmFlagsType>(), py::arg("num_comp"),
             py::arg("dim"), py::arg("flags"))
        .def(py::init<const FullGmm &, GmmFlagsType>(), py::arg("gmm"),
             py::arg("flags"))
        .def(py::init<const AccumFullGmm &>(), py::arg("other"))
        .def("Read", &PyClass::Read, py::arg("is"), py::arg("binary"),
             py::arg("add"), py::call_guard<py::gil_scoped_release>())
        .def("Write", &PyClass::Write, py::arg("os"), py::arg("binary"),
             py::call_guard<py::gil_scoped_release>())
        .def("Resize",
             py::overload_cast<int32, int32, GmmFlagsType>(&PyClass::Resize),
             "Allocates memory for accumulators", py::arg("num_components"),
             py::arg("dim"), py::arg("flags"))
        .def("Resize",
             py::overload_cast<const FullGmm &, GmmFlagsType>(&PyClass::Resize),
             "Calls ResizeAccumulators with arguments based on gmm",
             py::arg("gmm"), py::arg("flags"))
        .def("ResizeVarAccumulator", &PyClass::ResizeVarAccumulator,
             py::arg("num_comp"), py::arg("dim"))
        .def("NumGauss", &PyClass::NumGauss,
             "Returns the number of mixture components")
        .def("Dim", &PyClass::Dim,
             "Returns the dimensionality of the feature vectors")
        ...
        .def("AccumulateFromPosteriors", &PyClass::AccumulateFromPosteriors,
             "Accumulate for all components, given the posteriors.",
             py::arg("data"), py::arg("gauss_posteriors"))
        .def("AccumulateFromFull", &PyClass::AccumulateFromFull,
        ...
        .def("occupancy", &PyClass::occupancy)
        .def("mean_accumulator", &PyClass::mean_accumulator)
        .def("covariance_accumulator", &PyClass::covariance_accumulator);
  }
  m.def("MleFullGmmUpdate", &MleFullGmmUpdate,
        ...
        py::arg("config"), py::arg("fullgmm_acc"), py::arg("flags"),
        py::arg("gmm"), py::arg("obj_change_out"), py::arg("count_out"));
  m.def("MlObjective",
        py::overload_cast<const FullGmm &, const AccumFullGmm &>(&MlObjective),
        "Calc using the DiagGMM exponential form", py::arg("gmm"),
        py::arg("fullgmm_acc"));
}
...
void pybind_model_common(py::module &m) {
  ...
  m.def("AugmentGmmFlags", &AugmentGmmFlags,
        ...
        py::arg("flags"));
  m.def("StringToGmmFlags", &StringToGmmFlags, py::arg("string"));
  m.def("GmmFlagsToString", &GmmFlagsToString, py::arg("flags"));
  py::enum_<SgmmUpdateFlags>(m, "SgmmUpdateFlags")
      ...
      .value("kSgmmPhoneWeightProjections",
             SgmmUpdateFlags::kSgmmPhoneWeightProjections)
      ...
      .value("kSgmmSpeakerProjections",
             SgmmUpdateFlags::kSgmmSpeakerProjections)
      .value("kSgmmTransitions", SgmmUpdateFlags::kSgmmTransitions)
      .value("kSgmmSpeakerWeightProjections",
             SgmmUpdateFlags::kSgmmSpeakerWeightProjections)
  ...
  py::enum_<SgmmWriteFlags>(m, "SgmmWriteFlags")
      ...
      .export_values();
  m.def("GetSplitTargets", &GetSplitTargets,
        ...
        py::arg("state_occs"), py::arg("target_components"), py::arg("power"),
        py::arg("min_count"), py::arg("targets"));
}
...
void init_gmm(py::module &_m) {
  ...
  m.def(
      "gmm_post_to_gpost",
      [](const TransitionModel &trans_model, const AmDiagGmm &am_gmm,
         const Posterior &posterior, const Matrix<BaseFloat> &mat,
      ...
      py::arg("trans_model"), py::arg("am_gmm"), py::arg("posterior"),
      py::arg("mat"), py::arg("rand_prune") = 0.0);

  m.def(
      "gmm_rescore_lattice",
      [](CompactLattice &clat, const Matrix<BaseFloat> &feats,
         const AmDiagGmm &am_gmm, const TransitionModel &trans_model) {
        py::gil_scoped_release release;
      ...
      py::arg("clat"), py::arg("feats"), py::arg("am_gmm"),
      py::arg("trans_model"));
  ...
  m.def(
      ...
      py::arg("trans_model"), py::arg("am_gmm"), py::arg("decode_fst"),
      py::arg("features"), py::arg("acoustic_scale") = 1.0,
      py::arg("transition_scale") = 1.0, py::arg("self_loop_scale") = 1.0,
      py::arg("beam") = 10.0, py::arg("retry_beam") = 40.0,
      py::arg("careful") = false);

  m.def(
      "gmm_init_mono",
      [](const HmmTopology &topo, const std::vector<Matrix<BaseFloat>> &feats,
         const std::vector<std::vector<int32>> &shared_phones,
         const std::string model_filename, const std::string tree_filename,
      ...
      py::arg("topo"), py::arg("feats"), py::arg("shared_phones"),
      py::arg("model_filename"), py::arg("tree_filename"),
      py::arg("perturb_factor") = 0.0);

  m.def(
      "gmm_init_model",
      [](const HmmTopology &topo, const ContextDependency &ctx_dep,
         const BuildTreeStatsType &stats, const std::string model_filename,
         double var_floor = 0.01, int32 mixup = 0, int32 mixdown = 0,
         BaseFloat perturb_factor = 0.01, BaseFloat power = 0.2,
        ...
        for (size_t i = 0; i < split_stats.size(); i++) {
          if (split_stats[i].empty()) {
            ...
            KALDI_WARN << "Tree has pdf-id " << i
                       << " with no stats; corresponding phone list: "
                       << ss.str();
            /*
              ...
              of the roots file). Be sure to regenerate roots.int from
              roots.txt, if using s5 scripts. To work out the phone, search for
      ...
      py::arg("topo"), py::arg("ctx_dep"), py::arg("stats"),
      py::arg("model_filename"), py::arg("var_floor") = 0.01,
      py::arg("mixup") = 0, py::arg("mixdown") = 0,
      py::arg("perturb_factor") = 0.1, py::arg("power") = 0.2,
      py::arg("min_count") = 20.0);

  m.def(
      "gmm_init_model_from_previous",
      [](const HmmTopology &topo, const ContextDependency &ctx_dep,
         const BuildTreeStatsType &stats, const AmDiagGmm &old_am_gmm,
         const TransitionModel &old_trans_model,
         const ContextDependency &old_tree, const std::string model_filename,
         double var_floor = 0.01, int32 mixup = 0, int32 mixdown = 0,
         BaseFloat perturb_factor = 0.01, BaseFloat power = 0.2,
         BaseFloat min_count = 20.0) {
        bool binary = true;
        ...
        if (static_cast<int32>(split_stats.size()) !=
        ...
        GaussClusterable *avg_stats_gc =
            dynamic_cast<GaussClusterable *>(avg_stats);
        ...
        for (int32 pdf = 0; pdf < num_pdfs; pdf++) {
          ...
          // that align to this "new" pdf... we'll use it to work out the old
          // pdf-id that's "closest" in stats overlap to this new pdf ("pdf").
          ...
          if (max_count ==
              0) {  // no overlap - probably a leaf with no stats at all.
            KALDI_WARN << "Leaf " << pdf
                       << " of new tree being initialized with "
      ...
      py::arg("topo"), py::arg("ctx_dep"), py::arg("stats"),
      py::arg("old_am_gmm"), py::arg("old_trans_model"), py::arg("old_tree"),
      py::arg("model_filename"), py::arg("var_floor") = 0.01,
      py::arg("mixup") = 0, py::arg("mixdown") = 0,
      py::arg("perturb_factor") = 0.1, py::arg("power") = 0.2,
      py::arg("min_count") = 20.0);
  ...
  m.def(
      "pdf_post_to_gpost",
      [](const Posterior &pdf_post, const AmDiagGmm &am_gmm,
         const Matrix<BaseFloat> &feats, BaseFloat rand_prune = 0.0) {
        py::gil_scoped_release release;
      ...
      py::arg("pdf_post"), py::arg("am_gmm"), py::arg("feats"),
      py::arg("rand_prune") = 0.0, py::return_value_policy::take_ownership);

  m.def(
      "gmm_compute_likes",
      [](const AmDiagGmm &am_gmm, const Matrix<BaseFloat> &features) {
        py::gil_scoped_release release;
