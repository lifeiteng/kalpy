...
void pybind_online_endpoint(py::module &m) {
  {
    ...
    auto online_endpoint_rule = py::class_<PyClass>(m, "OnlineEndpointRule");
    online_endpoint_rule
        .def(py::init<bool, BaseFloat, BaseFloat, BaseFloat>(),
             py::arg("must_contain_nonsilence") = true,
             py::arg("min_trailing_silence") = 1.0,
             py::arg("max_relative_cost") =
...

  {
    ...
    online_endpoint_config.def(py::init<>())
        ...
        .def_readwrite("rule1", &PyClass::rule1,
                       "rule1 times out after 5 seconds of silence, even if we "
                       "decoded nothing.")
        .def_readwrite("rule2", &PyClass::rule2,
                       "rule2 times out after 0.5 seconds of silence if we "
                       "reached the final-state "
                       "with good probability (relative_cost < 2.0) after "
                       "decoding something.")
        .def_readwrite("rule3", &PyClass::rule3,
                       "rule3 times out after 1.0 seconds of silence if we "
                       "reached the final-state "
                       "with OK probability (relative_cost < 8.0) after "
                       "decoding something")
        .def_readwrite("rule4", &PyClass::rule4,
                       "rule4 times out after 2.0 seconds of silence after "
                       "decoding something, "
...
void pybind_online_feature_pipeline(py::module &m) {
  {
    ...
    py::class_<PyClass>(
        ...
        .def_readwrite("global_cmvn_stats_rxfilename",
                       &PyClass::global_cmvn_stats_rxfilename)
...
void pybind_online_gmm_decodable(py::module &m) {
  {
    ...
    auto decodable_diag_gmm_scaled_online =
        py::class_<PyClass>(m, "DecodableDiagGmmScaledOnline");
    decodable_diag_gmm_scaled_online
        .def(py::init<const AmDiagGmm &, const TransitionModel &,
                      const BaseFloat, OnlineFeatureInterface *>(),
             py::arg("am"), py::arg("trans_model"), py::arg("scale"),
             py::arg("input_feats"))
...
void pybind_online_gmm_decoding(py::module &m) {
  {
    ...
    auto online_gmm_decoding_adaptation_policy_config =
        py::class_<PyClass>(m, "OnlineGmmDecodingAdaptationPolicyConfig");
    online_gmm_decoding_adaptation_policy_config.def(py::init<>())
        .def_readwrite("adaptation_first_utt_delay",
                       &PyClass::adaptation_first_utt_delay)
        .def_readwrite("adaptation_first_utt_ratio",
                       &PyClass::adaptation_first_utt_ratio)
        ...
        .def("DoAdapt", &PyClass::DoAdapt,
             ...
             py::arg("chunk_begin_secs"), py::arg("chunk_end_secs"),
...

  {
    ...
    auto online_gmm_decoding_config =
        py::class_<PyClass>(m, "OnlineGmmDecodingConfig");
    online_gmm_decoding_config.def(py::init<>())
        ...
        .def_readwrite("adaptation_policy_opts",
                       &PyClass::adaptation_policy_opts)
        .def_readwrite("online_alimdl_rxfilename",
                       &PyClass::online_alimdl_rxfilename,
                       "rxfilename for model trained with online-CMN features "
                       "(only needed if different from model_rxfilename)")
        .def_readwrite(
            "model_rxfilename", &PyClass::model_rxfilename,
            "rxfilename for model used for estimating fMLLR transforms")
        .def_readwrite("rescore_model_rxfilename",
                       &PyClass::rescore_model_rxfilename,
                       "rxfilename for possible discriminatively trained model"
                       "(only needed if different from model_rxfilename)")
        .def_readwrite(
            "fmllr_basis_rxfilename", &PyClass::fmllr_basis_rxfilename,
  ...
  }
  {
    ...
    auto online_gmm_decoding_models =
        py::class_<PyClass>(m, "OnlineGmmDecodingModels");
    online_gmm_decoding_models
        .def(py::init<const OnlineGmmDecodingConfig &>(), py::arg("config"))
        .def("GetTransitionModel", &PyClass::GetTransitionModel)
        .def("GetOnlineAlignmentModel", &PyClass::GetOnlineAlignmentModel)
        .def("GetModel", &PyClass::GetModel)
        .def("GetFinalModel", &PyClass::GetFinalModel)
        .def("GetFmllrBasis", &PyClass::GetFmllrBasis);
  }
  {
    using PyClass = OnlineGmmAdaptationState;

    auto online_gmm_adaptation_state =
        py::class_<PyClass>(m, "OnlineGmmAdaptationState");
  ...
  }
  {
    ...
    single_utterance_gmm_decoder
        .def(py::init<const OnlineGmmDecodingConfig &,
                      const OnlineGmmDecodingModels &,
                      const OnlineFeaturePipeline &,
                      const fst::Fst<fst::StdArc> &,
                      const OnlineGmmAdaptationState &>(),
             py::arg("config"), py::arg("models"), py::arg("feature_prototype"),
             py::arg("fst"), py::arg("adaptation_state"))
        .def("FeaturePipeline", &PyClass::FeaturePipeline)
        .def("AdvanceDecoding", &PyClass::AdvanceDecoding,
             "advance the decoding as far as we can.  May also estimate fMLLR "
             "after "
             "advancing the decoding, depending on the configuration values in "
             "config_.adaptation_policy_opts.  [Note: we expect the user will "
             "also call "
             "EstimateFmllr() at utterance end, which should generally improve "
             "the "
             "quality of the estimated transforms, although we don't rely on "
             "this].")
        .def("FinalizeDecoding", &PyClass::FinalizeDecoding,
             "Finalize the decoding. Cleanups and prunes remaining tokens, so "
             "the final result "
             "is faster to obtain.")
        .def("HaveTransform", &PyClass::HaveTransform,
             "Returns true if we already have an fMLLR transform.  The user "
             "will "
             "already know this; the call is for convenience.")
        .def("EstimateFmllr", &PyClass::EstimateFmllr,
             "Estimate the [basis-]fMLLR transform and apply it to the "
             "features. "
             ...
             "\"end_of_utterance\" just affects how we interpret the "
             "final-probs in the "
             "lattice.  This should generally be true if you think you've "
             "reached "
             "the end of the grammar, and false otherwise.",
             py::arg("end_of_utterance"))
        .def("GetAdaptationState", &PyClass::GetAdaptationState,
             py::arg("adaptation_state"))
        .def("GetLattice", &PyClass::GetLattice,
             "Gets the lattice.  If rescore_if_needed is true, and if there is "
             "any point "
             "in rescoring the state-level lattice (see RescoringIsNeeded()), "
             "it will "
             "rescore the lattice.  The output lattice has any acoustic "
             "scaling in it "
             "(which will typically be desirable in an online-decoding "
             "context); if you "
             "want an un-scaled lattice, scale it using ScaleLattice() with "
             "the inverse "
             "of the acoustic weight.  \"end_of_utterance\" will be true if "
             "you want the "
             "final-probs to be included.",
             py::arg("rescore_if_needed"), py::arg("end_of_utterance"),
             py::arg("clat"))
        .def("GetBestPath", &PyClass::GetBestPath,
             "Outputs an FST corresponding to the single best path through the "
             "current "
             "lattice. If \"use_final_probs\" is true AND we reached the "
             "final-state of "
             "the graph then it will include those as final-probs, else it "
             "will treat "
             "all final-probs as one.",
             py::arg("end_of_utterance"), py::arg("best_path"))
        .def("FinalRelativeCost", &PyClass::FinalRelativeCost,
             "This function outputs to \"final_relative_cost\", if non-NULL, a "
             "number >= 0 "
             "that will be close to zero if the final-probs were close to the "
             "best probs "
             "active on the final frame.  (the output to final_relative_cost "
             "is based on "
             "the first-pass decoding).  If it's close to zero (e.g. < 5, as a "
             "guess), "
             "it means you reached the end of the grammar with good "
             "probability, which "
             "can be taken as a good sign that the input was OK.")
        .def("EndpointDetected", &PyClass::EndpointDetected,
...
void pybind_online_ivector_feature(py::module &m) {
  {
    ...
    auto online_ivector_extraction_config = py::class_<PyClass>(
        ...
        "configuration class can be used from the command line, but before "
        "giving it "
        "to the code we create a config class called "
        "OnlineIvectorExtractionInfo which contains the actual configuration "
        "classes as well as various objects that are needed.  The principle is "
        "that "
        "any code should be callable from other code, so we didn't want to "
        "force "
    ...
    online_ivector_extraction_config.def(py::init<>())
        ...
        .def_readwrite("splice_config_rxfilename",
                       &PyClass::splice_config_rxfilename,
                       "to read OnlineSpliceOptions")
        .def_readwrite("cmvn_config_rxfilename",
                       &PyClass::cmvn_config_rxfilename,
                       "to read in OnlineCmvnOptions")
        .def_readwrite(
            "online_cmvn_iextractor", &PyClass::online_cmvn_iextractor,
            "flag activating online-cmvn in iextractor feature pipeline")
        .def_readwrite("diag_ubm_rxfilename", &PyClass::diag_ubm_rxfilename,
                       "reads type DiagGmm.")
        .def_readwrite("ivector_extractor_rxfilename",
                       &PyClass::ivector_extractor_rxfilename,
        ...
        .def_readwrite(
            "num_cg_iters", &PyClass::num_cg_iters,
        ...
        .def_readwrite("use_most_recent_ivector",
                       ...
                       "If use_most_recent_ivector is true, we always return "
                       "the most recent "
                       "available iVector rather than the one for the current "
                       "frame.  This means "
                       "that if audio is coming in faster than we can process "
                       "it, we will return a "
                       "more accurate iVector.")
        .def_readwrite("greedy_ivector_extractor",
                       &PyClass::greedy_ivector_extractor,
                       "If true, always read ahead to NumFramesReady() when "
                       "getting iVector stats.")
        .def_readwrite("max_remembered_frames", &PyClass::max_remembered_frames,
                       "max_remembered_frames is the largest number of frames "
                       "it will remember "
                       "between utterances of the same speaker; this affects "
                       "the output of "
                       "GetAdaptationState(), and has the effect of limiting "
                       "the number of frames "
                       "of both the CMVN stats and the iVector stats.  Setting "
                       "this to a smaller "
                       "value means the adaptation is less constrained by "
                       "previous utterances "
                       "(assuming you provided info from a previous utterance "
                       "of the same speaker "
  ...
  }
  {
    ...
    auto online_ivector_extraction_info =
        py::class_<PyClass>(m, "OnlineIvectorExtractionInfo",
                            "This struct contains various things that are "
                            "needed (as const references) "
    ...
    online_ivector_extraction_info.def(py::init<>())
        ...
        .def_readwrite("online_cmvn_iextractor",
                       &PyClass::online_cmvn_iextractor,
                       "flag activating online CMN/CMVN for iextractor input.")
        .def_readwrite(
            "splice_opts", &PyClass::splice_opts,
        ...
        .def_readwrite("greedy_ivector_extractor",
                       &PyClass::greedy_ivector_extractor)
        .def_readwrite("max_remembered_frames", &PyClass::max_remembered_frames)
        .def("Init", &PyClass::Init, py::arg("config"))
        .def("ExpectedFeatureDim", &PyClass::ExpectedFeatureDim)
        .def("Check", &PyClass::Check);
  }
  {
    using PyClass = OnlineIvectorExtractorAdaptationState;

    auto online_ivector_extractor_adaptation_state =
        py::class_<PyClass>(m, "OnlineIvectorExtractorAdaptationState");
    online_ivector_extractor_adaptation_state
        .def(py::init<const OnlineIvectorExtractorAdaptationState &>(),
             py::arg("other"))
        .def(py::init<const OnlineIvectorExtractionInfo &>(), py::arg("info"))
        .def_readwrite("cmvn_state", &PyClass::cmvn_state)
        .def_readwrite("ivector_stats", &PyClass::ivector_stats)
        .def(
            "LimitFrames", &PyClass::LimitFrames,
            "Scales down the stats if needed to ensure the number of frames in "
            "the "
            ...
            "by that factor.]",
            py::arg("max_remembered_frames"), py::arg("posterior_scale"))
  ...
  }
  {
    ...
    auto online_ivector_feature =
        py::class_<OnlineIvectorFeature, OnlineFeatureInterface>(
            m, "OnlineIvectorFeature");
    online_ivector_feature
        .def(py::init<const OnlineIvectorExtractionInfo &,
                      OnlineFeatureInterface *>(),
             py::arg("info"), py::arg("base_feature"))
        .def("Dim", &PyClass::Dim)
        .def("IsLastFrame", &PyClass::IsLastFrame, py::arg("frame"))
        .def("NumFramesReady", &PyClass::NumFramesReady)
        .def("FrameShiftInSeconds", &PyClass::FrameShiftInSeconds)
        .def("GetFrame", &PyClass::GetFrame, py::arg("frame"), py::arg("feat"))
        .def("SetAdaptationState", &PyClass::SetAdaptationState,
             "Set the adaptation state to a particular value, e.g. reflecting "
             "previous "
             "utterances of the same speaker; this will generally be called "
             "after "
             "constructing a new instance of this class.",
             py::arg("adaptation_state"))
        .def("GetAdaptationState", &PyClass::GetAdaptationState,
             "Get the adaptation state; you may want to call this before "
             "destroying this "
             "object, to get adaptation state that can be used to improve "
             "decoding of "
             "later utterances of this speaker.",
             py::arg("adaptation_state"))
        .def("UbmLogLikePerFrame", &PyClass::UbmLogLikePerFrame)
        .def("ObjfImprPerFrame", &PyClass::ObjfImprPerFrame)
        .def("NumFrames", &PyClass::NumFrames)
        .def("UpdateFrameWeights", &PyClass::UpdateFrameWeights,
             "If you are downweighting silence, you can call "
             "OnlineSilenceWeighting::GetDeltaWeights and supply the output to "
             "this class "
             "using UpdateFrameWeights().  The reason why this call happens "
             "outside this "
             "class, rather than this class pulling in the data weights, "
             "relates to "
             "multi-threaded operation and also from not wanting this class to "
             "have "
             "excessive dependencies. "
             "\n"
             "You must either always call this as soon as new data becomes "
             "available "
             "(ideally just after calling AcceptWaveform), or never call it "
             "for the "
  ...
  }
  {
    ...
    auto online_silence_weighting_config =
  ...
  }
  {
    ...
    auto online_silence_weighting =
        py::class_<PyClass>(m, "OnlineSilenceWeighting",
                            "This class is responsible for keeping track of "
                            "the best-path traceback from "
                            "the decoder (efficiently) and computing a "
                            "weighting of the data based on the "
                            "classification of frames as silence (or not "
                            "silence)... also with a duration "
                            "limitation, so data from a very long run of the "
                            "same transition-id will get "
                            "weighted down.  (this is often associated with "
                            "misrecognition or silence).");
    online_silence_weighting
        .def(py::init<const TransitionModel &,
                      const OnlineSilenceWeightingConfig &, int32>(),
             py::arg("trans_model"), py::arg("config"),
        ...
        .def(
            ...
            py::overload_cast<int32, int32,
                ...
                &PyClass::GetDeltaWeights),
            "Calling this function gets the changes in weight that require us "
            "to modify "
            "the stats... the output format is (frame-index, delta-weight). "
            "\n"
            "The num_frames_ready argument is the number of frames available "
            "at "
            "the input (or equivalently, output) of the online iVector feature "
            "in the "
            "feature pipeline from the stream start. It may be more than the "
            "currently "
            "available decoder traceback. "
            "\n"
            "The first_decoder_frame is the offset from the start of the "
            "stream in "
            "pipeline frames when decoder was restarted last time. We do not "
            "change "
            "weight for the frames earlier than first_decoder_frame. Set it to "
            "0 in "
            "case of compilation error to reproduce the previous behavior or "
            "for a "
            "single utterance decoding. "
            "\n"
            "How many frames of weights it outputs depends on how much "
            "\"num_frames_ready\" "
            "increased since last time we called this function, and whether "
            "the decoder "
            "traceback changed.  Negative delta_weights might occur if frames "
            "previously "
            "classified as non-silence become classified as silence if the "
            "decoder's "
            "traceback changes.  You must call this function with "
            "\"num_frames_ready\" "
            "arguments that only increase, not decrease, with time.  You would "
            "provide "
            "this output to class OnlineIvectorFeature by calling its function "
            "UpdateFrameWeights with the output. "
            "\n"
            "Returned frame-index is in pipeline frames from the pipeline "
            "start.",
            py::arg("num_frames_ready"), py::arg("first_decoder_frame"),
            py::arg("delta_weights"))
        .def(
            "GetDeltaWeights",
            ...
            "Returned frame-index is in pipeline frames from the pipeline "
            "start.",
            py::arg("num_frames_ready"), py::arg("delta_weights"))
        .def("GetNonsilenceFrames", &PyClass::GetNonsilenceFrames,
             "Gets a list of nonsilence frames collected on traceback. Useful "
             "for algorithms to extract speaker properties like speaker "
             "identification "
             "vectors.",
             py::arg("num_frames_ready"), py::arg("first_decoder_frame"),
             py::arg("frames"));
...
void pybind_online_nnet2_decoding_threaded(py::module &m) {
  {
    ...
    auto thread_synchronizer = py::class_<PyClass>(
        ...
        "class ThreadSynchronizer acts to guard an arbitrary type of buffer "
        "between a "
        "producing and a consuming thread (note: it's all symmetric between "
        "the two "
        "thread types).  It has a similar interface to a mutex, except that "
        "instead of "
        "just Lock and Unlock, it has Lock, UnlockSuccess and UnlockFailure, "
        "and each "
        "function takes an argument kProducer or kConsumer to identify whether "
        "the "
        "producing or consuming thread is waiting. "
        "\n"
        "The basic concept is that you lock the object; and if you discover "
        "the you're "
        "blocked because you're either trying to read an empty buffer or "
        "trying to "
        "write to a full buffer, you unlock with UnlockFailure; and this will "
        "cause "
        "your next call to Lock to block until the *other* thread has called "
        "Lock and "
        "then UnlockSuccess.  However, if at that point the other thread calls "
        "Lock "
        "and then UnlockFailure, it is an error because you can't have both "
        "producing "
        "and consuming threads claiming that the buffer is full/empty.  If you "
        "lock "
        "the object and were successful you call UnlockSuccess; and you call "
        "UnlockSuccess even if, for your own reasons, you ended up not "
        "changing the "
        "state of the buffer.");
    thread_synchronizer.def(py::init<>())
        .def("Lock", &PyClass::Lock,
             "call this to lock the object being guarded.", py::arg("t"))
        .def("UnlockSuccess", &PyClass::UnlockSuccess,
             "Call this to unlock the object being guarded, if you don't want "
             "the next call to "
             "Lock to stall.",
             py::arg("t"))
        .def("UnlockFailure", &PyClass::UnlockFailure,
             "Call this if you want the next call to Lock() to stall until the "
             "other "
             "(producer/consumer) thread has locked and then unlocked the "
             "mutex.  Note "
             "that, if the other thread then calls Lock and then "
             "UnlockFailure, this will "
             "generate a printed warning (and if repeated too many times, an "
             "exception).",
             py::arg("t"))
        .def("SetAbort", &PyClass::SetAbort,
             "Sets abort_ flag so future calls will return false, and future "
             "calls to "
...

  {
    ...
    auto online_nnet2_decoding_threaded_config = py::class_<PyClass>(
        ...
        "SingleUtteranceNnet2DecoderThreaded.  The "
  ...
  }
  {
    ...
    auto single_utterance_nnet2_decoder_threaded = py::class_<PyClass>(
        ...
        "You will instantiate this class when you want to decode a single "
        "utterance using the online-decoding setup for neural nets.  Each time "
        "this "
        "class is created, it creates three background threads, and the "
        "feature "
        "extraction, neural net evaluation, and search aspects of decoding all "
        "happen in different threads. "
        "Note: we assume that all calls to its public interface happen from a "
        "single "
        "thread.");
    single_utterance_nnet2_decoder_threaded
        .def(py::init<const OnlineNnet2DecodingThreadedConfig &,
                      const TransitionModel &, const nnet2::AmNnet &,
                      const fst::Fst<fst::StdArc> &,
                      const OnlineNnet2FeaturePipelineInfo &,
                      const OnlineIvectorExtractorAdaptationState &,
                      const OnlineCmvnState &>(),
             py::arg("config"), py::arg("tmodel"), py::arg("am_nnet"),
             py::arg("fst"), py::arg("feature_info"),
             py::arg("adaptation_state"), py::arg("cmvn_state"))
        .def("AcceptWaveform", &PyClass::AcceptWaveform,
             "You call this to provide this class with more waveform to "
             "decode.  This "
             "call is, for all practical purposes, non-blocking.",
             py::arg("samp_freq"), py::arg("wave_part"))
        .def("NumWaveformPiecesPending", &PyClass::NumWaveformPiecesPending,
             "Returns the number of pieces of waveform that are still waiting "
             "to be "
             "processed.  This may be useful for calling code to judge whether "
             "to supply "
             "more waveform or to wait.")
        .def("InputFinished", &PyClass::InputFinished,
             "You call this to inform the class that no more waveform will be "
             "provided; "
             "this allows it to flush out the last few frames of features, and "
             "is "
             "necessary if you want to call Wait() to wait until all decoding "
             "is done. "
             "After calling InputFinished() you cannot call AcceptWaveform any "
             "more.")
        .def("TerminateDecoding", &PyClass::TerminateDecoding,
             "You can call this if you don't want the decoding to proceed "
             "further with "
             "this utterance.  It just won't do any more processing, but you "
             "can still "
             "use the lattice from the decoding that it's already done.  Note: "
             "it may "
             "still continue decoding up to decode_batch_size (default: 2) "
             "frames of "
             "data before the decoding thread exits.  You can call Wait() "
             "after calling "
             "this, if you want to wait for that.")
        .def("Wait", &PyClass::Wait,
             "This call will block until all the data has been decoded; it "
             "must only be "
             "called after either InputFinished() has been called or "
             "TerminateDecoding() has "
             "been called; otherwise, to call it is an error.")
        .def("FinalizeDecoding", &PyClass::FinalizeDecoding,
             "Finalizes the decoding. Cleans up and prunes remaining tokens, "
             "so the final "
             "lattice is faster to obtain.  May not be called unless either "
             "InputFinished() "
             "or TerminateDecoding() has been called.  If InputFinished() was "
             "called, it "
             "calls Wait() to ensure that the decoding has finished (it's not "
             "an error "
             "if you already called Wait()).")
        .def("NumFramesReceivedApprox", &PyClass::NumFramesReceivedApprox,
             "Returns *approximately* (ignoring end effects), the number of "
             "frames of "
             "data that we expect given the amount of data that the pipeline "
             "has "
             "received via AcceptWaveform().  (ignores small end effects).  "
             "This might "
             "be useful in application code to compare with NumFramesDecoded() "
             "and gauge "
             "how much latency there is.")
        .def("NumFramesDecoded", &PyClass::NumFramesDecoded,
             "Returns the number of frames currently decoded.  Caution: don't "
             "rely on "
             "the lattice having exactly this number if you get it after this "
             "call, as "
             "it may increase after this-- unless you've already called either "
             "TerminateDecoding() or InputFinished(), followed by Wait().")
        .def("GetLattice", &PyClass::GetLattice,
             "Gets the lattice.  The output lattice has any acoustic scaling "
             "in it "
             "(which will typically be desirable in an online-decoding "
             "context); if you "
             "want an un-scaled lattice, scale it using ScaleLattice() with "
             ...
             "of the acoustic weight.  \"end_of_utterance\" will be true if "
             "you want the "
             "final-probs to be included.  If this is at the end of the "
             "utterance, "
             "you might want to first call FinalizeDecoding() first; this will "
             "make this "
             "call return faster. "
             "If no frames have been decoded yet, it will set clat to a "
             "lattice with "
             "a single state that is final and with unit weight (no cost or "
             "alignment). "
             "The output to final_relative_cost (if non-NULL) is a number >= 0 "
             "that's "
             "closer to 0 if a final-state was close to the best-likelihood "
             "state "
             "active on the last frame, at the time we obtained the lattice.",
             py::arg("end_of_utterance"), py::arg("clat"),
             py::arg("final_relative_cost"))
        .def("GetBestPath", &PyClass::GetBestPath,
             "Outputs an FST corresponding to the single best path through the "
             "current "
             ...
             "will treat "
             "all final-probs as one. "
             "If no frames have been decoded yet, it will set best_path to a "
             "lattice with "
             "a single state that is final and with unit weight (no cost). "
             "The output to final_relative_cost (if non-NULL) is a number >= 0 "
             "that's "
             "closer to 0 if a final-state were close to the best-likelihood "
             "state "
        ...
        .def("EndpointDetected", &PyClass::EndpointDetected,
             ...
             py::arg("config"))
        .def("GetAdaptationState", &PyClass::GetAdaptationState,
             "Outputs the adaptation state of the feature pipeline to "
             "\"adaptation_state\".  This "
             "mostly stores stats for iVector estimation, and will generally "
             "be called at the "
             "end of an utterance, assuming it's a scenario where each speaker "
             "is seen for "
             "more than one utterance. "
             "You may only call this function after either calling "
             "TerminateDecoding() or "
             "InputFinished, and then Wait().  Otherwise it is an error.",
             py::arg("adaptation_state"))
        .def("GetCmvnState", &PyClass::GetCmvnState,
             "Outputs the OnlineCmvnState of the feature pipeline to "
             "\"cmvn_stat\".  This "
             "stores cmvn stats for the non-iVector features, and will be "
             "called at the "
             "end of an utterance, assuming it's a scenario where each speaker "
             "is seen for "
             "more than one utterance. "
             ...
             "InputFinished, and then Wait().  Otherwise it is an error.",
             py::arg("cmvn_state"))
        .def("GetRemainingWaveform", &PyClass::GetRemainingWaveform,
             "Gets the remaining, un-decoded part of the waveform and returns "
             "the sample "
             "rate.  May only be called after Wait(), and it only makes sense "
             "to call "
             "this if you called TerminateDecoding() before Wait().  The idea "
             "is that "
             "you can then provide this un-decoded piece of waveform to "
             "another decoder.",
             py::arg("waveform_out"));
  }
}
...
void pybind_online_nnet2_decoding(py::module &m) {
  {
    ...
    auto online_nnet2_decoding_config =
        py::class_<PyClass>(m, "OnlineNnet2DecodingConfig");
  ...
  }
  {
    ...
    auto single_utterance_nnet2_decoder =
        py::class_<PyClass>(m, "SingleUtteranceNnet2Decoder");
    single_utterance_nnet2_decoder
        .def(
            py::init<const OnlineNnet2DecodingConfig &, const TransitionModel &,
                     const nnet2::AmNnet &, const fst::Fst<fst::StdArc> &,
                     OnlineFeatureInterface *>(),
            py::arg("config"), py::arg("tmodel"), py::arg("model"),
            py::arg("fst"), py::arg("feature_pipeline"))
        .def("AdvanceDecoding", &PyClass::AdvanceDecoding,
             "advance the decoding as far as we can.")
        .def("FinalizeDecoding", &PyClass::FinalizeDecoding,
             "Finalizes the decoding. Cleans up and prunes remaining tokens, "
             "so the "
             "GetLattice() call will return faster.  You must not call this "
             "before "
             "calling (TerminateDecoding() or InputIsFinished()) and then "
             "Wait().")
        .def("NumFramesDecoded", &PyClass::NumFramesDecoded)
        .def("GetLattice", &PyClass::GetLattice,
             "Gets the lattice.  The output lattice has any acoustic scaling "
...
void pybind_online_nnet2_feature_pipeline(py::module &m) {
  {
    ...
    auto online_nnet2_feature_pipeline_config =
    ...
    online_nnet2_feature_pipeline_config.def(py::init<>())
        ...
        .def_readwrite("global_cmvn_stats_rxfilename",
                       &PyClass::global_cmvn_stats_rxfilename)
        .def_readwrite("add_pitch", &PyClass::add_pitch,
                       "Note: if we do add pitch, it will not be added to the "
                       "features we give to "
                       "the iVector extractor but only to the features we give "
                       "to the neural "
                       "network, after the base features but before the "
                       "iVector.  We don't think "
                       "the iVector will be particularly helpful in "
                       "normalizing the pitch features.")
        .def_readwrite(
            "online_pitch_config", &PyClass::online_pitch_config,
            "the following contains the type of options that you could give to "
            "compute-and-process-kaldi-pitch-feats.")
        .def_readwrite("ivector_extraction_config",
                       &PyClass::ivector_extraction_config,
                       "The configuration variables in "
                       "ivector_extraction_config relate to the "
                       "iVector extractor and options related to it, see type "
                       "OnlineIvectorExtractionConfig.")
        .def_readwrite("silence_weighting_config",
                       &PyClass::silence_weighting_config,
                       "Config that relates to how we weight silence for "
                       "(ivector) adaptation "
                       "this is registered directly to the command line as you "
                       "might want to "
  ...
  }
  {
    ...
    auto online_nnet2_feature_pipeline_info = py::class_<PyClass>(
        ...
        "This class is responsible for storing configuration variables, "
        "objects and "
        ...
        "ivector_extractor_info.  This class does not register options on the "
        "command "
        "line; instead, it is initialized from class "
        "OnlineNnet2FeaturePipelineConfig "
        "which reads the options from the command line.  The reason for "
        "structuring "
        "it this way is to make it easier to configure from code as well as "
        "from the "
    ...
    online_nnet2_feature_pipeline_info.def(py::init<>())
        ...
        .def_readwrite(
            "use_ivectors", &PyClass::use_ivectors,
            "If the user specified --ivector-extraction-config, we assume "
            "we're using "
            "iVectors as an extra input to the neural net.  Actually, we don't "
            "anticipate running this setup without iVectors.")
        .def_readonly("ivector_extractor_info",
                      &PyClass::ivector_extractor_info)
        .def_readwrite(
            "silence_weighting_config", &PyClass::silence_weighting_config,
        ...
        .def("GetSamplingFrequency", &PyClass::GetSamplingFrequency,
             "Returns the frequency expected by the model")
        .def("IvectorDim", &PyClass::IvectorDim);
  }
  {
    using PyClass = OnlineNnet2FeaturePipeline;

    auto online_nnet2_feature_pipeline =
        py::class_<OnlineNnet2FeaturePipeline, OnlineFeatureInterface>(
            m, "OnlineNnet2FeaturePipeline");
    online_nnet2_feature_pipeline
        .def(py::init<const OnlineNnet2FeaturePipelineInfo &>(),
             py::arg("info"))
        .def("Dim", &PyClass::Dim,
             "Dim() will return the base-feature dimension (e.g. 13 for normal "
             "MFCC); "
             "plus the pitch-feature dimension (e.g. 3), if used; plus the "
             "iVector "
             "dimension, if used.  Any frame-splicing happens inside the "
             "neural-network "
             "code.")
        .def("IsLastFrame", &PyClass::IsLastFrame, py::arg("info"))
        .def("NumFramesReady", &PyClass::NumFramesReady)
        .def("GetFrame", &PyClass::GetFrame, py::arg("frame"), py::arg("feat"))
        .def("UpdateFrameWeights", &PyClass::UpdateFrameWeights,
             "If you are downweighting silence, you can call "
             "OnlineSilenceWeighting::GetDeltaWeights and supply the output to "
             "this "
             "class using UpdateFrameWeights().  The reason why this call "
             "happens "
             "outside this class, rather than this class pulling in the data "
             "weights, "
             "relates to multi-threaded operation and also from not wanting "
             "this class "
        ...
        .def("GetAdaptationState", &PyClass::GetAdaptationState,
             ...
             "object, to get adaptation state that can be used to improve "
             "decoding of "
             "later utterances of this speaker.  You might not want to do "
             "this, though, "
             "if you have reason to believe that something went wrong in the "
             "recognition "
             "(e.g., low confidence).",
             py::arg("adaptation_state"))
        .def("SetCmvnState", &PyClass::SetCmvnState,
        ...
        .def("AcceptWaveform", &PyClass::AcceptWaveform,
             "Accept more data to process.  It won't actually process it until "
             "you call "
             "GetFrame() [probably indirectly via "
             "(decoder).AdvanceDecoding()], when you "
             "call this function it will just copy it).  sampling_rate is "
             "necessary just "
             "to assert it equals what's in the config.",
             py::arg("sampling_rate"), py::arg("waveform"))
        .def("FrameShiftInSeconds", &PyClass::FrameShiftInSeconds)
        .def("InputFinished", &PyClass::InputFinished,
             "If you call InputFinished(), it tells the class you won't be "
             "providing any "
             "more waveform.  This will help flush out the last few frames of "
             "delta or "
             "LDA features, and finalize the pitch features (making them more "
             "accurate)... although since in neural-net decoding we don't "
             "anticipate "
             "rescoring the lattices, this may not be much of an issue.")
        .def("IvectorFeature", py::overload_cast<>(&PyClass::IvectorFeature),
             "This function returns the iVector-extracting part of the feature "
             "pipeline "
             "(or NULL if iVectors are not being used); the pointer ownership "
             "is retained "
             "by this object and not transferred to the caller.  This function "
             "is used in "
             "nnet3, and also in the silence-weighting code used to exclude "
             "silence from "
             "the iVector estimation.")
        .def(
            "InputFeature", &PyClass::InputFeature,
            "This function returns the part of the feature pipeline that would "
            "be given "
...
void pybind_online_nnet3_decoding(py::module &m) {
  {
    ...
    auto single_utterance_nnet3_decoder =
        py::class_<PyClass>(m, "SingleUtteranceNnet3Decoder");
    single_utterance_nnet3_decoder
        .def(py::init<
                 const LatticeFasterDecoderConfig &, const TransitionModel &,
                 const nnet3::DecodableNnetSimpleLoopedInfo &,
                 const fst::Fst<fst::StdArc> &, OnlineNnet2FeaturePipeline *>(),
             py::arg("decoder_opts"), py::arg("trans_model"), py::arg("info"),
             py::arg("fst"), py::arg("features"))
        .def("InitDecoding", &PyClass::InitDecoding,
             "Initializes the decoding and sets the frame offset of the "
             "underlying "
             "decodable object. This method is called by the constructor. You "
             "can also "
             "call this method when you want to reset the decoder state, but "
             "want to "
             "keep using the same decodable object, e.g. in case of an "
             "endpoint.",
...
void pybind_online_nnet3_incremental_decoding(py::module &m) {
  {
    ...
    auto single_utterance_nnet3_incremental_decoder =
        py::class_<PyClass>(m, "SingleUtteranceNnet3IncrementalDecoder");
    single_utterance_nnet3_incremental_decoder
        ...
        .def("FinalizeDecoding", &PyClass::FinalizeDecoding,
             ...
             "Wait().")
        .def("NumFramesDecoded", &PyClass::NumFramesDecoded)
        .def("NumFramesInLattice", &PyClass::NumFramesInLattice)
        .def(
            "GetLattice", &PyClass::GetLattice,
            "Gets the lattice.  The output lattice has any acoustic scaling in "
            "it "
            "(which will typically be desirable in an online-decoding "
            "context); if you "
            "want an un-scaled lattice, scale it using ScaleLattice() with the "
            "inverse "
            "of the acoustic weight. "
            "\n"
            "    @param [in] num_frames_to_include  The number of frames you "
            "want "
            "              to be included in the lattice.  Must be in the "
            "range "
            "              [NumFramesInLattice().. NumFramesDecoded()].  If "
            "you "
            "              make it a few frames less than NumFramesDecoded(), "
            "it "
            "              will save significant computation. "
            "    @param [in] use_final_probs   True if you want the lattice to "
            "              contain final-probs (if at least one state was "
            "final "
...
void pybind_online_nnet3_wake_word_faster_decoder(py::module &m) {
  ...
  }
  {
    ...
    auto online_wake_word_faster_decoder =
        py::class_<PyClass>(m, "OnlineWakeWordFasterDecoder");
    online_wake_word_faster_decoder
        .def(py::init<const fst::Fst<fst::StdArc> &,
                      const OnlineWakeWordFasterDecoderOpts &>(),
             py::arg("fst"), py::arg("opts"))
        .def("PartialTraceback", &PyClass::PartialTraceback,
             "Makes a linear graph, by tracing back from the last \"immortal\" "
             "token "
             "to the previous one",
             py::arg("out_fst"))
        .def("FinishTraceBack", &PyClass::FinishTraceBack,
             "Makes a linear graph, by tracing back from the best currently "
             "active token "
             "to the last immortal token. This method is meant to be invoked "
             "at the end "
...
void pybind_online_speex_wrapper(py::module &m) {
  {
    ...
    auto speex_options = py::class_<PyClass>(m, "SpeexOptions");
    speex_options.def(py::init<>())
        .def_readwrite(
            "sample_rate", &PyClass::sample_rate,
            "The sample frequency of the waveform, it decides which Speex mode "
            "to use. Often 8kHz---> narrow band, 16kHz---> wide band and 32kHz "
            "---> ultra wide band")
        .def_readwrite(
            "speex_quality", &PyClass::speex_quality,
            "Ranges from 0 to 10, the higher the quality is better. In my "
            "preliminary "
            "tests with the RM recipe, if set it to 8, I observed the WER "
            "increased by "
            "0.1%; while set it to 10, the WER almost kept unchanged.")
        .def_readwrite("speex_bits_frame_size", &PyClass::speex_bits_frame_size,
                       "In bytes. "
                       "Should be set according to speex_quality. Just name a "
                       "few here(wideband): "
                       ...
                       "       10                 106")
        .def_readwrite(
            "speex_wave_frame_size", &PyClass::speex_wave_frame_size,
            "In samples. The Speex toolkit uses a 20ms long window by default");
  }
  {
    using PyClass = OnlineSpeexEncoder;

    auto online_speex_encoder = py::class_<PyClass>(m, "OnlineSpeexEncoder");
    online_speex_encoder
        .def(py::init<const SpeexOptions &>(), py::arg("config"))
        .def("AcceptWaveform", &PyClass::AcceptWaveform, py::arg("sample_rate"),
             py::arg("waveform"))
        .def("InputFinished", &PyClass::InputFinished)
        .def("GetSpeexBits", &PyClass::GetSpeexBits, py::arg("spx_bits"));
  }
  {
    using PyClass = OnlineSpeexDecoder;

    auto online_speex_decoder = py::class_<PyClass>(m, "OnlineSpeexDecoder");
    online_speex_decoder
        .def(py::init<const SpeexOptions &>(), py::arg("config"))
        .def("AcceptSpeexBits", &PyClass::AcceptSpeexBits,
             py::arg("spx_enc_bits"))
        .def("GetWaveform", &PyClass::GetWaveform, py::arg("waveform"));
  }
}
...
void pybind_online_timing(py::module &m) {
  {
    ...
    auto online_timing_stats = py::class_<PyClass>(
        m, "OnlineTimingStats",
        "class OnlineTimingStats stores statistics from timing of online "
        "decoding, "
        "which will enable the Print() function to print out the average "
        "real-time "
        "factor and average delay per utterance.  See class OnlineTimer.");
    online_timing_stats.def(py::init<>())
        .def("Print", &PyClass::Print, py::arg("online") = true);
  }
  {
    ...
    auto online_timer = py::class_<PyClass>(
        m, "OnlineTimer",
        "class OnlineTimer is used to test real-time decoding algorithms and "
        "evaluate "
        "how long the decoding of a particular utterance would take.  The "
        "'obvious' "
        "way to evaluate this would be to measure the wall-clock time, and if "
        "we're "
        "processing the data in chunks, to sleep() until a given chunk would "
        "become "
        "available in a real-time application-- e.g. say we need to process a "
        "chunk "
        "that ends half a second into the utterance, we would sleep until half "
        "a "
    ...
    online_timer.def(py::init<const std::string &>(), py::arg("utterance_id"))
        .def(
            "SleepUntil", &PyClass::SleepUntil,
            "The call to SleepUntil(t) will sleep until cur_utterance_length "
            "seconds "
        ...
        .def("Elapsed", &PyClass::Elapsed,
             "Returns the simulated time elapsed in seconds since the timer "
             "was started; "
...
void pybind_online2bin_util(py::module &m) {
  m.def("ReadDecodeGraph", &ReadDecodeGraph,
        "Reads a decoding graph from a file", py::arg("filename"));
  m.def(
      "PrintPartialResult", &PrintPartialResult,
      ...
      py::arg("words"), py::arg("word_syms"), py::arg("line_break"));
}
