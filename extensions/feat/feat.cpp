...
void feat_signal(py::module &m) {
  m.def(
      "ConvolveSignals", &ConvolveSignals,
      "This function implements a simple non-FFT-based convolution of two "
      "signals. "
      "It is suggested to use the FFT-based convolution function which is more "
      "efficient.",
      py::arg("filter"), py::arg("signal"),
      py::call_guard<py::gil_scoped_release>());

  m.def(
      "FFTbasedConvolveSignals", &FFTbasedConvolveSignals,
  ...
  m.def("FFTbasedBlockConvolveSignals", &FFTbasedBlockConvolveSignals,
        "This function implements FFT-based block convolution of two signals "
        "using "
...
void feat_feat_functions(py::module &m) {
  m.def("ComputePowerSpectrum", &ComputePowerSpectrum,
        ...
        "a power spectrum.  If the complex FFT is a vector of size n "
        "(representing "
        ...
        py::arg("complex_fft"), py::call_guard<py::gil_scoped_release>());
  ...
  py::class_<DeltaFeatures>(
      ...
      "of type VectorBase, of size (original-feature-dimension) * "
      "(opts.order+1). "
      "This is not the most efficient way to do the computation, but it's "
      "state-free and thus easier to understand")
      .def(py::init<const DeltaFeaturesOptions &>(), py::arg("opts"))
      .def("Process", &DeltaFeatures::Process, py::arg("input_feats"),
           py::arg("frame"), py::arg("output_frame"),
           py::call_guard<py::gil_scoped_release>());
  ...
  py::class_<ShiftedDeltaFeatures>(
      m, "ShiftedDeltaFeatures",
      "This class provides a low-level function to compute shifted "
      "delta cesptra (SDC). "
      "The function takes as input a matrix of features and a frame index "
      "that it should compute the deltas on.  It puts its output in an object "
      "of type VectorBase, of size original-feature-dimension + (1  * "
      "num_blocks).")
      .def(py::init<const ShiftedDeltaFeaturesOptions &>(), py::arg("opts"))
      .def("Process", &ShiftedDeltaFeatures::Process, py::arg("input_feats"),
           py::arg("frame"), py::arg("output_frame"));

  m.def("ComputeDeltas", &ComputeDeltas,
        "ComputeDeltas is a convenience function that computes deltas on a "
        "feature "
        "file.  If you want to deal with features coming in bit by bit you "
        "would have "
        "to use the DeltaFeatures class directly, and do the computation frame "
        "by "
        "frame.  Later we will have to come up with a nice mechanism to do "
        "this for "
        "features coming in.",
        py::arg("delta_opts"), py::arg("input_features"),
        py::arg("output_features"), py::call_guard<py::gil_scoped_release>());
  ...
  m.def(
      ...
      [](const DeltaFeaturesOptions &delta_opts,
        ...
        ComputeDeltas(delta_opts, input_features, &output_features);
        return output_features;
      },
      "ComputeDeltas is a convenience function that computes deltas on a "
      "feature "
      "file.  If you want to deal with features coming in bit by bit you would "
      "have "
      "to use the DeltaFeatures class directly, and do the computation frame "
      "by "
      "frame.  Later we will have to come up with a nice mechanism to do this "
      "for "
      "features coming in.",
      py::arg("delta_opts"), py::arg("input_features"));

  m.def("ComputeShiftedDeltas", &ComputeShiftedDeltas,
  ...
  m.def("SpliceFrames", &SpliceFrames,
        ...
        "a good choice).",
        py::arg("input_features"), py::arg("left_context"),
        py::arg("right_context"), py::arg("output_features"),
        py::call_guard<py::gil_scoped_release>());

  m.def(
      "splice_frames",

      [](const MatrixBase<BaseFloat> &input_features, int32 left_context,
         int32 right_context) {
        py::gil_scoped_release gil_release;
        Matrix<BaseFloat> output_features;
        SpliceFrames(input_features, left_context, right_context,
                     &output_features);
  ...
  m.def("InitIdftBases", &InitIdftBases, py::arg("n_bases"),
        py::arg("dimension"), py::arg("mat_out"));

  m.def(
      "paste_feats",
      [](const std::vector<Matrix<BaseFloat>> &in, int32 tolerance) {
        py::gil_scoped_release gil_release;
  ...
  py::class_<SlidingWindowCmnOptions>(m, "SlidingWindowCmnOptions")
      ...
      .def_readwrite(
          "min_window", &SlidingWindowCmnOptions::min_window,
          "Minimum CMN window "
      ...
      .def_readwrite("normalize_variance",
                     &SlidingWindowCmnOptions::normalize_variance,
                     "If true, normalize "
                     "variance to one.")
      .def_readwrite(
          "center", &SlidingWindowCmnOptions::center,
  ...
  m.def("SlidingWindowCmn", &SlidingWindowCmn,
        "Applies sliding-window cepstral mean and/or variance normalization.  "
        "See the "
        "strings registering the options in the options class for information "
        "on how "
        "this works and what the options are.  input and output must have the "
        "same "
        "dimension.",
        py::arg("opts"), py::arg("input"), py::arg("output"));
  ...
  m.def(
      ...
      [](const SlidingWindowCmnOptions &opts,
        ...
        py::gil_scoped_release gil_release;
        Matrix<BaseFloat> output(input.NumRows(), input.NumCols(), kUndefined);
        SlidingWindowCmn(opts, input, &output);
        return output;
      },
      "Applies sliding-window cepstral mean and/or variance normalization.  "
      "See the "
      "strings registering the options in the options class for information on "
      "how "
      "this works and what the options are.  input and output must have the "
      "same "
      "dimension.",
...
void feat_pitch_functions(py::module &m) {
  py::class_<PitchExtractionOptions>(m, "PitchExtractionOptions")
      ...
      .def_readwrite("frame_length_ms",
                     &PitchExtractionOptions::frame_length_ms,
                     "Frame shift in "
                     "milliseconds")
      ...
      .def_readwrite("lowpass_filter_width",
                     &PitchExtractionOptions::lowpass_filter_width,
                     "Integer that determines filter width of "
                     "lowpass filter, more gives sharper filter")
      .def_readwrite(
          "upsample_filter_width",
          &PitchExtractionOptions::upsample_filter_width,
          "Integer that determines filter width when upsampling NCCF")
      .def_readwrite(
          "max_frames_latency", &PitchExtractionOptions::max_frames_latency,
          "Maximum number "
      ...
      .def_readwrite(
          "frames_per_chunk", &PitchExtractionOptions::frames_per_chunk,
          "Only relevant for "
      ...
      .def_readwrite(
          ...
          &PitchExtractionOptions::simulate_first_pass_online,
          ...
          "--frames-per-chunk > 0")
      .def_readwrite(
          "recompute_frame", &PitchExtractionOptions::recompute_frame,
          "Only relevant for "
          ...
          "--frames-per-chunk > 0")
      .def_readwrite("nccf_ballast_online",
                     &PitchExtractionOptions::nccf_ballast_online,
      ...
      .def("NccfWindowSize", &PitchExtractionOptions::NccfWindowSize,
           ...
           "\"basic window size\", not the full window size after extending by "
           "max-lag. "
           "Because of floating point representation, it is more reliable to "
           "divide "
      ...
      .def(py::pickle(
          [](const PitchExtractionOptions &p) {  // __getstate__
            ...
            return py::make_tuple(
                p.samp_freq, p.frame_shift_ms, p.frame_length_ms,
                p.preemph_coeff, p.min_f0, p.max_f0, p.soft_min_f0,
                p.penalty_factor, p.lowpass_cutoff, p.resample_freq,
                p.delta_pitch, p.nccf_ballast, p.lowpass_filter_width,
                p.upsample_filter_width, p.max_frames_latency,
                p.frames_per_chunk, p.simulate_first_pass_online,
                p.recompute_frame, p.nccf_ballast_online, p.snip_edges);
          },
          [](py::tuple t) {  // __setstate__
            if (t.size() != 20) throw std::runtime_error("Invalid state!");
  ...
  py::class_<ProcessPitchOptions>(m, "ProcessPitchOptions")
      ...
      .def_readwrite("pov_offset", &ProcessPitchOptions::pov_offset,
                     ...
                     " CMN.")
      .def_readwrite("delta_pitch_scale",
                     &ProcessPitchOptions::delta_pitch_scale,
                     "Term to scale the final delta log-pitch feature")
      .def_readwrite(
          "delta_pitch_noise_stddev",
          &ProcessPitchOptions::delta_pitch_noise_stddev,
          ...
          "values.")
      .def_readwrite("normalization_left_context",
                     &ProcessPitchOptions::normalization_left_context,
                     "Left-context (in frames) for moving window normalization")
      .def_readwrite(
          "normalization_right_context",
          &ProcessPitchOptions::normalization_right_context,
          "Right-context (in frames) for moving window normalization")
      .def_readwrite(
          "delta_window", &ProcessPitchOptions::delta_window,
      ...
      .def_readwrite(
          ...
          &ProcessPitchOptions::add_normalized_log_pitch,
      ...
      .def_readwrite("add_delta_pitch", &ProcessPitchOptions::add_delta_pitch,
                     ...
                     "features")
      .def_readwrite("add_raw_log_pitch",
                     &ProcessPitchOptions::add_raw_log_pitch,
                     "If true, log(pitch) is added to output features")
      .def(py::pickle(
          [](const ProcessPitchOptions &p) {  // __getstate__
            /* Return a tuple that fully encodes the state of the object */
            return py::make_tuple(
                p.pitch_scale, p.pov_scale, p.pov_offset, p.delta_pitch_scale,
                p.delta_pitch_noise_stddev, p.normalization_left_context,
                p.normalization_right_context, p.delta_window, p.delay,
                p.add_pov_feature, p.add_normalized_log_pitch,
                p.add_delta_pitch, p.add_raw_log_pitch);
          },
          [](py::tuple t) {  // __setstate__
          ...
          }));
  {
    ...
    auto online_pitch_feature = py::class_<PyClass>(m, "OnlinePitchFeature");
    online_pitch_feature
        .def(py::init<const PitchExtractionOptions &>(), py::arg("opts"))
        .def("Dim", &PyClass::Dim)
  ...
  }
  {
    ...
    auto online_process_pitch =
        py::class_<PyClass, OnlineFeatureInterface>(m, "OnlineProcessPitch");
    online_process_pitch
        .def(py::init<const ProcessPitchOptions &, OnlineFeatureInterface *>(),
  ...
  m.def(
      "ComputeKaldiPitch", &ComputeKaldiPitch,
      "This function extracts (pitch, NCCF) per frame, using the pitch "
      "extraction "
      "method described in \"A Pitch Extraction Algorithm Tuned for Automatic "
      "Speech "
      "Recognition\", Pegah Ghahremani, Bagher BabaAli, Daniel Povey, "
      "Korbinian "
      "Riedhammer, Jan Trmal and Sanjeev Khudanpur, ICASSP 2014.  The output "
      "will "
      "have as many rows as there are frames, and two columns corresponding to "
      "(NCCF, pitch)",
      py::arg("opts"), py::arg("wave"), py::arg("output"));
  m.def("ProcessPitch", &ProcessPitch,
        "This function processes the raw (NCCF, pitch) quantities computed by "
        "ComputeKaldiPitch, and processes them into features.  By default it "
        "will "
        "output three-dimensional features, (POV-feature, "
        "mean-subtracted-log-pitch, "
        "delta-of-raw-pitch), but this is configurable in the options.  The "
        "number of "
        "rows of \"output\" will be the number of frames (rows) in \"input\", "
        "and the "
        ...
        "--add-raw-log-pitch determine which features we create; by default we "
        "create "
        "the first three.",
        py::arg("opts"), py::arg("input"), py::arg("output"));
  m.def(
      "ComputeAndProcessKaldiPitch", &ComputeAndProcessKaldiPitch,
      ...
      "training models matched to the \"first-pass\" features.  It is "
      "sensitive to "
      ...
      "recompute_frame.",
      py::arg("pitch_opts"), py::arg("process_opts"), py::arg("wave"),
      py::arg("output"));
  m.def(
      "compute_pitch",
      [](py::array_t<float> wave, const PitchExtractionOptions &pitch_opts,
         const ProcessPitchOptions &process_opts) {
        auto r = wave.unchecked<1>();
        auto vector = Vector<float>(r.shape(0));
        Matrix<float> features;
        for (py::size_t i = 0; i < r.shape(0); i++) vector(i) = r(i);
        ComputeAndProcessKaldiPitch(pitch_opts, process_opts, vector,
                                    &features);
        return features;
      },
      "This function combines ComputeKaldiPitch and ProcessPitch.  The reason "
      ...
      "i.e. max_frames_latency, frames_per_chunk, simulate_first_pass_online, "
      "recompute_frame.",
      py::arg("wave"), py::arg("pitch_opts"), py::arg("process_opts"));
}

template <class Feature>
void online_base_feature(py::module &m, const std::string &feat_type) {
  py::class_<OnlineGenericBaseFeature<Feature>, OnlineBaseFeature>(
      m, feat_type.c_str())
...
void init_feat(py::module &_m) {
  ...
  py::class_<FrameExtractionOptions>(m, "FrameExtractionOptions")
      ...
      .def(py::pickle(
          [](const FrameExtractionOptions &p) {  // __getstate__
            /* Return a tuple that fully encodes the state of the object */
            return py::make_tuple(
                p.samp_freq, p.frame_shift_ms, p.frame_length_ms, p.dither,
                p.preemph_coeff, p.remove_dc_offset, p.window_type,
                p.round_to_power_of_two, p.blackman_coeff, p.snip_edges,
                p.allow_downsample, p.allow_upsample, p.max_feature_vectors);
          },
          [](py::tuple t) {  // __setstate__
            if (t.size() != 13) throw std::runtime_error("Invalid state!");
  ...
  py::class_<MelBanksOptions>(m, "MelBanksOptions")
      ...
      .def(py::pickle(
          [](const MelBanksOptions &p) {  // __getstate__
            /* Return a tuple that fully encodes the state of the object */
            return py::make_tuple(p.num_bins, p.low_freq, p.high_freq,
                                  p.vtln_low, p.vtln_high, p.debug_mel,
                                  p.htk_mode);
          },
          [](py::tuple t) {  // __setstate__
  ...
  py::class_<MfccOptions>(m, "MfccOptions")
      ...
      .def(py::pickle(
          [](const MfccOptions &p) {  // __getstate__
            /* Return a tuple that fully encodes the state of the object */
            return py::make_tuple(p.frame_opts, p.mel_opts, p.num_ceps,
                                  p.use_energy, p.energy_floor, p.raw_energy,
                                  p.cepstral_lifter, p.htk_compat);
          },
          [](py::tuple t) {  // __setstate__
  ...
  py::class_<OnlineFeatureInterface>(
      m, "OnlineFeatureInterface",
      ...
      "How it obtains the features, e.g. from a previous object or objects of "
      "type"
      "OnlineFeatureInterface, is not specified in the interface and you will"
      "likely define new constructors or methods in the derived type to do "
      "that.")
  ...
  py::class_<OnlineBaseFeature, OnlineFeatureInterface>(m, "OnlineBaseFeature")
      .def("AcceptWaveform", &OnlineBaseFeature::AcceptWaveform,
           "This would be called from the application, when you get more wave "
           "data."
           "Note: the sampling_rate is typically only provided so the code can "
           "assert"
  ...
  py::class_<OnlineSpliceFrames, OnlineFeatureInterface>(m,
                                                         "OnlineSpliceFrames")
  ...
  py::class_<OnlineCacheFeature, OnlineFeatureInterface>(m,
                                                         "OnlineCacheFeature")
      .def(py::init<OnlineFeatureInterface *>(), py::arg("src"))
      .def("ClearCache", &OnlineCacheFeature::ClearCache);

  py::class_<OnlineAppendFeature, OnlineFeatureInterface>(m,
                                                          "OnlineAppendFeature")
  ...
  py::class_<WaveInfo>(m, "WaveInfo")
      .def(py::init<>())
      .def(
          "IsStreamed", &WaveInfo::IsStreamed,
          "Is stream size unknown? Duration and SampleCount not valid if true.")
      .def("SampFreq", &WaveInfo::SampFreq, "Sample frequency, Hz.")
      ...
      .def("BlockAlign", &WaveInfo::BlockAlign, "Bytes per sample.")
  ...
  py::class_<WaveData>(m, "WaveData")
      .def(py::init<>())
      .def(py::init<const float, const Matrix<float>>(), py::arg("samp_freq"),
           py::arg("data"))
      .def("Duration", &WaveData::Duration, "Returns the duration in seconds")
      ...
      .def("Read", &WaveData::Read, py::call_guard<py::gil_scoped_release>())
  ...
  pybind_random_access_table_reader<WaveInfoHolder>(
      m, "RandomAccessWaveInfoReader");
  feat_pitch_functions(m);
  feat_feat_functions(m);
  feat_signal(m);

  m.def(
      "select_voiced_frames",
      [](const Matrix<BaseFloat> &feat, const Vector<BaseFloat> &voiced) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("feat"), py::arg("voiced"));

  m.def(
      "subsample_feats",
      [](const Matrix<BaseFloat> &feats, int32 n = 1, int32 offset = 0) {
        py::gil_scoped_release gil_release;
      ...
      py::arg("feats"), py::arg("n") = 1, py::arg("offset") = 0);
}
